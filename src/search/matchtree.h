...
class MatchAtom {
  ...
  **/
  ...
  virtual MatchAtomOperator* as_operator() { return NULLPTR; }

  virtual MatchAtomTest* as_test() { return NULLPTR; }
};
...
class MatchAtomOperator : public MatchAtom {
 ...
 public:
  explicit MatchAtomOperator(AtomOperator op) : m_operator(op), m_left(NULLPTR), m_right(NULLPTR) {}

  ~MatchAtomOperator();

  bool match(PackageReader* p) OVERRIDE;

  MatchAtomOperator* as_operator() OVERRIDE { return this; }
};
...
class MatchAtomTest : public MatchAtom {
 ...
 public:
  ...
  MatchAtomTest* as_test() OVERRIDE { return this; }
};
...
class MatchParseData {
  ...
  **/
  ...
  ATTRIBUTE_NONNULL((2)) explicit MatchParseData(MatchAtom** p) : parent(p), subroot(NULLPTR), useright(false), negatebrace(false) {}
};
