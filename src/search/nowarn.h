...
class NowarnFlags {
 ...
 public:
  ...
  void apply_red(Keywords::Redundant* r) const { *r &= ~red; }

  void apply_ins(PackageTest::TestInstalled* i) const { *i &= ~ins; }
...
class NowarnMaskList FINAL : public MaskList<NowarnMask> {
 ...
 public:
  void apply(Package* p, Keywords::Redundant* r, PackageTest::TestInstalled* i, CorePkgSettings* corepkgsettings) const;
};
...
class NowarnPreList FINAL : public PreList {
 ...
 public:
  ...
  NowarnPreList(const LineVec& lines, const std::string& filename, bool only_add) : super(lines, filename, NULLPTR, only_add, false) {}
