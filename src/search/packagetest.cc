...
const PackageTest::MatchField PackageTest::NONE, PackageTest::NAME, PackageTest::DESCRIPTION, PackageTest::LICENSE, PackageTest::CATEGORY, PackageTest::CATEGORY_NAME, PackageTest::HOMEPAGE,
    PackageTest::IUSE, PackageTest::USE_ENABLED, PackageTest::USE_DISABLED, PackageTest::SRC_URI, PackageTest::EAPI, PackageTest::INST_EAPI, PackageTest::SLOT, PackageTest::FULLSLOT,
    PackageTest::INST_SLOT, PackageTest::INST_FULLSLOT, PackageTest::SET, PackageTest::DEPENDA, PackageTest::RDEPENDA, PackageTest::PDEPENDA, PackageTest::BDEPENDA, PackageTest::IDEPENDA,
    PackageTest::DEPENDI, PackageTest::RDEPENDI, PackageTest::PDEPENDI, PackageTest::BDEPENDI, PackageTest::IDEPENDI, PackageTest::DEPEND, PackageTest::RDEPEND, PackageTest::PDEPEND,
    PackageTest::BDEPEND, PackageTest::IDEPEND, PackageTest::DEPSA, PackageTest::DEPSI, PackageTest::DEPS, PackageTest::ANY;

const PackageTest::TestInstalled PackageTest::INS_NONE, PackageTest::INS_NONEXISTENT, PackageTest::INS_OVERLAY, PackageTest::INS_MASKED;

const PackageTest::TestStability PackageTest::STABLE_NONE, PackageTest::STABLE_FULL, PackageTest::STABLE_TESTING, PackageTest::STABLE_NONMASKED, PackageTest::STABLE_SYSTEM,
    PackageTest::STABLE_PROFILE, PackageTest::STABLE_SYSTEMPROFILE;
...
PackageTest::PackageTest(VarDbPkg* vdb, CorePkgSettings* p, const PrintFormat* f, const SetStability* set_stability, const DBHeader* dbheader, const ParseError* e) {
  vardbpkg = vdb;
  corepkgsettings = p;
  print_format = f;
  ...
  overlay = obsolete = upgrade = installed = multi_installed = slotted = multi_slot = world = world_only_selected = world_only_file = worldset = worldset_only_selected = dup_versions = dup_packages =
      have_virtual = have_nonvirtual = know_pattern = false;
  restrictions = ExtendedVersion::RESTRICT_NONE;
  properties = ExtendedVersion::PROPERTIES_NONE;
  binarynum = 0;
  test_installed = INS_NONE;
  test_instability = test_stability_default = test_stability_local = test_stability_nonlocal = STABLE_NONE;
}
...
void PackageTest::calculateNeeds() {
  ...
  if (((field & (IUSE | DEPSA)) != NONE) || dup_packages || dup_versions || slotted || upgrade || overlay || obsolete || world || worldset || have_virtual || have_nonvirtual ||
      (from_overlay_inst_list != NULLPTR) || (from_foreign_overlay_inst_list != NULLPTR) || (overlay_list != NULLPTR) || (overlay_only_list != NULLPTR) || (in_overlay_inst_list != NULLPTR) ||
      (marked_list != NULLPTR) || (restrictions != ExtendedVersion::RESTRICT_NONE) || (properties != ExtendedVersion::PROPERTIES_NONE) || (binarynum != 0) || (test_instability != STABLE_NONE) ||
      (test_stability_default != STABLE_NONE) || (test_stability_local != STABLE_NONE) || (test_stability_nonlocal != STABLE_NONE))
    setNeeds(PackageReader::VERSIONS);
}
...
bool PackageTest::stringMatch(Package* pkg) const {
  if ((((field & NAME) != NONE) && (*algorithm)(pkg->name.c_str(), pkg, true)) || (((field & DESCRIPTION) != NONE) && (*algorithm)(pkg->desc.c_str(), pkg)) ||
      (((field & LICENSE) != NONE) && (*algorithm)(pkg->licenses.c_str(), pkg)) || (((field & CATEGORY) != NONE) && (*algorithm)(pkg->category.c_str(), pkg, true)) ||
      (((field & CATEGORY_NAME) != NONE) && (*algorithm)((pkg->category + "/" + pkg->name).c_str(), pkg, true)) || (((field & HOMEPAGE) != NONE) && (*algorithm)(pkg->homepage.c_str(), pkg))) {
    return true;
  }
  ...
  if ((field & IUSE) != NONE) {
    ...
    for (IUseSet::IUseStd::const_iterator it(s.begin()); it != s.end(); ++it) {
  ...
  if ((field & DEPSA) != NONE) {
    ...
    for (Package::iterator it(pkg->begin()); likely(it != pkg->end()); ++it) {
      ...
      if ((depend && (*algorithm)(dep.get_depend().c_str(), pkg)) || (rdepend && (*algorithm)(dep.get_rdepend().c_str(), pkg)) || (pdepend && (*algorithm)(dep.get_pdepend().c_str(), pkg)) ||
          (bdepend && (*algorithm)(dep.get_bdepend().c_str(), pkg)) || (idepend && (*algorithm)(dep.get_idepend().c_str(), pkg))) {
        return true;
      }
    }
  }
  ...
  if ((field & SET) != NONE) {
    WordSet setnames;
    corepkgsettings->get_setnames(&setnames, pkg);
    for (WordSet::const_iterator it(setnames.begin()); likely(it != setnames.end()); ++it) {
      if ((*algorithm)(it->c_str(), NULLPTR) || (*algorithm)((string("@") + *it).c_str(), NULLPTR)) {
        return true;
      }
    }
  }
  ...
  if ((field & INST_EAPI) != NONE) {
    for (InstVec::iterator it(installed_versions->begin()); likely(it != installed_versions->end()); ++it) {
  ...
  if ((field & (USE_ENABLED | USE_DISABLED)) != NONE) {
    for (InstVec::iterator it(installed_versions->begin()); it != installed_versions->end(); ++it) {
      if (!vardbpkg->readUse(*pkg, &(*it))) {
        continue;
      }
      if ((field & USE_ENABLED) != NONE) {
        for (WordSet::iterator uit((it->usedUse).begin()); likely(uit != (it->usedUse).end()); ++uit) {
      ...
      if ((field & USE_DISABLED) != NONE) {
        for (WordVec::iterator uit((it->inst_iuse).begin()); likely(uit != (it->inst_iuse).end()); ++uit) {
          if (!(*algorithm)(uit->c_str(), NULLPTR)) {
            continue;
          }
...
bool PackageTest::have_redundant(const Package& p, Keywords::Redundant r, const RedAtom& t) const {
  ...
  if (r & t.all) {
    ...
    for (Package::const_reverse_iterator pi(p.rbegin()); likely(pi != p.rend()); prev_ver = *pi, ++pi) {
      ...
      if (((pi->get_redundant()) & r) != Keywords::RED_NOTHING) {
        ...
        coreq::SignedBool is_installed(vardbpkg->isInstalledVersion(p, *pi, *header));
    ...
    for (Package::const_iterator pi(p.begin()); likely(pi != p.end()); ++pi) {
      if ((pi->get_redundant()) & r) {
        ...
        if (test_uninstalled == !(vardbpkg->isInstalledVersion(p, *pi, *header))) {
          return true;
        }
      }
    }
...
bool PackageTest::match(PackageReader* pkg) const {
  ...
  2. Call one of
     ...
     (Default means according to LOCAL_COREPKG_CONFIG,
     Nonlocal means as with LOCAL_COREPKG_CONFIG=false)
  ...
  if (unlikely((from_overlay_inst_list != NULLPTR) || (from_foreign_overlay_inst_list != NULLPTR))) {
  ...
  if (unlikely(dup_versions)) {
    // -D
    get_p(&p, pkg);
    Package::Duplicates testfor((dup_versions_overlay) ? Package::DUP_OVERLAYS : Package::DUP_SOME);
    if (((p->have_duplicate_versions) & testfor) != testfor) {
      return false;
    }
  }

  if (unlikely((restrictions != ExtendedVersion::RESTRICT_NONE) || (properties != ExtendedVersion::PROPERTIES_NONE))) {
    get_p(&p, pkg);
    bool found(false);
    for (Package::iterator it(p->begin()); likely(it != p->end()); ++it) {
      if (unlikely((((it->restrictFlags) & restrictions) == restrictions) && (((it->propertiesFlags) & properties) == properties))) {
        found = true;
        break;
      }
    }
  ...
  if (binarynum != 0) {
    ...
    for (Package::iterator it(p->begin()); it != p->end(); ++it) {
      if (unlikely(it->have_bin_pkg(corepkgsettings, p, binarynum))) {
        found = true;
        break;
      }
    }
  ...
  while (unlikely(obsolete)) {
    ...
    if ((test_installed == INS_NONE) && (redundant_flags == Keywords::RED_NOTHING)) {
      return false;
    }
    ...
    nowarn_list->apply(p, &rflags, &test_ins, corepkgsettings);

    Keywords::Redundant r(rflags & Keywords::RED_ALL_MASKSTUFF);
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->setMasks(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_MASK) || have_redundant(*p, r & Keywords::RED_DOUBLE_UNMASK) || have_redundant(*p, r & Keywords::RED_MASK) ||
            have_redundant(*p, r & Keywords::RED_UNMASK) || have_redundant(*p, r & Keywords::RED_IN_MASK) || have_redundant(*p, r & Keywords::RED_IN_UNMASK)) {
          break;
        }
      }
    }
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->setKeyflags(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE) || have_redundant(*p, r & Keywords::RED_MIXED) || have_redundant(*p, r & Keywords::RED_WEAKER) ||
            have_redundant(*p, r & Keywords::RED_STRANGE) || have_redundant(*p, r & Keywords::RED_NO_CHANGE) || have_redundant(*p, r & Keywords::RED_IN_KEYWORDS) ||
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->CheckUse(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_USE) || have_redundant(*p, r & Keywords::RED_IN_USE)) {
          break;
        }
      }
    }
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->CheckEnv(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_ENV) || have_redundant(*p, r & Keywords::RED_IN_ENV)) {
          break;
        }
      }
    }
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->CheckLicense(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_LICENSE) || have_redundant(*p, r & Keywords::RED_IN_LICENSE)) {
          break;
        }
      }
    }
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->CheckAcceptRestrict(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_RESTRICT) || have_redundant(*p, r & Keywords::RED_IN_RESTRICT)) {
          break;
        }
      }
    }
    ...
    if (r != Keywords::RED_NOTHING) {
      if (r && corepkgsettings->user_config->CheckCflags(p, r)) {
        if (have_redundant(*p, r & Keywords::RED_DOUBLE_CFLAGS) || have_redundant(*p, r & Keywords::RED_IN_CFLAGS)) {
          break;
        }
      }
    }
    ...
    if ((test_ins & INS_MASKED) != INS_NONE) {
      corepkgsettings->user_config->setMasks(p);
      corepkgsettings->user_config->setKeyflags(p);
    }
    ...
    for (; likely(current != installed_versions->end()); ++current) {
      ...
      for (Package::iterator version_it(p->begin()); likely(version_it != p->end()); ++version_it) {
  ...
  if (unlikely(upgrade)) {
    ...
    if (!(p->recommend(vardbpkg, corepkgsettings, true, true))) {
      return false;
    }
  }
