...
class PackageTest {
 ...
 public:
  ...
  static CONSTEXPR const MatchField NONE = 0x0000000U,  ///< Search in name
      ...
      DEPEND = DEPENDA | DEPENDI, RDEPEND = RDEPENDA | RDEPENDI, PDEPEND = PDEPENDA | PDEPENDI, BDEPEND = BDEPENDA | BDEPENDI, IDEPEND = IDEPENDA | IDEPENDI,
                                    DEPSA = DEPENDA | RDEPENDA | PDEPENDA | BDEPENDA | IDEPENDA, DEPSI = DEPENDI | RDEPENDI | PDEPENDI | BDEPENDI | IDEPENDI, DEPS = DEPSA | DEPSI,
                                    ANY = NAME | DESCRIPTION | LICENSE | CATEGORY | CATEGORY_NAME | HOMEPAGE | IUSE | USE_ENABLED | USE_DISABLED | SRC_URI | EAPI | INST_EAPI | SLOT | FULLSLOT |
                                          INST_SLOT | INST_FULLSLOT | SET | DEPS;

  enum MatchAlgorithm { ALGO_REGEX, ALGO_REGEXCASE, ALGO_EXACT, ALGO_BEGIN, ALGO_END, ALGO_SUBSTRING, ALGO_PATTERN, ALGO_FUZZY, ALGO_ERROR };

  typedef uint8_t TestInstalled;
  static CONSTEXPR const TestInstalled INS_NONE = 0x00U,
  ...
  static CONSTEXPR const TestStability STABLE_NONE = 0x00U,
  ...
  Set default values.
  **/
  ATTRIBUTE_NONNULL_ PackageTest(VarDbPkg* vdb, CorePkgSettings* p, const PrintFormat* f, const SetStability* stability, const DBHeader* dbheader, const ParseError* e);
  ...
  */
  ...
  void Installed() { installed = true; }

  void MultiInstalled() { installed = multi_installed = true; }

  void Slotted() { slotted = true; }

  void MultiSlotted() { slotted = multi_slot = true; }
  ...
  void SetStabilityDefault(TestStability require) { test_stability_default |= require; }

  void SetStabilityLocal(TestStability require) { test_stability_local |= require; }

  void SetStabilityNonlocal(TestStability require) { test_stability_nonlocal |= require; }

  void SetInstability(TestStability avoid) { test_instability |= avoid; }

  void SetMarkedList(MaskList<Mask>* markedlist) { marked_list = markedlist; }

  void Virtual() { have_virtual = true; }

  void Nonvirtual() { have_nonvirtual = true; }

  void Overlay() { overlay = true; }

  void Restrictions(ExtendedVersion::Restrict flags) { restrictions |= flags; }

  void Properties(ExtendedVersion::Properties flags) { properties |= flags; }

  void Binary(ExtendedVersion::CountBinPkg num) { binarynum = num; }

  void WorldAll() { world = true; }

  void WorldFile() { world = world_only_file = true; }

  void SelectedAll() { world = world_only_selected = true; }

  void SelectedFile() { world = world_only_selected = world_only_file = true; }

  void WorldSet() { worldset = true; }

  void SelectedSet() { worldset = worldset_only_selected = true; }

  ATTRIBUTE_NONNULL_ void StabilityDefault(Package* p) const { stability->set_stability(p); }

  ATTRIBUTE_NONNULL_ void StabilityLocal(Package* p) const { stability->set_stability(true, p); }

  ATTRIBUTE_NONNULL_ void StabilityNonlocal(Package* p) const { stability->set_stability(false, p); }
  ...
  MatchField operator|=(const MatchField m) { return field |= m; }
  ...
  **/
  bool overlay, obsolete, upgrade, installed, multi_installed, slotted, multi_slot, world, world_only_file, world_only_selected, worldset, worldset_only_selected, have_virtual, have_nonvirtual,
      know_pattern;
  ...
  std::set<ExtendedVersion::Overlay>*overlay_list, *overlay_only_list, *in_overlay_inst_list;
  ...
  **/
  ...
  TestStability test_stability_default, test_stability_local, test_stability_nonlocal;
