...
bool MatchAtomOperator::match(PackageReader* p) {
  ...
  coreq::print((m_operator == AtomAnd) ? '&' : '|');
  if (m_right == NULLPTR) {
    coreq::print(" NULLPTR ");
  }
  else {
...
bool MatchAtomTest::match(PackageReader* p) {
  ...
  if (m_test == NULLPTR) {
    ...
    coreq::print() % *reinterpret_cast<int*>(m_test);
  }
  coreq::print("] ");
  return false;
#else
  bool is_match((likely(m_pipe == NULLPTR)) || (((*m_pipe) != NULLPTR) && (*m_pipe)->match(p)));
...
MatchAtomTest* MatchTree::parse_new_leaf() {
  ...
  MatchAtom** r(top.useright ? &(top.subroot->as_operator()->m_right) : &(top.subroot));
  if (*r != NULLPTR) {
    return (*r)->as_test();
  }
...
void MatchTree::parse_open() {
  ...
  MatchParseData& top(parser_stack.top());
  parser_stack.push(MatchParseData(top.useright ? &(top.subroot->as_operator()->m_right) : &(top.subroot)));
  if (local_negate) {
    local_negate = false;
...
void MatchTree::parse_close() {
  if (parser_stack.size() <= 1) {
    coreq::say_error(_("warning: ignoring --close without --open"));
    return;
  }
  local_finished = true;
  if (local_negate) {
