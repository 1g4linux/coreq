...
void NowarnMask::init_static() {
  coreq_assert_static(nowarn_keywords == NULLPTR);
...
void NowarnMask::init_nowarn(const WordVec& flagstrings) {
  ...
  set_flags.clear();
  clear_flags.clear();
  for (WordVec::const_iterator it(flagstrings.begin()); likely(it != flagstrings.end()); ++it) {
    if (it->empty()) {
      continue;
    }
    if ((*it)[0] == '-') {
...
void NowarnMaskList::apply(Package* p, Keywords::Redundant* r, PackageTest::TestInstalled* i, CorePkgSettings* corepkgsettings) const {
  ...
  for (super::Get::const_iterator it(masks->begin()); likely(it != masks->end()); ++it) {
    if (!it->have_match(*p)) {
      continue;
    }
    found = true;
  ...
  for (Package::iterator v(p->begin()); likely(v != p->end()); ++v) {
    ...
    for (Version::SetsIndizes::const_iterator sit(v->sets_indizes.begin()); unlikely(sit != v->sets_indizes.end()); ++sit) {
      my_sets.INSERT(*sit);
    }
  }
  if (unlikely(!my_sets.empty())) {
    for (MySets::const_iterator sit(my_sets.begin()); unlikely(sit != my_sets.end()); ++sit) {
      super::Get* setmasks(get_setname(corepkgsettings->set_names[*sit]));
      if (setmasks == NULLPTR) {
        continue;
      }
      for (super::Get::const_iterator m(setmasks->begin()); likely(m != setmasks->end()); ++m) {
        found = true;
