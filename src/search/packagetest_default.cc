...
static void init_match_field_map() {
  coreq_assert_static(static_match_field_map == NULLPTR);
...
PackageTest::MatchField PackageTest::name2field(const string& p, bool default_match_field) {
  ...
  if (unlikely(it == static_match_field_map->end())) {
    if (default_match_field) {
      coreq::say_error(_("unknown match field \"%s\" in DEFAULT_MATCH_FIELD")) % p;
      return NAME;
    }
    coreq::say_error(_("unknown match field \"%s\" in DEFAULT_MATCH_ALGORITHM")) % p;
    return NONE;
  }
  MatchField result(it->second);
  if (unlikely((!default_match_field) && (result == NONE))) {
    coreq::say_error(_("match field \"error\" is invalid in DEFAULT_MATCH_ALGORITHM"));
  }
  return result;
}
...
class MatcherField {
 ...
 public:
  explicit MatcherField(const string& s) {
    ...
    for (WordVec::iterator it(pairs.begin()); likely(it != pairs.end()); ++it) {
      string& regex = *it;
      ++it;
...
static void init_match_algorithm_map() {
  coreq_assert_static(static_match_algorithm_map == NULLPTR);
...
PackageTest::MatchAlgorithm PackageTest::name2algorithm(const string& p) {
  ...
  if (unlikely(it == static_match_algorithm_map->end())) {
    coreq::say_error(_("unknown match algorithm \"%s\" in DEFAULT_MATCH_ALGORITHM")) % p;
    return ALGO_REGEX;
  }
  return it->second;
}
  ...
  for (;;) {
    ...
    switch (spec[i]) {
      ...
      case '!':
        ...
        break;
      default:
        coreq::say_error(_("missing '|', '&', or '!' in \"%s\" in DEFAULT_MATCH_ALGORITHM")) % spec;
        continue;
    }
    ++i;
 ...
 public:
  explicit MatcherAlgorithm(const string& s) {
    ...
    for (WordVec::iterator it(pairs.begin()); likely(it != pairs.end()); ++it) {
      ...
      PackageTest::MatchField or_field(PackageTest::NONE), and_field(PackageTest::NONE), not_field(PackageTest::NONE);
      ...
      if (expression[0] == '(') {
        ...
        if (likely(closing != string::npos)) {
          ...
          PackageTest::parse_field_specification(expression.substr(1, closing - 1), &or_field, &and_field, &not_field);
        }
        else {
          coreq::say_error(_("closing ')' missing in \"%s\" in DEFAULT_MATCH_ALGORITHM")) % expression;
        }
      }
  ...
  PackageTest::MatchAlgorithm parse(const char* p, PackageTest::MatchField field) {
    for (MatchType::size_type i(0); likely(i != match.size()); ++i) {
      ...
      if ((curr != PackageTest::NONE) && ((curr & field) == PackageTest::NONE)) {
        continue;
      }
      curr = and_list[i];
      if ((curr & field) != curr) {
      ...
      curr = not_list[i];
      if ((curr != PackageTest::NONE) && ((field & ~curr) != PackageTest::NONE)) {
        continue;
      }
      if (match[i]->match(p)) {
        return algorithms[i];
...
void PackageTest::setAlgorithm(MatchAlgorithm a) {
  switch (a) {
    ...
    // case ALGO_FUZZY:
    default:
      setAlgorithm(new FuzzyAlgorithm(get_coreqrc().getInteger("LEVENSHTEIN_DISTANCE")));
      break;
  }
}
