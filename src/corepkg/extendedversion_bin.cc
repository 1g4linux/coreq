...
class RestrictMap : public UNORDERED_MAP<string, ExtendedVersion::Restrict> {
 private:
  ATTRIBUTE_NONNULL_ void mapinit(const char* s, ExtendedVersion::Restrict r) { (*this)[s] = r; }
...
class PropertiesMap : public UNORDERED_MAP<string, ExtendedVersion::Properties> {
 private:
  ATTRIBUTE_NONNULL_ void mapinit(const char* s, ExtendedVersion::Properties p) { (*this)[s] = p; }
...
void ExtendedVersion::init_static() {
  coreq_assert_static(restrict_map == NULLPTR);
...
ExtendedVersion::Restrict ExtendedVersion::calcRestrict(const string& str) {
  ...
  for (WordVec::const_iterator it(restrict_words.begin()); likely(it != restrict_words.end()); ++it) {
...
ExtendedVersion::Properties ExtendedVersion::calcProperties(const string& str) {
  coreq_assert_static(properties_map != NULLPTR);
  Properties p(PROPERTIES_NONE);
  WordVec properties_words;
  split_string(&properties_words, str);
  for (WordVec::const_iterator it(properties_words.begin()); it != properties_words.end(); ++it) {
    p |= properties_map->getProperties(*it);
  }
  return p;
}

bool ExtendedVersion::have_bin_pkg(const CorePkgSettings* ps, const Package* pkg) const {
  return (have_tbz_pkg(ps, pkg) || (num_gpkg_pkg(ps, pkg) != 0) || (num_pak_pkg(ps, pkg) != 0));
}

bool ExtendedVersion::have_bin_pkg(const CorePkgSettings* ps, const Package* pkg, CountBinPkg minimal) const {
...
bool ExtendedVersion::have_tbz_pkg(const CorePkgSettings* ps, const Package* pkg) const {
...
ExtendedVersion::CountBinPkg ExtendedVersion::num_gpkg_pkg(const CorePkgSettings* ps, const Package* pkg) const {
...
ExtendedVersion::CountBinPkg ExtendedVersion::num_pak_pkg(const CorePkgSettings* ps, const Package* pkg) const {
  count_multiversions(ps, pkg);
  return count_pak_m;
}

void ExtendedVersion::count_multiversions(const CorePkgSettings* ps, const Package* pkg) const {
  ...
  for (WordVec::const_iterator it(bin_packages.begin()); it != bin_packages.end(); ++it) {
