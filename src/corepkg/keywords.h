...
#ifndef SRC_COREPKG_KEYWORDS_H_
#define SRC_COREPKG_KEYWORDS_H_ 1
...
class MaskFlags {
 public:
  typedef coreq::UChar MaskType;
  static CONSTEXPR const MaskType MASK_NONE = 0x00U, MASK_PACKAGE = 0x01U, MASK_PROFILE = 0x02U, MASK_HARD = MASK_PACKAGE | MASK_PROFILE, MASK_SYSTEM = 0x04U, MASK_WORLD = 0x08U,
                                  MASK_WORLD_SETS = 0x10U, MASK_IN_PROFILE = 0x20U, MASK_MARKED = 0x40U;

  MaskFlags() : m_mask(MASK_NONE) {}

  explicit MaskFlags(MaskType t) : m_mask(t) {}

  void set(MaskType t) { m_mask = t; }

  MaskType get() const { return m_mask; }

  MaskType getall(MaskType t) const { return ((m_mask & t) != MaskFlags::MASK_NONE); }

  bool empty() const { return m_mask == MaskFlags::MASK_NONE; }

  bool havesome(MaskType t) const { return ((m_mask & t) != MaskFlags::MASK_NONE); }

  bool haveall(MaskType t) const { return ((m_mask & t) == t); }

  void setbits(MaskType t) { m_mask |= t; }

  void clearbits(MaskType t) { m_mask &= ~t; }

  bool isHardMasked() const { return havesome(MaskFlags::MASK_HARD); }

  /**
  @return true if version is masked by profile
  **/
  bool isProfileMask() const { return havesome(MaskFlags::MASK_PROFILE); }

  /**
  @return true if version is masked by a package.mask
  **/
  bool isPackageMask() const { return havesome(MaskFlags::MASK_PACKAGE); }

  /**
  @return true if version is part of a package that is a system-package
  **/
  bool isSystem() const { return havesome(MaskFlags::MASK_SYSTEM); }

  /**
  @return true if version is part of a package that is a system-package
  **/
  bool isProfile() const { return havesome(MaskFlags::MASK_IN_PROFILE); }

  /**
  @return true if version is part of world
  **/
  bool isWorld() const { return havesome(MaskFlags::MASK_WORLD); }

  /**
  @return true if version is part of world sets
  **/
  bool isWorldSets() const { return havesome(MaskFlags::MASK_WORLD_SETS); }

  /**
  @return true if version is marked
  **/
  bool isMarked() const { return havesome(MaskFlags::MASK_MARKED); }
...
class KeywordsFlags {
 public:
  typedef coreq::UChar KeyType;
  static CONSTEXPR const KeyType KEY_EMPTY = 0x00U,
      ...
      KEY_SOMESTABLE = KEY_ARCHSTABLE | KEY_ALIENSTABLE, KEY_SOMEUNSTABLE = KEY_ARCHUNSTABLE | KEY_ALIENUNSTABLE, KEY_TILDESTARMATCH = KEY_SOMESTABLE | KEY_SOMEUNSTABLE;
  ...
  void set_keyflags(KeyType t) { m_keyword = t; }

  KeyType get() const { return m_keyword; }

  KeyType getall(KeyType t) const { return (m_keyword & t); }

  bool empty() const { return m_keyword == KEY_EMPTY; }

  bool havesome(KeyType t) const { return (m_keyword & t); }

  bool haveall(KeyType t) const { return ((m_keyword & t) == t); }

  void setbits(KeyType t) { m_keyword |= t; }

  void clearbits(KeyType t) { m_keyword &= ~t; }

  /**
  @return true if version is marked stable
  **/
  bool isStable() const { return havesome(KEY_STABLE); }

  /**
  @return true if version is arch stable
  **/
  bool isArchStable() const { return havesome(KEY_ARCHSTABLE); }

  /**
  @return true if version is unstable
  **/
  bool isUnstable() const { return havesome(KEY_ARCHUNSTABLE); }

  /**
  @return true if version is masked by -* keyword
  **/
  bool isMinusAsterisk() const { return havesome(KEY_MINUSASTERISK); }

  /**
  @return true if version is masked by -~* keyword
  **/
  bool isMinusUnstable() const { return havesome(KEY_MINUSUNSTABLE); }

  /**
  @return true if version is masked by -keyword
  **/
  bool isMinusKeyword() const { return havesome(KEY_MINUSKEYWORD); }

  /**
  @return true if version is masked by ALIENARCH
  **/
  bool isAlienStable() const { return havesome(KEY_ALIENSTABLE); }

  /**
  @return true if version is masked by ~ALIENARCH
  **/
  bool isAlienUnstable() const { return havesome(KEY_ALIENUNSTABLE); }

  /**
  @return true if version is masked (only) by missing keyword
  **/
  bool isMissingKeyword() const { return ((m_keyword == KEY_EMPTY) || (m_keyword == KEY_ARCHSTABLE)); }
...
class Keywords {
 public:
  ...
  static CONSTEXPR const Redundant RED_NOTHING = 0x000000U,  ///< None of the following
      ...
      RED_ALL_MASK = RED_MASK | RED_DOUBLE_MASK | RED_IN_MASK, RED_ALL_UNMASK = RED_UNMASK | RED_DOUBLE_UNMASK | RED_IN_UNMASK, RED_ALL_MASKSTUFF = RED_ALL_MASK | RED_ALL_UNMASK,
  ...
  Keywords() : keyflags(KeywordsFlags::KEY_EMPTY), maskflags(MaskFlags::MASK_NONE), redundant(RED_NOTHING), red_mask(0x00U) {}

  Keywords(KeywordsFlags::KeyType k, MaskFlags::MaskType m) : keyflags(k), maskflags(m), redundant(RED_NOTHING), red_mask(0x00U) {}
  ...
  **/
  ...
  void set_redundant(Redundant or_redundant) { redundant |= or_redundant; }

  Redundant get_redundant() const { return redundant; }

  void set_was_masked() { red_mask |= 0x01U; }

  bool was_masked() const { return (red_mask & 0x01U); }

  void set_was_unmasked() { red_mask |= 0x02U; }

  bool was_unmasked() const { return (red_mask & 0x02U); }

  void set_wanted_masked() { red_mask |= 0x04U; }

  bool wanted_masked() const { return (red_mask & 0x04U); }

  void set_wanted_unmasked() { red_mask |= 0x08U; }

  bool wanted_unmasked() const { return (red_mask & 0x08U); }
...
class KeywordSave {
 ...
 public:
  ...
  explicit KeywordSave(const Keywords* k) { store(k); }
