...
template <>
ATTRIBUTE_NONNULL_ bool MaskList<Mask>::add_file(const char* file, Mask::Type mask_type, bool recursive, bool keep_commentlines, const ParseError* parse_error) {
  LineVec lines;
...
template <>
ATTRIBUTE_NONNULL_ bool MaskList<Mask>::MaskMatches(Package* p) const {
  Get* masks(get(p));
  if (likely(masks == NULLPTR)) {
    return false;
  }
  bool ret(false);
  for (Get::const_iterator it(masks->begin()); likely(it != masks->end()); ++it) {
    if (it->ismatch(*p)) {
      ret = true;
      break;
    }
...
template <>
ATTRIBUTE_NONNULL_ bool MaskList<Mask>::applyMasks(Package* p, Keywords::Redundant check) const {
  Get* masks(get(p));
  if (masks == NULLPTR) {
    return false;
  }
...
template <>
ATTRIBUTE_NONNULL_ void MaskList<Mask>::applySetMasks(Version* v, const string& set_name) const {
...
bool PreList::handle_lines(const LineVec& lines, FilenameIndex file, const bool only_add, LineNumber* num, bool keep_commentlines) {
  ...
  for (LineVec::const_iterator it(lines.begin()); likely(it != lines.end()); ++it) {
...
void PreList::initialize(MaskList<Mask>* l, Mask::Type t, bool keep_commentlines, const ParseError* parse_error) {
  ...
  for (const_iterator it(begin()); likely(it != end()); ++it) {
    ...
    BasicVersion::ParseResult r(m.parseMask(name, &errtext, 1, repo_if_only(it->filename_index)));
    if (unlikely(r != BasicVersion::parsedOK)) {
...
void PreList::initialize(MaskList<KeywordMask>* l, string raised_arch, const ParseError* parse_error) {
  ...
  for (const_iterator it(begin()); likely(it != end()); ++it) {
    KeywordMask m;
    string errtext;
    BasicVersion::ParseResult r(m.parseMask(it->name.c_str(), &errtext, 1, repo_if_only(it->filename_index)));
    if (unlikely(r != BasicVersion::parsedOK)) {
      parse_error->output(file_name(it->filename_index), it->linenumber, it->name + " ...", errtext);
    }
    if (likely(r != BasicVersion::parsedError)) {
