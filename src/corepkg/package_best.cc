...
Version* VersionList::best(bool allow_unstable) const {
  for (const_reverse_iterator ri(rbegin()); likely(ri != rend()); ++ri) {
    ...
    if ((*ri)->keyflags.isStable() || (allow_unstable && (*ri)->keyflags.isUnstable())) {
      return *ri;
    }
  }
  return NULLPTR;
...
bool Package::calc_allow_upgrade_slots(const CorePkgSettings* ps) const {
...
void Package::best_slots(Package::VerVec* l, bool allow_unstable) const {
  l->clear();
  for (SlotList::const_iterator sit(slotlist().begin()); likely(sit != slotlist().end()); ++sit) {
...
void Package::best_slots_upgrade(Package::VerVec* versions, VarDbPkg* v, const CorePkgSettings* ps, bool allow_unstable) const {
  versions->clear();
  ...
  for (VerSet::iterator it(versionset.begin()); likely(it != versionset.end()); ++it) {
...
bool Package::is_best_upgrade(bool check_slots, const Version* version, VarDbPkg* v, const CorePkgSettings* ps, bool allow_unstable) const {
  if (unlikely(v == NULLPTR)) {
    return false;
  }
  InstVec* ins(v->getInstalledVector(*this));
...
bool Package::guess_slotname(InstVersion* v, const VarDbPkg* vardbpkg, const char* force) const {
  ...
  if (slotlist().size() == 1) {
    ...
    // (e.g. in coreq-diff) and so there might be new slots elsewhere
...
-  0: else
**/
coreq::TinySigned Package::worse_best_slots(const Package& p) const {
  coreq::TinySigned ret(0);
  for (SlotList::const_iterator it(slotlist().begin()); it != slotlist().end(); ++it) {
...
**/
coreq::TinySigned Package::compare_best_slots(const Package& p) const {
  coreq::TinySigned worse(worse_best_slots(p));
  coreq::TinySigned better(p.worse_best_slots(*this));
  if (worse == 1) {
    if (better == 1) {
      return 2;
    }
...
-  3: same, but overlays (or slots if test_slot)
      are different
**/
coreq::TinySigned Package::compare_best(const Package& p, bool test_slot) const {
  Version* t_best(best());
...
**/
coreq::TinySigned Package::check_best_slots(VarDbPkg* v, bool only_installed) const {
  InstVec* ins(NULLPTR);
  if (likely(v != NULLPTR)) {
...
-  4: (if only_installed) nothing is installed,
      but one can be installed
**/
coreq::TinySigned Package::check_best(VarDbPkg* v, bool only_installed, bool test_slot) const {
  ...
  if ((ins != NULLPTR) && !ins->empty()) {
    ...
    for (InstVec::iterator it(ins->begin()); likely(it != ins->end()); ++it) {
      coreq::SignedBool vgl(BasicVersion::compare(*t_best, *it));
      if (vgl > 0) {
        continue;
      }
      if (vgl < 0) {
