...
class BasicPart {
 public:
  enum PartType { garbage, alpha, beta, pre, rc, revision, inter_rev, patch, character, primary, first };
...
class BasicVersion {
 ...
 public:
  enum ParseResult { parsedOK, parsedError, parsedGarbage };
  ...
  **/
  ...
  BasicVersion::ParseResult parseVersion(const std::string& str, std::string* errtext) { return parseVersion(str, errtext, 1); }
  ...
  **/
  ...
  Compare the version
  **/
  static coreq::SignedBool compare(const BasicVersion& left, const BasicVersion& right) { return BasicVersion::compare(left, right, false); }

  /**
  Compare the version, but return equality if right is shorter
  **/
  static coreq::SignedBool compare_right_maybe_shorter(const BasicVersion& left, const BasicVersion& right) { return BasicVersion::compare(left, right, true); }
