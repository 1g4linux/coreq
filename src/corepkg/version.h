...
#ifndef SRC_COREPKG_VERSION_H_
#define SRC_COREPKG_VERSION_H_ 1
...
class IUse : public std::string {
 public:
  typedef coreq::UChar Flags;
  static CONSTEXPR const Flags USEFLAGS_NIL = 0, USEFLAGS_NORMAL = 1, USEFLAGS_PLUS = 2, USEFLAGS_MINUS = 4;
  Flags flags;

  ATTRIBUTE_NONNULL_ static Flags parse(std::string* s);

  std::string& name() { return *static_cast<std::string*>(this); }

  const std::string& name() const { return *static_cast<const std::string*>(this); }

  explicit IUse(const std::string& s) : std::string(s) { flags = parse(&name()); }
  ...
  bool operator==(const IUse& c) const { return (name() == c.name()); }
  bool operator!=(const IUse& c) const { return (name() != c.name()); }
  bool operator<(const IUse& c) const { return (name() < c.name()); }
  bool operator>(const IUse& c) const { return (name() > c.name()); }
  bool operator<=(const IUse& c) const { return (name() <= c.name()); }
...
class IUseNatural {
 public:
  IUseNatural(const IUse* use) { m_iuse = use; }

  const IUse& iuse() const { return *m_iuse; }

  const std::string& name() const { return m_iuse->name(); }

  std::string asString() const { return m_iuse->asString(); }

  bool operator==(const IUseNatural& c) const { return (name() == c.name()); }
  bool operator!=(const IUseNatural& c) const { return (name() != c.name()); }
  bool operator<(const IUseNatural& c) const { return (natcmp(name(), c.name()) < 0); }
  bool operator>(const IUseNatural& c) const { return (natcmp(name(), c.name()) > 0); }
  bool operator<=(const IUseNatural& c) const { return (natcmp(name(), c.name()) <= 0); }
  bool operator>=(const IUseNatural& c) const { return (natcmp(name(), c.name()) >= 0); }
...
class IUseSet {
 public:
  ...
  bool empty() const { return m_iuse.empty(); }

  void clear() { m_iuse.clear(); }

  const IUseStd& asStd() const { return m_iuse; }

  IUseNaturalOrder asNaturalOrder() const;

  void insert(const IUseStd& iuse);

  void insert(const IUseSet& iuse) { insert(iuse.asStd()); }

  void insert(const std::string& iuse);

  void insert_fast(const std::string& iuse) { insert(IUse(iuse)); }
...
class Version FINAL : public ExtendedVersion, public Keywords {
 public:
  ...
  typedef enum { SAVEKEY_USER, SAVEKEY_ACCEPT, SAVEKEY_ARCH, SAVEKEY_SIZE } SavedKeyIndex;

  typedef enum { SAVEMASK_USER, SAVEMASK_USERFILE, SAVEMASK_USERPROFILE, SAVEMASK_PROFILE, SAVEMASK_FILE, SAVEMASK_SIZE } SavedMaskIndex;

  typedef enum { SAVEEFFECTIVE_USERPROFILE, SAVEEFFECTIVE_PROFILE, SAVEEFFECTIVE_SIZE } SavedEffectiveIndex;

  typedef coreq::UChar EffectiveState;
  static CONSTEXPR const EffectiveState EFFECTIVE_UNSAVED = 0, EFFECTIVE_USED = 1, EFFECTIVE_UNUSED = 2;

  coreq::array<KeywordsFlags, SAVEKEY_SIZE> saved_keywords;
  coreq::array<bool, SAVEKEY_SIZE> have_saved_keywords;
  coreq::array<MaskFlags, SAVEMASK_SIZE> saved_masks;
  coreq::array<bool, SAVEMASK_SIZE> have_saved_masks;
  coreq::array<std::string, SAVEEFFECTIVE_SIZE> saved_effective;
  coreq::array<std::string, SAVEEFFECTIVE_SIZE> saved_accepted;
  coreq::array<EffectiveState, EFFECTIVE_UNUSED> states_effective;
  ...
  bool is_in_set(SetsIndex m_set) const { return (std::find(sets_indizes.begin(), sets_indizes.end(), m_set) != sets_indizes.end()); }
  ...
  void set_full_keywords(const std::string& keywords) { full_keywords = keywords; }

  std::string get_full_keywords() const { return full_keywords; }
  ...
  **/
  ...
  const std::string get_effective_keywords() const { return ((effective_state == EFFECTIVE_USED) ? effective_keywords : full_keywords); }
  ...
  void set_keyflags(const WordSet& accepted_keywords) { keyflags.set_keyflags(get_keyflags(accepted_keywords)); }
  ...
  bool have_reasons() const { return !reasons.empty(); }

  const Reasons* reasons_ptr() const { return &reasons; }
