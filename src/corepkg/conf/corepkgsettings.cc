...
ATTRIBUTE_NONNULL_ static ArchUsed
apply_keyword(const string& key, const WordUnorderedSet& keywords_set, KeywordsFlags kf, const WordSet* arch_set, Keywords::Redundant* redundant, Keywords::Redundant check, bool shortcut);
ATTRIBUTE_NONNULL_ inline static void increase(char* s);

const string& CorePkgSettings::operator[](const string& var) const {
...
const char* CorePkgSettings::cstr(const string& var) const {
  const_iterator it(find(var));
  if (it == end()) {
    return NULLPTR;
  }
...
bool CorePkgSettings::grab_setmasks(const char* file, SetsIndex i, WordVec* contains_set, bool recursive) {
  LineVec lines;
...
**/
static CONSTEXPR const char* default_accumulating_keys[] = {"USE", "USE_EXPAND*", "CONFIG_*", "FEATURES", "ACCEPT_KEYWORDS", NULLPTR};

/**
Environment variables which should take effect before reading profiles
**/
static CONSTEXPR const char* test_in_env_early[] = {"COREPKG_PROFILE", "COREPKG_REPOSITORIES", "PORTDIR", "PORTDIR_OVERLAY", NULLPTR};

/**
Environment variables which should add/override all other settings
**/
static CONSTEXPR const char* test_in_env_late[] = {"USE", "USE_EXPAND", "USE_EXPAND_HIDDEN", "CONFIG_PROTECT", "CONFIG_PROTECT_MASK", "FEATURES", "ARCH", "ACCEPT_KEYWORDS", NULLPTR};

void CorePkgSettings::override_by_map(const char* const* vars, const WordIterateMap& varmap) {
...
void CorePkgSettings::override_by_env(const char* const* vars) {
  for (const char* var(*vars); likely(var != NULLPTR); var = *(++vars)) {
    override_by_value(var, std::getenv(var));
  }
}

void CorePkgSettings::override_by_value(const char* var, const string& value) {
  string& ref((*this)[var]);
...
void CorePkgSettings::override_by_value(const char* var, const char* value) {
  if (value == NULLPTR) {
    return;
  }
  string& ref((*this)[var]);
...
void CorePkgSettings::read_config(const string& name, const string& prefix, WordIterateMap* vars) {
  (*this)["EPREFIX"] = m_eprefix;
  VarsReader configfile(VarsReader::SUBST_VARS | VarsReader::INTO_MAP | VarsReader::APPEND_VALUES | VarsReader::ALLOW_SOURCE | VarsReader::COREPKG_ESCAPES | VarsReader::RECURSE);
...
void CorePkgSettings::read_make_conf(const string& eprefixsource, WordIterateMap* vars) {
...
void CorePkgSettings::read_make_conf_early(const string& eprefixsource) {
  WordIterateMap make_conf(*this);
  read_make_conf(eprefixsource, &make_conf);
  override_by_map(test_in_env_early, make_conf);
}

void CorePkgSettings::read_make_conf_late(const string& eprefixsource) {
...
void CorePkgSettings::read_make_globals(const string& eprefixsource) {
...
void CorePkgSettings::read_repos_conf(const string& eprefixsource) {
  bool have_repos;
  VarsReader reposfile(VarsReader::SUBST_VARS | VarsReader::COREPKG_SECTIONS | VarsReader::RECURSE);
  reposfile.setPrefix(eprefixsource);
  string& repositories = (*this)["COREPKG_REPOSITORIES"];
  if (!repositories.empty()) {
    ...
    string errtext;
    have_repos = reposfile.read((*settings_rc)["COREPKG_REPOS_CONF"].c_str(), &errtext, true);
    if (!have_repos) {
      coreq::say_error(_("error: %s")) % errtext;
    }
  ...
  if (likely(have_repos)) {
    ...
    for (VarsReader::const_iterator it(reposfile.begin()); likely(it != reposfile.end()); ++it) {
...
string CorePkgSettings::resolve_overlay_name(const string& path, bool resolve) {
  if (resolve) {
...
void CorePkgSettings::add_repo(const string& path, bool resolve, const char* label, OverlayIdent::Priority priority, bool is_main) {
  OverlayIdent o(resolve_overlay_name(path, resolve).c_str(), label, priority, is_main);
  repos.push_back(o, true);
}

void CorePkgSettings::add_repo_vector(const WordVec& v, bool resolve) {
...
**/
void CorePkgSettings::init(CoreqRc* coreqrc, const ParseError* e, bool getlocal, bool init_world, bool print_profile_paths) {
  settings_rc = coreqrc;
  parse_error = e;
#ifndef HAVE_SETENV
  ...
  m_recurse_sets = coreqrc->getBool("RECURSIVE_SETS");
  m_eprefixconf = coreqrc->m_eprefixconf;
  m_eprefix = (*coreqrc)["EPREFIX"];
  m_eprefixprofile = (*coreqrc)["EPREFIX_COREPKG_PROFILE"];
  m_eprefixportdir = (*coreqrc)["EPREFIX_PORTDIR"];
  m_eprefixoverlays = (*coreqrc)["EPREFIX_OVERLAYS"];
  m_eprefixaccessoverlays = (*coreqrc)["EPREFIX_ACCESS_OVERLAYS"];
  (*this)["ARCH"] = (*coreqrc)["DEFAULT_ARCH"];

  const string& eprefixsource((*coreqrc)["EPREFIX_SOURCE"]);
  ...
  if (unlikely(print_profile_paths)) {
    ...
    s = (*coreqrc)["PRINT_APPEND"];
    unescape_string(&s);
  }
  store_world_sets(NULLPTR);
  bool read_world(false);
  if (init_world) {
    if (coreqrc->getBool("SAVE_WORLD"))
      read_world = true;
  }
  else {
    if (coreqrc->getBool("CURRENT_WORLD"))
      read_world = true;
  }
  if (read_world) {
    if (profile->m_world.add_file((*coreqrc)["COREQ_WORLD"].c_str(), Mask::maskInWorld, false, parse_error)) {
      profile->use_world = true;
    }
    profile->m_world.finalize();
    read_world_sets((*coreqrc)["COREQ_WORLD_SETS"].c_str());
  }
  ...
  coreq::SignedBool as_arch(coreqrc->getBoolText("ACCEPT_KEYWORDS_AS_ARCH", "full"));
  if (as_arch != 0) {
    m_plain_accepted_keywords_set.clear();
    for (WordSet::const_iterator it(m_accepted_keywords_set.begin()); unlikely(it != m_accepted_keywords_set.end()); ++it) {
  ...
  }
  {
    ...
    WordSet archset;
    for (WordSet::const_iterator it(m_arch_set.begin()); unlikely(it != m_arch_set.end()); ++it) {
      if (std::strchr("-~", (*it)[0]) == NULLPTR) {
  ...
  if (getlocal) {
    ...
    user_config = new CorePkgUserConfig(this, local_profile);
  }

  WordVec sets_dirs;
  split_string(&sets_dirs, (*coreqrc)["COREQ_LOCAL_SETS"], true);
  // Expand relative pathnames and the magic "*" of COREQ_LOCAL_SETS:
...
CorePkgSettings::~CorePkgSettings() {
  delete profile;
  delete user_config;
}

void CorePkgSettings::read_world_sets(const char* file) {
  LineVec lines;
  if (!pushback_lines(file, &lines, false, false)) {
    return;
  }
  WordVec the_sets;
  for (WordVec::const_iterator it(lines.begin()); likely(it != lines.end()); ++it) {
    string s;
    if ((*it)[0] == '@') {
...
void CorePkgSettings::store_world_sets(const WordVec* s_world_sets, bool override) {
  if (!s_world_sets) {
    // set defaults:
  ...
  world_setslist_up_to_date = false;
  world_sets.clear();
  for (WordVec::const_iterator it(s_world_sets->begin()); likely(it != s_world_sets->end()); ++it) {
    if (it->empty()) {
      continue;
    }
...
void CorePkgSettings::add_name(SetsList* l, const string& s, bool recurse) const {
  if (s == "system") {
    l->add_system();
    return;
  }
...
void CorePkgSettings::update_world_setslist() {
  world_setslist_up_to_date = true;
  world_setslist.clear();
  for (WordVec::const_iterator it(world_sets.begin()); likely(it != world_sets.end()); ++it) {
    add_name(&world_setslist, *it, m_recurse_sets);
  }
}

void CorePkgSettings::calc_world_sets(Package* p) {
  ...
  for (Package::iterator it(p->begin()); likely(it != p->end()); ++it) {
    ...
    bool world(false);
    for (Version::SetsIndizes::const_iterator sit(it->sets_indizes.begin()); sit != it->sets_indizes.end(); ++sit) {
...
void CorePkgSettings::get_setnames(WordSet* names, const Package* p, bool also_nonlocal) const {
  names->clear();
  for (Package::const_iterator it(p->begin()); likely(it != p->end()); ++it) {
    for (Version::SetsIndizes::const_iterator sit(it->sets_indizes.begin()); likely(sit != it->sets_indizes.end()); ++sit) {
...
string CorePkgSettings::get_setnames(const Package* p, bool also_nonlocal) const {
  WordSet names;
  get_setnames(&names, p, also_nonlocal);
  return join_to_string(names);
}

void CorePkgSettings::read_local_sets(const WordVec& dir_list) {
  world_setslist_up_to_date = false;
  set_names.clear();
  ...
  **/
  ...
  for (WordVec::size_type i(0); likely(i != dir_list.size()); ++i) {
    ...
    for (WordVec::const_iterator it(temporary_set_names.begin()); likely(it != temporary_set_names.end()); ++it) {
  ...
  for (SetsIndex i(0); likely(i != set_names.size()); ++i) {
    for (WordVec::const_iterator it(child_names[i].begin()); likely(it != child_names[i].end()); ++it) {
  ...
  for (bool startnew(true); likely(startnew);) {
    startnew = false;
    for (SetsIndex i(0); likely(i != set_names.size()); ++i) {
      for (SetsList::const_iterator it(children_sets[i].begin()); it != children_sets[i].end(); ++it) {
...
void CorePkgSettings::calc_recursive_sets(Package* p) const {
  for (Package::iterator vi(p->begin()); likely(vi != p->end()); ++vi) {
    SetsList will_add;
    for (Version::SetsIndizes::const_iterator it(vi->sets_indizes.begin()); it != vi->sets_indizes.end(); ++it)
      will_add.add(parent_sets[*it]);
    for (SetsList::const_iterator it(will_add.begin()); it != will_add.end(); ++it)
      vi->add_to_set(*it);
  }
}

bool CorePkgSettings::calc_allow_upgrade_slots(const Package* p) const {
  if (unlikely(!know_upgrade_policy)) {
    upgrade_policy = settings_rc->getBool("UPGRADE_TO_HIGHEST_SLOT");
    upgrade_policy_exceptions.clear();
    WordVec exceptions;
    split_string(&exceptions, ((*settings_rc)[upgrade_policy ? "SLOT_UPGRADE_FORBID" : "SLOT_UPGRADE_ALLOW"]), true);
    for (WordVec::const_iterator it(exceptions.begin()); likely(it != exceptions.end()); ++it) {
...
**/
WordVec::size_type CorePkgSettings::verify_categories(const string& name, WordVec* vec, WordVec::size_type index) const {
  LineVec::size_type line(0);
  while (index < vec->size()) {
    ++line;
    string& s = (*vec)[index];
    // Remove trailing and leading /, see https://github.com/vaeth/coreq/issues/8
...
**/
void CorePkgSettings::pushback_categories(WordVec* vec) {
  WordVec::size_type index(vec->size());
  /**
  Merge categories from /etc/corepkg/categories and
...
void CorePkgSettings::addOverlayProfiles(CascadingProfile* p) const {
...
CorePkgUserConfig::CorePkgUserConfig(CorePkgSettings* psettings, CascadingProfile* local_profile) {
...
CorePkgUserConfig::~CorePkgUserConfig() {
  delete profile;
}

bool CorePkgUserConfig::readMasks() {
...
bool CorePkgUserConfig::readKeywords() {
  PreList pre_list;
  bool added(false);
  LineVec lines;
...
void CorePkgUserConfig::ReadVersionFile(const char* file, MaskList<KeywordMask>* list) const {
  LineVec lines;
  pushback_lines(file, &lines, true);
  for (LineVec::iterator i(lines.begin()); likely(i != lines.end()); ++i) {
    if (i->empty())
      continue;
    KeywordMask m;
...
**/
bool CorePkgUserConfig::CheckList(Package* p, const MaskList<KeywordMask>* list, Keywords::Redundant flag_double, Keywords::Redundant flag_in) const {
  bool rvalue(false);
  UNORDERED_MAP<Version*, char> counter;
  MaskList<KeywordMask>::Get* keyword_masks(list->get(p));
  if (keyword_masks != NULLPTR) {
    for (MaskList<KeywordMask>::Get::const_iterator it(keyword_masks->begin()); likely(it != keyword_masks->end()); ++it) {
      rvalue = true;
      Mask::Matches matches;
      it->match(&matches, p);
      for (Mask::Matches::iterator v(matches.begin()); likely(v != matches.end()); ++v) {
  ...
  for (Package::iterator v(p->begin()); likely(v != p->end()); ++v) {
    ...
    for (Version::SetsIndizes::const_iterator it(v->sets_indizes.begin()); unlikely(it != v->sets_indizes.end()); ++it) {
      ...
      rvalue = true;
      char& c(counter[*v]);
      for (MaskList<KeywordMask>::Get::const_iterator m(key_masks->begin()); likely(m != key_masks->end()); ++m) {
        if (!m->keywords.empty()) {
          increase(&c);
        }
      }
...
bool CorePkgUserConfig::CheckFile(Package* p, const char* file, MaskList<KeywordMask>* list, bool* readfile, Keywords::Redundant flag_double, Keywords::Redundant flag_in) const {
  if (!(*readfile)) {
...
static CONSTEXPR const ArchUsed ARCH_NOTHING = 0, ARCH_STABLE = 1, ARCH_UNSTABLE = 2, ARCH_ALIENSTABLE = 3, ARCH_ALIENUNSTABLE = 4, ARCH_EVERYTHING = 5,
...
apply_keyword(const string& key, const WordUnorderedSet& keywords_set, KeywordsFlags kf, const WordSet* arch_set, Keywords::Redundant* redundant, Keywords::Redundant check, bool shortcut) {
  ...
  if ((keywords_set.count(key) == 0) && ((key[0] == '*') || (keywords_set.count("*") == 0)) && ((key[0] != '~') || (keywords_set.count("~*") == 0))) {
    // Not found:
    if (key == "**")
...
/**
@return true if something from /etc/corepkg/package.* applied and check involves keywords
**/
bool CorePkgUserConfig::setKeyflags(Package* p, Keywords::Redundant check) const {
      ...
      for (Mask::Matches::iterator v(matches.begin()); likely(v != matches.end()); ++v) {
        ...
        if (it->locally_double) {
          if (check & Keywords::RED_DOUBLE_LINE)
            v->set_redundant((v->get_redundant()) | Keywords::RED_DOUBLE_LINE);
        }
      }
    }
    delete keyword_masks;
  ...
  for (Package::iterator it(p->begin()); likely(it != p->end()); ++it) {
    ...
    // Were keywords added from /etc/corepkg/package.accept_keywords?
    ...
    if (calc_lkw) {
      if (!kvfile.empty()) {
        ...
        // Add items from /etc/corepkg/package.accept_keywords to kv
        // but remove matching -... items from kv_set_nofile
        for (WordVec::const_iterator fit(kvfile.begin()); fit != kvfile.end(); ++fit) {
          if ((*fit)[0] == '-') {
      ...
      else if ((check & (Keywords::RED_ALL_KEYWORDS & ~(Keywords::RED_DOUBLE_LINE | Keywords::RED_IN_KEYWORDS))) == Keywords::RED_NOTHING) {
        calc_lkw = false;
      }
    }
    ...
    if (calc_lkw) {
      ...
      if (kv_set_nofile == NULLPTR) {
        // The case that nothing was added from /etc/corepkg/package.accept_keywords?
        for (WordSet::const_iterator sit(kv_set.begin()); sit != kv_set.end(); ++sit) {
          if (apply_keyword(*sit, keywords_set, kf, m_settings->m_local_arch_set, &redundant, Keywords::RED_NOTHING, true) != ARCH_NOTHING) {
            stable = true;
          }
        }
        kv_set.clear();
        ...
        // The case that we have some data from /etc/corepkg/package.accept_keywords in kv_set:
        // In this case, we use kv_set_nofile instead of kv_set for demasking.
        // Moreover, we must remove the demasked data from kv_set...
        for (WordSet::const_iterator sit(kv_set_nofile->begin()); sit != kv_set_nofile->end(); ++sit) {
          if (apply_keyword(*sit, keywords_set, kf, m_settings->m_local_arch_set, &redundant, Keywords::RED_NOTHING, true) != ARCH_NOTHING) {
            stable = true;
          }
      ...
      // Now apply the remaining keywords (i.e. from /etc/corepkg/package.accept_keywords)
      ArchUsed arch_used(ARCH_NOTHING);
      for (WordSet::iterator kvi(kv_set.begin()); likely(kvi != kv_set.end()); ++kvi) {
        ArchUsed arch_curr(apply_keyword(*kvi, keywords_set, kf, m_settings->m_local_arch_set, &redundant, check, shortcut));
...
void CorePkgUserConfig::pushback_set_accepted_keywords(WordVec* result, const Version* v) const {
  for (Version::SetsIndizes::const_iterator it(v->sets_indizes.begin()); unlikely(it != v->sets_indizes.end()); ++it) {
    const MaskList<KeywordMask>::Get* keyword_masks(m_accept_keywords.get_setname(m_settings->set_names[*it]));
    if (keyword_masks == NULLPTR) {
      continue;
    }
    for (MaskList<KeywordMask>::Get::const_iterator i(keyword_masks->begin()); i != keyword_masks->end(); ++i) {
...
**/
void CorePkgSettings::setKeyflags(Package* p, bool use_accepted_keywords) const {
...
void CorePkgSettings::get_effective_keywords_profile(Package* p) const {
...
void CorePkgSettings::get_effective_keywords_userprofile(Package* p) const {
...
void CorePkgUserConfig::setProfileMasks(Package* p) const {
...
/**
@return true if something from /etc/corepkg/package.* applied and check involves masks
**/
bool CorePkgUserConfig::setMasks(Package* p, Keywords::Redundant check, bool file_mask_is_profile) const {
  Version::SavedMaskIndex ind(file_mask_is_profile ? Version::SAVEMASK_USERFILE : Version::SAVEMASK_USER);
  if ((check & Keywords::RED_ALL_KEYWORDS) == Keywords::RED_NOTHING) {
  ...
  if (file_mask_is_profile) {
    if (unlikely(!(p->restore_maskflags(Version::SAVEMASK_FILE)))) {
      coreq::say_error(_("internal error: Tried to restore nonlocal mask without saving"));
...
void CorePkgSettings::setMasks(Package* p, bool filemask_is_profile) const {
...
void CorePkgSettings::calc_local_sets(Package* p) const {
...
bool CorePkgSettings::use_expand(string* var, string* expvar, const string& value) const {
  string::size_type s(value.size());
  for (string::size_type pos(0); ((pos = value.find('_', pos)) != string::npos) && (pos != 0) && (pos + 1 < s); ++pos) {
    if (!know_expands) {
      know_expands = true;
      WordSet use_expands;
      resolve_plus_minus(&use_expands, (*this)["USE_EXPAND"]);
      for (WordSet::const_iterator it(use_expands.begin()); it != use_expands.end(); ++it) {
