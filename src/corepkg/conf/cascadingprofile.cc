...
#define PROFILE_LINK2 "/etc/corepkg/make.profile"
...
bool CascadingProfile::addProfile(const char* profile, WordUnorderedSet* sourced_files) {
  ...
  if (unlikely(print_profile_paths)) {
    if (likely(is_dir(truename.c_str()))) {
      coreq::print() % truename;
      if (profile_paths_append.empty()) {
        coreq::print('\0');
      }
      else {
        coreq::print() % profile_paths_append;
      }
    }
  }
  if (truename.empty()) {
  ...
  if (unlikely(topcall)) {
    ...
    coreq::say_error(_("Recursion in cascading profile"));
    return false;
  }
  ...
  if (pushback_lines(currfile.c_str(), &parents)) {
    for (WordVec::const_iterator it(parents.begin()); likely(it != parents.end()); ++it) {
      if (it->empty()) {
        continue;
      }
      if ((*it)[0] == '/') {
      ...
      const char* path;
      RepoList repos(m_corepkgsettings->repos);
      if (colon == 0) {
      ...
      if (path == NULLPTR) {
        coreq::say_error(_("warning: ignoring parent %s of file %s")) % (*it) % currfile;
        continue;
      }
  ...
  for (WordVec::const_iterator it(filenames.begin()); likely(it != filenames.end()); ++it) {
...
bool CascadingProfile::readremoveFiles() {
  coreq_assert_static(profile_filenames != NULLPTR);
  bool ret(false);
  for (ProfileFiles::iterator file(m_profile_files.begin()); likely(file != m_profile_files.end()); ++file) {
    ...
    OverlayIdent& overlay(m_corepkgsettings->repos[file->repo_num()]);
    overlay.readLabel();
    if ((this->*handler)(file->name(), (overlay.label.empty() ? NULLPTR : overlay.label.c_str()), file->only_repo())) {
      ret = true;
    }
  }
...
bool CascadingProfile::readPackages(const string& filename, const char* repo, bool only_repo) {
  ...
  for (LineVec::const_iterator it(lines.begin()); likely(it != lines.end()); ++number, ++it) {
    if (it->empty()) {
      continue;
    }
...
void CascadingProfile::readMakeDefaults() {
  for (WordVec::size_type i(0); likely(i < m_profile_files.size()); ++i) {
    if (unlikely(std::strcmp(std::strrchr(m_profile_files[i].c_str(), '/'), "/make.defaults") == 0)) {
      m_corepkgsettings->read_config(m_profile_files[i].name(), "");
    }
  }
}
...
void CascadingProfile::finalize() {
  ...
  finalized = true;
  const ParseError* parse_error(m_corepkgsettings->parse_error);
  ...
  p_package_accept_keywords.initialize(&m_package_accept_keywords, m_corepkgsettings->m_raised_arch, parse_error);
}
...
void CascadingProfile::listaddProfile(const char* profile_dir) {
  ...
  const string& s((*m_corepkgsettings)["COREPKG_PROFILE"]);
  if (unlikely(!s.empty())) {
    if (addProfile(s.c_str())) {
      return;
    }
  }
  if (addProfile(((m_corepkgsettings->m_eprefixconf) + PROFILE_LINK1).c_str())) {
    return;
  }
  addProfile(((m_corepkgsettings->m_eprefixconf) + PROFILE_LINK2).c_str());
}
...
void CascadingProfile::applyMasks(Package* p) const {
  ...
  for (Package::iterator v(p->begin()); likely(v != p->end()); ++v) {
    ...
    for (Version::SetsIndizes::const_iterator it(v->sets_indizes.begin()); unlikely(it != v->sets_indizes.end()); ++it) {
      const string& set_name(m_corepkgsettings->set_names[*it]);
...
void CascadingProfile::applyKeywords(Package* p) const {
  ...
  for (Package::iterator v(p->begin()); likely(v != p->end()); ++v) {
    ...
    for (Version::SetsIndizes::const_iterator it(v->sets_indizes.begin()); unlikely(it != v->sets_indizes.end()); ++it) {
      const char* set_name(m_corepkgsettings->set_names[*it].c_str());
