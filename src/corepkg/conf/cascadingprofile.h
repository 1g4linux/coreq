...
#ifndef SRC_COREPKG_CONF_CASCADINGPROFILE_H_
#define SRC_COREPKG_CONF_CASCADINGPROFILE_H_ 1
...
class ProfileFile {
 ...
 public:
  ProfileFile(const std::string& s, OverlayVec::size_type repo_num, bool only_repo) : filename(s), reponum(repo_num), honour_repo(only_repo) {}

  const std::string& name() const { return filename; }

  const char* c_str() const { return filename.c_str(); }

  OverlayVec::size_type repo_num() const { return reponum; }

  bool only_repo() const { return honour_repo; }
};
...
class CascadingProfile {
 ...
 protected:
  ...
  CorePkgSettings* m_corepkgsettings;  ///< Profilesettings to which this instance "belongs"
  ...
  PreList p_system, p_profile, p_package_masks, p_package_unmasks, p_package_keywords, p_package_accept_keywords;
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool addProfile(const char* profile) { return addProfile(profile, NULLPTR); }
 ...
 public:
  ATTRIBUTE_NONNULL_ CascadingProfile(CorePkgSettings* corepkgsettings, bool init_world)
      : print_profile_paths(false), use_world(false), finalized(false), m_init_world(init_world), m_corepkgsettings(corepkgsettings) {}
  ...
  **/
  void listaddFile(const std::string& file, OverlayVec::size_type i, bool only_repo) { m_profile_files.EMPLACE_BACK(ProfileFile, (file, i, only_repo)); }

  /**
  Clear m_profile_files
  **/
  void listclear() { m_profile_files.clear(); }
