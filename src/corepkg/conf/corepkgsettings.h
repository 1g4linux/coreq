...
#ifndef SRC_COREPKG_CONF_COREPKGSETTINGS_H_
#define SRC_COREPKG_CONF_COREPKGSETTINGS_H_ 1
...
#define MAKE_CONF_FILE_NEW "/etc/corepkg/make.conf"
#define USER_CATEGORIES_FILE "/etc/corepkg/categories"
#define USER_KEYWORDS_FILE1 "/etc/corepkg/package.keywords"
#define USER_KEYWORDS_FILE2 "/etc/corepkg/package.accept_keywords"
#define USER_MASK_FILE "/etc/corepkg/package.mask"
#define USER_UNMASK_FILE "/etc/corepkg/package.unmask"
#define USER_USE_FILE "/etc/corepkg/package.use"
#define USER_ENV_FILE "/etc/corepkg/package.env"
#define USER_LICENSE_FILE "/etc/corepkg/package.license"
#define USER_RESTRICT_FILE "/etc/corepkg/package.accept_restrict"
#define USER_CFLAGS_FILE "/etc/corepkg/package.cflags"
#define USER_REPOS_CONF "/etc/corepkg/repos.conf"
#define USER_PROFILE_DIR "/etc/corepkg/profile"
...
class CorePkgUserConfig {
  friend class CorePkgSettings;

 private:
  CorePkgSettings* m_settings;
 ...
 public:
  ATTRIBUTE_NONNULL((2)) CorePkgUserConfig(CorePkgSettings* psettings, CascadingProfile* local_profile);
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool setMasks(Package* p, Keywords::Redundant check) const { return setMasks(p, check, false); }
  ATTRIBUTE_NONNULL_ bool setMasks(Package* p) const { return setMasks(p, Keywords::RED_NOTHING); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool setKeyflags(Package* p) const { return setKeyflags(p, Keywords::RED_NOTHING); }

  /**
  @return true if something from /etc/corepkg/package.use applied
  **/
  ...
  /**
  @return true if something from /etc/corepkg/package.env applied
  **/
  ...
  /**
  @return true if something from /etc/corepkg/package.license applied
  **/
  ...
  /**
  @return true if something from /etc/corepkg/package.accept_restrict applied
  **/
  ...
  /**
  @return true if something from /etc/corepkg/package.cflags applied
  **/
...
/**
Holds CorePkg's settings, e.g. masks, categories, overlay paths.
Reads needed files if content is requested .. so don't worry about initialization :)
**/
class CorePkgSettings : public WordIterateMap {
 private:
  friend class CascadingProfile;
  friend class CorePkgUserConfig;
  ...
  **/
  ...
  WordSet m_accepted_keywords_set, m_arch_set, m_plain_accepted_keywords_set, *m_local_arch_set, *m_auto_arch_set;
  ...
  **/
  ...
  void read_config(const std::string& name, const std::string& prefix) { read_config(name, prefix, this); }
  ...
  const MaskList<SetMask>* getPackageSets() const { return &m_package_sets; }
  ...
  **/
  ATTRIBUTE_NONNULL_ void init(CoreqRc* coreqrc, const ParseError* e, bool getlocal, bool init_world, bool print_profile_paths);

  ATTRIBUTE_NONNULL_ CorePkgSettings(CoreqRc* coreqrc, const ParseError* e, bool getlocal, bool init_world, bool print_profile_paths) { init(coreqrc, e, getlocal, init_world, print_profile_paths); }

  ATTRIBUTE_NONNULL_ CorePkgSettings(CoreqRc* coreqrc, const ParseError* e, bool getlocal, bool init_world) { init(coreqrc, e, getlocal, init_world, false); }
  ...
  **/
  ...
  void add_repo(const std::string& path, bool resolve) { add_repo(path, resolve, NULLPTR, 0, false); }

  void add_repo_vector(const WordVec& v, bool resolve);

  void store_world_sets(const WordVec* s_world_sets, bool override);
  void store_world_sets(const WordVec* s_world_sets) { store_world_sets(s_world_sets, false); }

  ATTRIBUTE_NONNULL_ void get_setnames(WordSet* names, const Package* p, bool also_nonlocal) const;
  ATTRIBUTE_NONNULL_ void get_setnames(WordSet* names, const Package* p) const { get_setnames(names, p, false); }

  std::string get_setnames(const Package* p, bool also_nonlocal) const;
  std::string get_setnames(const Package* p) const { return get_setnames(p, false); }

  void read_local_sets(const WordVec& dir_list);

  const WordVec* get_world_sets() const { return &world_sets; }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ void setMasks(Package* p) const { setMasks(p, false); }
