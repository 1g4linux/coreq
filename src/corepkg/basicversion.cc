...
coreq::SignedBool BasicPart::compare(const BasicPart& left, const BasicPart& right) {
  /*
  ...
  */
  coreq::SignedBool ret(coreq::default_compare(left.parttype, right.parttype));
  if (ret != 0) {
    return ret;
  }
  ...
  */
  if (left.partcontent.size() == right.partcontent.size()) {
    return coreq::toSignedBool(left.partcontent.compare(right.partcontent));
  }
  if (left.parttype == BasicPart::primary) {
    ...
    */
    ...
    if (((left.partcontent)[0] == '0') || ((right.partcontent)[0] == '0')) {
      ...
      */
      ...
      return coreq::toSignedBool(l1.compare(l2));
    }
    /*
    ...
    */
    ...
    return coreq::toSignedBool(left.partcontent.compare(left.partcontent));
  }
  ...
  comparison."
  */
  return coreq::numeric_compare(left.partcontent, right.partcontent);
}
...
static ostream& operator<<(ostream& s, const BasicPart& part) {
  ...
  coreq::say_error(_("internal error: unknown PartType on (\"%s\",\"%s\")")) % static_cast<int>(part.parttype) % part.partcontent;
  return s;
}
...
string BasicVersion::getPlain() const {
  stringstream ss;
  for (PartsType::const_iterator it(m_parts.begin()); likely(it != m_parts.end()); ++it) {
...
BasicVersion::ParseResult BasicVersion::parseVersion(const string& str, string* errtext, coreq::SignedBool accept_garbage) {
  m_parts.clear();
  ...
  if (unlikely((endpos == pos) || (pos == str.size()))) {
    ...
    if (errtext != NULLPTR) {
      *errtext = coreq::format(_("malformed (first primary at position %s) version string \"%s\"")) % pos % str;
    }
    return parsedError;
  }
  ...
  while (str[pos] == '.') {
    ...
    if (unlikely((endpos == pos) || (pos == str.size()))) {
      m_parts.EMPLACE_BACK(BasicPart, (BasicPart::garbage, str, pos));
      if (errtext != NULLPTR) {
        *errtext = coreq::format(_("malformed (primary at position %s) version string \"%s\"")) % pos % str;
      }
      return parsedError;
    }
  ...
  while (str[pos] == '_') {
    ...
    if (str.compare(pos, 5, "alpha") == 0) {
      ...
      if (errtext != NULLPTR) {
        *errtext = coreq::format(_("malformed (suffix at position %s) version string \"%s\"")) % pos % str;
      }
      return parsedError;
    }
  ...
  if (str.compare(pos, 2, "-r") == 0) {
    ...
    if (str[pos] == '.') {
      // inter-revision used by prefixed corepkg.
  ...
  if (errtext != NULLPTR) {
    *errtext = coreq::format(accept_garbage ? _("garbage (%s) at end of version \"%s\"\n"
...
coreq::SignedBool BasicVersion::compare(const BasicVersion& left, const BasicVersion& right, bool right_maybe_shorter) {
  for (PartsType::const_iterator it_left(left.m_parts.begin()), it_right(right.m_parts.begin());; ++it_left) {
    ...
    coreq::SignedBool ret(BasicPart::compare(*it_left, *it_right));
    if (ret != 0) {
...
coreq::SignedBool BasicVersion::compareTilde(const BasicVersion& left, const BasicVersion& right) {
  for (PartsType::const_iterator it_left(left.m_parts.begin()), it_right(right.m_parts.begin());; ++it_left, ++it_right) {
    bool right_end((it_right == right.m_parts.end()) || (it_right->parttype == BasicPart::revision));
    if ((it_left == left.m_parts.end()) || (it_left->parttype == BasicPart::revision)) {
