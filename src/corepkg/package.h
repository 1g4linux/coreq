...
#ifndef SRC_COREPKG_PACKAGE_H_
#define SRC_COREPKG_PACKAGE_H_ 1
...
class VersionList : public std::vector<Version*> {  // null entries are not allowed
 public:
  ...
  ATTRIBUTE_PURE Version* best() const { return best(false); }
};
...
class SlotVersions {
 ...
 public:
  ...
  const VersionList& const_version_list() const { return m_version_list; }

  VersionList& version_list() { return m_version_list; }

  ATTRIBUTE_NONNULL_ SlotVersions(const char* s, Version* v) : m_slotname(s), m_version_list(v) {}
};
...
/**
A class to represent a package in corepkg It contains various information
about a package, including a sorted(!) list of versions.
**/
class Package : public coreq::ptr_container<std::list<Version*> > {
 public:
  ...
  **/
  ...
  static CONSTEXPR const Duplicates DUP_NONE = 0x00U,
  ...
  **/
  ...
  static CONSTEXPR const Versioncollects COLLECT_NONE = 0x00U, COLLECT_HAVE_NONTRIVIAL_SLOTS = 0x01U, COLLECT_HAVE_SAME_OVERLAY_KEY = 0x02U, COLLECT_AT_LEAST_TWO_OVERLAYS = 0x04U,
                                         COLLECT_HAVE_MAIN_REPO_KEY = 0x08U, COLLECT_DEFAULT = COLLECT_HAVE_SAME_OVERLAY_KEY;
  Versioncollects version_collects;

  MaskFlags local_collects;

  coreq::array<MaskFlags, Version::SAVEMASK_SIZE> saved_collects;
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool calc_allow_upgrade_slots(const CorePkgSettings* ps) const;
  ...
  **/
  bool have_nontrivial_slots() const { return (version_collects & COLLECT_HAVE_NONTRIVIAL_SLOTS); }

  /**
  @return true if all versions come from one overlay.
  **/
  bool have_same_overlay_key() const { return (version_collects & COLLECT_HAVE_SAME_OVERLAY_KEY); }

  /**
  @return true if any version comes from main repository.
  **/
  bool have_main_repo_key() const { return (version_collects & COLLECT_HAVE_MAIN_REPO_KEY); }

  /**
  @return true if all versions come from at least two overlays.
  **/
  bool at_least_two_overlays() const { return (version_collects & COLLECT_AT_LEAST_TWO_OVERLAYS); }

  /**
  @return true if some version is in @system.
  **/
  bool is_system_package() const { return local_collects.isSystem(); }

  /**
  @return true if some version is in @profile.
  **/
  bool is_profile_package() const { return local_collects.isProfile(); }

  /**
  @return true if some version is in the world file.
  **/
  bool is_world_package() const { return local_collects.isWorld(); }

  /**
  @return true if some version is in the world sets.
  **/
  bool is_world_sets_package() const { return local_collects.isWorldSets(); }

  /**
  Preset with defaults
  **/
  Package() {  // NOLINT(whitespace/braces)
    defaults();
  }
  ...
  **/
  Package(const std::string& c, const std::string& n) : category(c), name(n) { defaults(); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ Version* best_slot(const char* slot_name) const { return best_slot(slot_name, false); }

  ATTRIBUTE_NONNULL_ void best_slots(Package::VerVec* l, bool allow_unstable) const;
  ATTRIBUTE_NONNULL_ void best_slots(Package::VerVec* l) const { return best_slots(l, false); }

  /**
  Calculate list of uninstalled upgrade candidates
  **/
  ATTRIBUTE_NONNULL((2, 4)) void best_slots_upgrade(Package::VerVec* versions, VarDbPkg* v, const CorePkgSettings* ps, bool allow_unstable) const;

  /**
  @return true if version is an (installed or uninstalled) upgrade candidate
  **/
  ATTRIBUTE_NONNULL((3, 5)) bool is_best_upgrade(bool check_slots, const Version* version, VarDbPkg* v, const CorePkgSettings* ps, bool allow_unstable) const;
  ...
  -  0: else
  **/
  coreq::TinySigned worse_best_slots(const Package& p) const;
  ...
  **/
  coreq::TinySigned compare_best_slots(const Package& p) const;
  ...
  -  3: same, but overlays (or slots if test_slot)
        are different
  **/
  ATTRIBUTE_PURE coreq::TinySigned compare_best(const Package& p, bool test_slot) const;
  ...
  **/
  coreq::TinySigned check_best_slots(VarDbPkg* v, bool only_installed) const;
  ...
  -  4: (if only_installed) nothing is installed,
        but one can be installed
  **/
  coreq::TinySigned check_best(VarDbPkg* v, bool only_installed, bool test_slot) const;

  /**
  @return true if we can upgrade v or has v different slots
  **/
  ATTRIBUTE_NONNULL((3)) bool can_upgrade(VarDbPkg* v, const CorePkgSettings* ps, bool only_installed, bool test_slots) const {
    if (!test_slots) {
  ...
  **/
  bool must_downgrade(VarDbPkg* v, bool test_slots) const {
    coreq::TinySigned c(check_best(v, true, test_slots));
    if ((c < 0) || (c == 3)) {
      return true;
    }
    if (!test_slots) {
  ...
  **/
  ATTRIBUTE_NONNULL((3)) bool recommend(VarDbPkg* v, const CorePkgSettings* ps, bool only_installed, bool test_slots) const {
    return can_upgrade(v, ps, only_installed, test_slots) || must_downgrade(v, test_slots);
  }

  bool differ(const Package& p, VarDbPkg* v, const CorePkgSettings* ps, bool only_installed, bool testvardb, bool test_slots) const {
    if (testvardb) {
  ...
  **/
  ...
  ATTRIBUTE_NONNULL((2)) bool guess_slotname(InstVersion* v, const VarDbPkg* vardbpkg) const { return guess_slotname(v, vardbpkg, NULLPTR); }

  Version* latest() const { return *rbegin(); }
...
class PackageSave {
 ...
 public:
  explicit PackageSave(const Package* p) { store(p); }
...
class PackagePtr {
 ...
 public:
  ...
  operator Package*() const { return ptr; }

  Package& operator*() const { return *ptr; }
