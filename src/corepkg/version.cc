...
const IUse::Flags IUse::USEFLAGS_NIL, IUse::USEFLAGS_NORMAL, IUse::USEFLAGS_PLUS, IUse::USEFLAGS_MINUS;
...
IUseSet::IUseNaturalOrder IUseSet::asNaturalOrder() const {
  ...
  for (IUseStd::const_iterator it(m_iuse.begin()); likely(it != m_iuse.end()); ++it) {
    ret.insert(&(*it));
  }
  return ret;
}
...
string IUseSet::asString() const {
  ...
  string ret;
  for (IUseNaturalOrder::const_iterator it(iuse.begin()); likely(it != iuse.end()); ++it) {
    if (!ret.empty())
      ret.append(1, ' ');
...
WordVec IUseSet::asVector() const {
  ...
  for (IUseNaturalOrder::const_iterator it(iuse.begin()); likely(it != iuse.end()); ++i, ++it) {
    ret[i] = it->asString();
  }
  return ret;
}
...
void IUseSet::insert(const IUseStd& iuse) {
  for (IUseStd::const_iterator it(iuse.begin()); likely(it != iuse.end()); ++it) {
    insert(*it);
  }
}
...
void IUseSet::insert(const string& iuse) {
  ...
  for (WordVec::const_iterator it(vec.begin()); likely(it != vec.end()); ++it) {
    insert_fast(*it);
  }
}
...
const Version::EffectiveState Version::EFFECTIVE_UNSAVED, Version::EFFECTIVE_USED, Version::EFFECTIVE_UNUSED;

bool Version::use_required_use;

Version::Version() : effective_state(EFFECTIVE_UNUSED) {
