...
#ifndef SRC_COREPKG_OVERLAY_H_
#define SRC_COREPKG_OVERLAY_H_ 1
...
class OverlayIdent {
 ...
 public:
  ...
  OverlayIdent(const char* patharg, const char* labelarg, Priority prio, bool ismain) : priority(prio), is_main(ismain) { init(patharg, labelarg); }

  OverlayIdent(const char* patharg, const char* labelarg) : priority(0), is_main(false) { init(patharg, labelarg); }

  explicit OverlayIdent(const char* patharg) : priority(0), is_main(false) { init(patharg, NULLPTR); }
  ...
  void readLabel() { readLabel(NULLPTR); }
  ...
  ATTRIBUTE_PURE static coreq::SignedBool compare(const OverlayIdent& left, const OverlayIdent& right);
...
class RepoList : public OverlayVec {
 ...
 public:
  ...
  ATTRIBUTE_NONNULL_ RepoList::iterator find_filename(const char* search, bool parent_ok) { return find_filename(search, parent_ok, true); }

  ATTRIBUTE_NONNULL_ RepoList::iterator find_filename(const char* search) { return find_filename(search, false); }
  ...
  void push_back(const OverlayIdent& s) { push_back(s, false); }
