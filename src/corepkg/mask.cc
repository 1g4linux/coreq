...
// best_match_to_list() from corepkg's corepkg/dep/__init__.py
...
// which deviates from that of corepkg which orders extended+repo lower:
// */* */bar */*::foo */bar::foo cat/pkg cat/pkg::foo
static const struct OperatorTable {
  const char* str;
  coreq::TinyUnsigned len;
  Mask::Operator op;
  coreq::TinyUnsigned priority;
} operators[] = {{"<=", 2, Mask::maskOpLessEqual, 16},
...
**/
BasicVersion::ParseResult Mask::parseMask(const char* str, string* errtext, coreq::SignedBool accept_garbage, const char* default_repo) {
  ...
  if (unlikely(m_type == maskPseudomask)) {
    ...
    for (const OperatorTable* curr = operators;; ++curr) {
      coreq::TinyUnsigned len(curr->len);
      if (unlikely(len == 0)) {
        // no operator
        m_operator = curr->op;
  ...
  if ((m_operator != maskOpAll) || (m_type == maskMarkOptional)) {
    ...
    if (unlikely((p == NULLPTR) || ((end != NULLPTR) && (p >= end)))) {
      if (unlikely(m_operator != maskOpAll)) {  // maskMarkOptional without explicit "="
        *errtext = ((m_type != maskPseudomask) ? _("operator without a version part") : _("version specification is missing"));
        return parsedError;
      }
    }
    else {
  ...
  if (have_version) {
    ...
    if (unlikely((unlikely(wildcard != NULLPTR)) && (likely((end == NULLPTR) || (wildcard <= end))))) {
      if (unlikely(m_operator == maskOpAll)) {  // maskMarkOptional without explicit "="
        m_operator = maskOpEqual;
        priority += 24;
      }
      ...
      if (unlikely(((wildcard + 1 != end) || (end == NULLPTR)) && ((end != NULLPTR) || (wildcard[1] != '\0')))) {
...
bool Mask::ismatch(const Package& pkg) const {
  if ((fnmatch(m_name.c_str(), pkg.name.c_str(), 0) != 0) || (fnmatch(m_category.c_str(), pkg.category.c_str(), 0) != 0))
    return false;
