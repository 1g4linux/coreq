...
const Package::Duplicates Package::DUP_NONE, Package::DUP_SOME, Package::DUP_OVERLAYS;

const Package::Versioncollects Package::COLLECT_NONE, Package::COLLECT_HAVE_NONTRIVIAL_SLOTS, Package::COLLECT_HAVE_SAME_OVERLAY_KEY, Package::COLLECT_AT_LEAST_TWO_OVERLAYS,
    Package::COLLECT_HAVE_MAIN_REPO_KEY, Package::COLLECT_DEFAULT;

void Package::defaults() {
  know_upgrade_slots = m_has_cached_slotlist = m_has_cached_subslots = false;
...
void Package::addVersionStart(Version* version) {
  Duplicates check_duplicates((have_duplicate_versions == DUP_OVERLAYS) ? DUP_NONE : ((version->overlay_key != 0) ? DUP_OVERLAYS : ((have_duplicate_versions == DUP_SOME) ? DUP_NONE : DUP_SOME)));
  ...
  for (iterator i(begin()); likely(i != end()); ++i) {
    ...
    if ((check_duplicates == DUP_NONE) || likely(BasicVersion::compare(**i, *version) != 0)) {
      continue;
    }
