...
#ifndef SRC_COREPKG_MASK_LIST_H_
#define SRC_COREPKG_MASK_LIST_H_ 1
...
#include "coreqTk/forward_list.h"
...
class Masks : protected coreq::forward_list<m_Type> {
 public:
  typedef typename coreq::forward_list<m_Type> MasksList;
...
class MaskList {
 ...
 public:
  typedef typename coreq::ptr_container<std::vector<const m_Type*> > Get;

  bool empty() const { return (exact_name.empty() && full_name.empty()); }
  ...
  inline static bool match_full(const std::string& mask, const std::string& name) { return !fnmatch(mask.c_str(), name.c_str(), FNM_PATHNAME); }

  bool match_full(const std::string& full) const {
    if (exact_name.count(full) != 0) {
      return true;
    }
    for (full_const_iterator it(full_name.begin()); likely(it != full_name.end()); ++it) {
  ...
  ATTRIBUTE_NONNULL_ bool match_name(const Package* p) const { return match_full(p->category + "/" + p->name); }
  ...
  Get* get_setname(const std::string& setname) const { return get_full(std::string(SET_CATEGORY) + "/" + setname); }

  ATTRIBUTE_NONNULL_ Get* get(const Package* p) const { return get_full(p->category + "/" + p->name); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool add_file(const char* file, Mask::Type mask_type, bool recursive, const ParseError* parse_error) { return add_file(file, mask_type, recursive, false, parse_error); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ void applyListItems(Package* p) const {
    ...
    for (typename Get::const_iterator it(masks->begin()); likely(it != masks->end()); ++it) {
      it->applyItem(p);
    }
    delete masks;
  }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ bool applyMasks(Package* p) const { return applyMasks(p, Keywords::RED_NOTHING); }
...
class PreListOrderEntry : public LineVec {
 public:
  ...
  PreListOrderEntry(const super& line, FilenameIndex file, LineNumber number) : super(line), filename_index(file), linenumber(number), removed(false), locally_double(false) {}
};
...
This corresponds to corepkg's sorting.
**/
class PreList : public std::vector<PreListEntry> {
 ...
 public:
  ...
  const std::string& file_name(FilenameIndex file) const { return filenames[file].name(); }

  const char* repo(FilenameIndex file) const { return filenames[file].repo(); }

  const char* repo_if_only(FilenameIndex file) const { return filenames[file].repo_if_only(); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ void initialize(MaskList<Mask>* l, Mask::Type t, const ParseError* parse_error) { initialize(l, t, false, parse_error); }
...
#endif  // SRC_COREPKG_MASK_LIST_H_
