...
#ifndef SRC_COREPKG_MASK_H_
#define SRC_COREPKG_MASK_H_ 1
...
class Mask : public BasicVersion {
  ...
  **/
  typedef enum { maskTypeNone, maskInProfile, maskInSystem, maskInWorld, maskMask, maskUnmask, maskPseudomask, maskMark, maskMarkOptional } Type;

  /**
  Describes the comparison operator before the mask.
  **/
  typedef enum { maskOpAll, maskOpEqual, maskOpLess, maskOpLessEqual, maskOpGreaterEqual, maskOpGreater, maskOpRevisions, maskOpGlob, maskOpGlobExt, maskIsSet } Operator;

  coreq::TinyUnsigned priority;
  StringList comments;
 ...
 public:
  typedef coreq::ptr_container<std::vector<Version*> > Matches;
  ...
  **/
  ...
  explicit Mask(Type type) : m_type(type), m_test_slot(false), m_test_subslot(false), m_test_reponame(false) {}
  ...
  **/
  ATTRIBUTE_NONNULL((2, 3)) BasicVersion::ParseResult parseMask(const char* str, std::string* errtext, coreq::SignedBool accept_garbage, const char* default_repo);
  ATTRIBUTE_NONNULL_ BasicVersion::ParseResult parseMask(const char* str, std::string* errtext, coreq::SignedBool accept_garbage) { return parseMask(str, errtext, accept_garbage, NULLPTR); }
  ATTRIBUTE_NONNULL_ BasicVersion::ParseResult parseMask(const char* str, std::string* errtext) { return parseMask(str, errtext, 1, NULLPTR); }
  ...
  **/
  ...
  ATTRIBUTE_NONNULL_ void checkMask(Package* pkg) const { checkMask(pkg, Keywords::RED_NOTHING); }

  bool ismatch(const Package& pkg) const;

  bool is_set() const { return (m_operator == maskIsSet); }
};
...
class KeywordMask FINAL : public Mask {
 public:
  ...
  KeywordMask() : Mask(maskTypeNone), locally_double(false) {}

  ATTRIBUTE_NONNULL_ void applyItem(Package* pkg) const;

  ATTRIBUTE_NONNULL_ void applyItem(Version* ver) const { ver->add_accepted_keywords(keywords); }
};
...
class PKeywordMask FINAL : public Mask {
 public:
  ...
  ATTRIBUTE_NONNULL_ void applyItem(Version* ver) const { ver->modify_effective_keywords(keywords); }
};
