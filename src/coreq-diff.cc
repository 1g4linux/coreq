...
ATTRIBUTE_NONNULL_ static void init_db(const char* file, Database* db, DBHeader* header, PackageReader** reader, CorePkgSettings* ps);
...
ATTRIBUTE_NONNULL_ static void parseFormat(Node** format, const char* varname, CoreqRc* rc);

static CorePkgSettings* corepkgsettings;
...
static void print_help() {
  coreq::say(
      _("Usage: %s [options] old-cache [new-cache]\n"
        "\n"
...
bool cli_show_help(false), cli_show_version(false), cli_dump_coreqrc(false), cli_dump_defaults(false), cli_known_vars(false), cli_ansi(false), cli_quick, cli_care, cli_deps_installed, cli_quiet;

const char* var_to_print(NULLPTR);

enum diff_options { O_DUMP = 300, O_DUMP_DEFAULTS, O_KNOWN_VARS, O_PRINT_VAR, O_CARE, O_DEPS_INSTALLED, O_ANSI, O_FORCE_COLOR };

/**
Arguments and options
**/
class CoreqDiffOptionList : public OptionList {
 public:
  CoreqDiffOptionList();
};

CoreqDiffOptionList::CoreqDiffOptionList() {
  ...
  push_back(Option("dump", O_DUMP, Option::BOOLEAN_T, &cli_dump_coreqrc));
...
static void init_db(const char* file, Database* db, DBHeader* header, PackageReader** reader, CorePkgSettings* ps) {
  if (likely(db->openread(file))) {
    ...
    coreq::say_error(_("error in database file %s: %s")) % file % errtext;
  }
  else {
    coreq::say_error(_("cannot open database file %s for reading (mode = 'rb')")) % file;
  }
...
class DiffReaders {
 public:
  ...
  ATTRIBUTE_NONNULL_ DiffReaders(VarDbPkg* vardbpkg, CorePkgSettings* corepkg_settings, bool only_installed, bool compare_slots, bool separate_deleted)
      : m_vardbpkg(vardbpkg), m_corepkg_settings(corepkg_settings), m_only_installed(only_installed), m_slots(compare_slots), m_separate_deleted(separate_deleted) {}
  ...
  **/
  int diff() {
    ...
    if (m_separate_deleted) {
      for (vector<Package*>::iterator it(lost_list.begin()); likely(it != lost_list.end()); ++it) {
        lost_package(*it);
        delete (*it);
      }
      lost_list.clear();
    ...
    if (m_separate_deleted) {
      for (vector<Package*>::iterator it(found_list.begin()); likely(it != found_list.end()); ++it) {
        found_package(*it);
        delete (*it);
      }
      found_list.clear();
 ...
 private:
  VarDbPkg* m_vardbpkg;
  CorePkgSettings* m_corepkg_settings;
  ...
  bool best_differs() { return new_pkg->differ(*old_pkg, m_vardbpkg, m_corepkg_settings, true, m_only_installed, m_slots); }
...
static void print_changed_package(Package* op, Package* np) {
  Package* p[2] = {op, np};
  format_for_new->print(p, get_diff_package_property, format_changed, new_header, varpkg_db, corepkgsettings, set_stability_new);
}

static void print_found_package(Package* p) {
  format_for_new->print(p, format_new, new_header, varpkg_db, corepkgsettings, set_stability_new);
}

static void print_lost_package(Package* p) {
  format_for_old->print(p, format_delete, old_header, varpkg_db, corepkgsettings, set_stability_old);
}

static void parseFormat(Node** format, const char* varname, CoreqRc* rc) {
  string errtext;
  if (likely((format_for_new->parseFormat(format, (*rc)[varname].c_str(), &errtext)))) {
    return;
  }
  coreq::say_error(_("problems while parsing %s: %s")) % varname % errtext;
  std::exit(EXIT_FAILURE);
}
...
int run_coreq_diff(int argc, char* argv[]) {
  ...
  CoreqRc& rc(get_coreqrc(DIFF_VARS_PREFIX));
  {
    string errtext;
  ...
  format_for_new->no_color = (rc.getBool("NOCOLORS") ? true : (rc.getBool("FORCE_COLORS") ? false : (isatty(1) == 0)));

  /* Setup ArgumentReader. */
  ArgumentReader argreader(argc, argv, CoreqDiffOptionList());
  ...
  if (unlikely(cli_dump_coreqrc || cli_dump_defaults)) {
  ...
  if (unlikely((current_param != argreader.end()) && (current_param->type == Parameter::ARGUMENT))) {
    ...
    old_file = rc["COREQ_PREVIOUS"];
  }
  if (!have_new) {
    new_file = rc["COREQ_CACHEFILE"];
  }
  ...
  corepkgsettings = new CorePkgSettings(&rc, &parse_error, true, false);

  varpkg_db = new VarDbPkg(rc["EPREFIX_INSTALLED"] + VAR_DB_PKG, !cli_quick, cli_care, cli_deps_installed, rc.getBool("RESTRICT_INSTALLED"), rc.getBool("CARE_RESTRICT_INSTALLED"),
                           rc.getBool("USE_BUILD_TIME"));
  varpkg_db->check_installed_overlays = rc.getBoolText("CHECK_INSTALLED_OVERLAYS", "repository");

  bool local_settings(rc.getBool("LOCAL_COREPKG_CONFIG"));
  bool always_accept_keywords(rc.getBool("ALWAYS_ACCEPT_KEYWORDS"));
  set_stability_old = new SetStability(corepkgsettings, local_settings, true, always_accept_keywords);
  set_stability_new = new SetStability(corepkgsettings, local_settings, false, always_accept_keywords);
  format_for_new->recommend_mode = rc.getLocalMode("RECOMMEND_LOCAL_MODE");

  Database new_db;
  new_header = new DBHeader;
  init_db(new_file.c_str(), &new_db, new_header, &new_reader, corepkgsettings);

  Database old_db;
  old_header = new DBHeader;
  init_db(old_file.c_str(), &old_db, old_header, &old_reader, corepkgsettings);
  ...
  DiffReaders differ(varpkg_db, corepkgsettings, rc.getBool("DIFF_ONLY_INSTALLED"), !rc.getBool("DIFF_NO_SLOTS"), rc.getBool("DIFF_SEPARATE_DELETED"));
  ...
  int ret(differ.diff());
  coreq::print() % format_for_new->color_end;

  delete varpkg_db;
  delete corepkgsettings;
  return ret;
}
