...
static void print_help() {
	coreq::say(_("Usage: %s [options] old-cache [new-cache]\n"
"\n"
...
class DiffReaders {
	public:
		...
		ATTRIBUTE_NONNULL_ DiffReaders(VarDbPkg *vardbpkg, CorePkgSettings *corepkg_settings, bool only_installed, bool compare_slots, bool separate_deleted) :
			m_vardbpkg(vardbpkg), m_corepkg_settings(corepkg_settings), m_only_installed(only_installed),
	...
	private:
		...
		bool best_differs() {
			return new_pkg->differ(*old_pkg, m_vardbpkg, m_corepkg_settings, true, m_only_installed, m_slots);
		}
...
static void parseFormat(Node **format, const char *varname, CoreqRc *rc) {
	...
	coreq::say_error(_("problems while parsing %s: %s"))
			% varname % errtext;
	std::exit(EXIT_FAILURE);
}
...
int run_coreq_diff(int argc, char *argv[]) {
	...
	DiffReaders differ(varpkg_db, corepkgsettings,
