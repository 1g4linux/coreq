...
class Pathname {
	...
	public:
		...
		ATTRIBUTE_NONNULL_ string resolve(CorePkgSettings *corepkg_settings) {
			return corepkg_settings->resolve_overlay_name(name, must_resolve);
		}
...
ATTRIBUTE_NONNULL_ static bool update(const char *outputfile, CacheTable *cache_table, CorePkgSettings *corepkg_settings, bool override_umask, const RepoNames& repo_names, const WordVec& exclude_labels, Statusline *statusline, string *errtext);
...
"further information.")) % program_name % COREQ_CACHEFILE;
}
...
int run_coreq_update(int argc, char *argv[]) {
	...
	/* calculate defaults for use_{percentage,status} */
		...
		if(percentage_tty || status_tty) {
			...
			if(status_tty) {
				use_status = (is_tty &&
					stringstart_in_wordlist(coreqrc["TERM"],
						split_string(coreqrc["TERM_STATUSLINE"])));
			}
		}
	}
	...
	Statusline statusline(use_status, (use_status &&
		stringstart_in_wordlist(coreqrc["TERM"],
			split_string(coreqrc["TERM_SOFTSTATUSLINE"]))),
		program_name, coreqrc["EXIT_STATUSLINE"]);
	...
	CacheTable table(coreqrc["CACHE_METHOD_PARSE"]); {
		...
		if(likely(find_filenames(excluded_overlays.begin(), excluded_overlays.end(),
				corepkg_settings["PORTDIR"].c_str(), true) == excluded_overlays.end())) {
			string errtext;
			if(unlikely(!table.addCache(coreqrc.prefix_cstr("EPREFIX_COREPKG_CACHE"),
				NULLPTR,
				corepkg_settings["PORTDIR"].c_str(),
				coreqrc["PORTDIR_CACHE_METHOD"],
				override_ptr,
				&errtext))) {
				coreq::say_error() % errtext;
			}
		...
		for(RepoList::const_iterator it(repos.second()); likely(it != repos.end()); ++it) {
			...
			if(likely(find_filenames(excluded_overlays.begin(), excluded_overlays.end(),
				...
				string errtext;
				if(unlikely(!table.addCache(coreqrc.prefix_cstr("EPREFIX_COREPKG_CACHE"),
					coreqrc.prefix_cstr("EPREFIX_ACCESS_OVERLAYS"),
					path,
					coreqrc["OVERLAY_CACHE_METHOD"], override_ptr, &errtext))) {
					coreq::say_error() % errtext;
				}
			} else {
	...
	string errtext;
	if(unlikely(!update(outputfile.c_str(), &table, &corepkg_settings, override_umask,
...
static bool update(const char *outputfile, CacheTable *cache_table, CorePkgSettings *corepkg_settings, bool override_umask, const RepoNames& repo_names, const WordVec& exclude_labels, Statusline *statusline, string *errtext) {
	DBHeader dbheader;
	WordVec categories;
	...
	for(CacheTable::iterator it(cache_table->begin());
		...
		INFO(_("[%s] \"%s\" %s (cache: %s)"))
			...
			% cache->getPathHumanReadable()
			% cache->getType();
		statusline->print(coreq::format(P_("Statusline coreq-update", "[%s] %s"))
				% cache->getKey()
				% cache->getOverlayName());
		...
		if(cache->can_read_multiple_categories()) {
			...
			if(use_percentage) {
				...
				reading_percent_status->init(coreq::format(NP_("Percent",
