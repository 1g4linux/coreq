...
ATTRIBUTE_NONNULL_ static bool update(const char* outputfile,
                                      CacheTable* cache_table,
                                      CorePkgSettings* corepkg_settings,
                                      bool override_umask,
                                      const RepoNames& repo_names,
                                      const WordVec& exclude_labels,
...
ATTRIBUTE_NONNULL_ static void add_override(Overrides* override_list, CoreqRc* coreqrc, const char* s);
ATTRIBUTE_NONNULL_ static void add_reponames(RepoNames* repo_names, CoreqRc* coreqrc, const char* s);
...
static void print_help() {
  coreq::say(
      _("Usage: %s [options]\n"
        ...
        "     --dump              show coreqrc-variables\n"
        "     --dump-defaults     show default coreqrc-variables\n"
        ...
        "further information.")) %
      program_name % COREQ_CACHEFILE;
}

enum update_options { O_DUMP = 260, O_DUMP_DEFAULTS, O_KNOWN_VARS, O_PRINT_VAR, O_FORCE_STATUS };

static bool quiet(false), show_help(false), show_version(false), known_vars(false), dump_coreqrc(false), dump_defaults(false);
...
**/
class CoreqUpdateOptionList : public OptionList {
 public:
  CoreqUpdateOptionList();
};

CoreqUpdateOptionList::CoreqUpdateOptionList() {
  ...
  push_back(Option("dump", O_DUMP, Option::BOOLEAN_T, &dump_coreqrc));
...
static void add_pathnames(PathVec* add_list, const WordVec& to_add, bool must_resolve) {
  for (WordVec::const_iterator it(to_add.begin()); unlikely(it != to_add.end()); ++it)
    add_list->EMPLACE_BACK(Pathname, (*it, must_resolve));
}

static void add_override(Overrides* override_list, CoreqRc* coreqrc, const char* s) {
  WordVec v;
  split_string(&v, (*coreqrc)[s], true);
  if (unlikely(v.size() & 1)) {
    coreq::say_error(_("%s must be a list of the form DIRECTORY METHOD")) % s;
...
static void add_reponames(RepoNames* repo_names, CoreqRc* coreqrc, const char* s) {
  WordVec v;
  split_string(&v, (*coreqrc)[s], true);
  if (unlikely(v.size() & 1)) {
    coreq::say_error(_("%s must be a list of the form DIR-PATTERN OVERLAY-LABEL")) % s;
    std::exit(EXIT_FAILURE);
  }
...
static void add_virtuals(Overrides* override_list, PathVec* add, RepoNames* repo_names, const string& cachefile, const string& eprefix_virtual) {
  ...
  if (unlikely(!db.openread(cachefile.c_str()))) {
    INFO(_("KEEP_VIRTUALS is ignored: there is no previous %s")) % cachefile;
    return;
  }
  ...
  if (unlikely(!is_current)) {
    coreq::say_error(_("warning: KEEP_VIRTUALS ignored because database format has changed"));
    return;
  }
  for (ExtendedVersion::Overlay i(0); likely(i != header.countOverlays()); ++i) {
    ...
    override_list->EMPLACE_BACK(Override, (name, string("coreq*::") + overlay));
...
static void override_label(OverlayIdent* overlay, const RepoNames& repo_names) {
  for (RepoNames::const_iterator it(repo_names.begin()); it != repo_names.end(); ++it) {
...
int run_coreq_update(int argc, char* argv[]) {
  ...
  /* Setup coreqrc. */
  CoreqRc& coreqrc(get_coreqrc(UPDATE_VARS_PREFIX));
  {
    string errtext;
  ...
  Depend::use_depend = coreqrc.getBool("DEP");
  Version::use_required_use = coreqrc.getBool("REQUIRED_USE");
  ExtendedVersion::use_src_uri = coreqrc.getBool("SRC_URI");
  string coreq_cachefile(coreqrc["COREQ_CACHEFILE"]);
  {
    /* calculate defaults for use_{percentage,status} */
    bool percentage_tty(false);
    if (coreqrc.getBool("NOPERCENTAGE")) {
      use_percentage = false;
    }
    else if (coreqrc.getBool("FORCE_PERCENTAGE")) {
      use_percentage = true;
    }
    else {
      percentage_tty = true;
    ...
    if (coreqrc.getBool("NOSTATUSLINE")) {
      use_status = false;
    }
    else if (coreqrc.getBool("FORCE_STATUSLINE")) {
      use_status = true;
    }
    else {
      status_tty = true;
    ...
    if (percentage_tty || status_tty) {
      ...
      if (status_tty) {
        use_status = (is_tty && stringstart_in_wordlist(coreqrc["TERM"], split_string(coreqrc["TERM_STATUSLINE"])));
      }
    }
  }

  /* other defaults */
  verbose = coreqrc.getBool("UPDATE_VERBOSE");

  /* Setup ArgumentReader. */
  ArgumentReader argreader(argc, argv, CoreqUpdateOptionList());
  ...
  if (unlikely(var_to_print != NULLPTR)) {
    if (coreqrc.print_var(var_to_print)) {
      return EXIT_SUCCESS;
    }
    return EXIT_FAILURE;
  }
  ...
  if (unlikely(dump_coreqrc || dump_defaults)) {
    coreqrc.dumpDefaults(stdout, dump_defaults);
    return EXIT_SUCCESS;
  }
  ...
  if (unlikely(quiet)) {
    if (!freopen(DEV_NULL, "w", stdout)) {
      coreq::say_error(_("cannot redirect to \"%s\"")) % DEV_NULL;
      return EXIT_FAILURE;
    }
  }

  /* set the outputfile */
  string outputfile(coreq_cachefile);
  ...
  Statusline statusline(use_status, (use_status && stringstart_in_wordlist(coreqrc["TERM"], split_string(coreqrc["TERM_SOFTSTATUSLINE"]))), program_name, coreqrc["EXIT_STATUSLINE"]);

  ParseError parse_error;
  INFO(_("Reading CorePkg settings..."));
  CorePkgSettings corepkg_settings(&coreqrc, &parse_error, false, true);
  ...
  /**/ {
    ...
    add_override(&override_list, &coreqrc, "CACHE_METHOD");
    PathVec excluded_list;
    add_pathnames(&excluded_list, split_string(coreqrc["EXCLUDE_OVERLAY"], true), true);
    PathVec add_list;
    add_pathnames(&add_list, split_string(coreqrc["ADD_OVERLAY"], true), true);

    if (unlikely(coreqrc.getBool("KEEP_VIRTUALS"))) {
      add_virtuals(&override_list, &add_list, &repo_names, coreq_cachefile, coreqrc["EPREFIX_VIRTUAL"]);
    }

    add_override(&override_list, &coreqrc, "OVERRIDE_CACHE_METHOD");

    /* Modify default (overlay/method/...) lists, using command line args */
    for (ExcludeArgs::iterator it(exclude_args->begin()); unlikely(it != exclude_args->end()); ++it)
      excluded_list.EMPLACE_BACK(Pathname, (*it, true));
    for (AddArgs::iterator it(add_args->begin()); unlikely(it != add_args->end()); ++it)
      add_list.EMPLACE_BACK(Pathname, (*it, true));
    for (MethodArgs::iterator it(method_args->begin()); unlikely(it != method_args->end()); ++it)
      override_list.EMPLACE_BACK(Override, (Pathname(it->first, true), it->second));

    /* For REPO_NAMES it is quite the opposite: */

    for (RepoArgs::iterator it(repo_args->begin()); unlikely(it != repo_args->end()); ++it)
      repo_names.EMPLACE_BACK(RepoName, (Pathname(it->first, false), it->second));

    add_reponames(&repo_names, &coreqrc, "REPO_NAMES");

    /* Normalize names: */

    for (PathVec::iterator it(excluded_list.begin()); unlikely(it != excluded_list.end()); ++it)
      excluded_overlays.PUSH_BACK(it->resolve(&corepkg_settings));

    for (PathVec::iterator it(add_list.begin()); unlikely(it != add_list.end()); ++it) {
      string add_name(it->resolve(&corepkg_settings));
      // Let exclude override added names
      if (find_filenames(excluded_overlays.begin(), excluded_overlays.end(), add_name.c_str(), true) == excluded_overlays.end()) {
        add_overlays.PUSH_BACK(MOVE(add_name));
      }
    }

    for (Overrides::iterator it(override_list.begin()); unlikely(it != override_list.end()); ++it) {
      override_vector.EMPLACE_BACK(OverridePair, ((it->name).resolve(&corepkg_settings), it->method));
    }
  }

  /* Calculate new PORTDIR_OVERLAY for export */

  if (likely(coreqrc.getBool("EXPORT_PORTDIR_OVERLAY"))) {
    bool modified(false);
    string& ref(corepkg_settings["PORTDIR_OVERLAY"]);
    WordVec overlayvec;
    split_string(&overlayvec, ref, true);
    for (WordVec::const_iterator it(add_overlays.begin()); unlikely(it != add_overlays.end()); ++it) {
      if (find_filenames(overlayvec.begin(), overlayvec.end(), it->c_str(), false, true) == overlayvec.end()) {
        overlayvec.PUSH_BACK(*it);
        modified = true;
      }
    }
    for (WordVec::iterator it(overlayvec.begin()); unlikely(it != overlayvec.end());) {
      if (find_filenames(excluded_overlays.begin(), excluded_overlays.end(), it->c_str(), true) == excluded_overlays.end()) {
        ++it;
      }
      else {
...
#else
      corepkg_settings.export_portdir_overlay = true;
#endif
    }
  }

  /* Create CacheTable and fill with PORTDIR and PORTDIR_OVERLAY. */
  CacheTable table(coreqrc["CACHE_METHOD_PARSE"]);
  {
    OverrideVector* override_ptr(override_vector.size() ? &override_vector : NULLPTR);
    if (likely(find_filenames(excluded_overlays.begin(), excluded_overlays.end(), corepkg_settings["PORTDIR"].c_str(), true) == excluded_overlays.end())) {
      string errtext;
      if (unlikely(!table.addCache(coreqrc.prefix_cstr("EPREFIX_COREPKG_CACHE"), NULLPTR, corepkg_settings["PORTDIR"].c_str(), coreqrc["PORTDIR_CACHE_METHOD"], override_ptr, &errtext))) {
        coreq::say_error() % errtext;
      }
    }
    else {
      INFO(_("Excluded PORTDIR: %s")) % corepkg_settings["PORTDIR"];
    }

    corepkg_settings.add_repo_vector(add_overlays, false);

    RepoList repos(corepkg_settings.repos);
    for (RepoList::const_iterator it(repos.second()); likely(it != repos.end()); ++it) {
      const char* path(it->path.c_str());
      if (likely(find_filenames(excluded_overlays.begin(), excluded_overlays.end(), path, true, false) == excluded_overlays.end())) {
        string errtext;
        if (unlikely(!table.addCache(coreqrc.prefix_cstr("EPREFIX_COREPKG_CACHE"), coreqrc.prefix_cstr("EPREFIX_ACCESS_OVERLAYS"), path, coreqrc["OVERLAY_CACHE_METHOD"], override_ptr, &errtext))) {
          coreq::say_error() % errtext;
        }
      }
      else {
        INFO(_("Excluded overlay %s")) % it->human_readable();
      }
    }
  }
  ...
  string errtext;
  if (unlikely(!update(outputfile.c_str(), &table, &corepkg_settings, override_umask, repo_names, excluded_overlays, &statusline, &errtext))) {
    coreq::say_error() % errtext;
    statusline.failure();
    return EXIT_FAILURE;
  }
...
static bool update(const char* outputfile,
                   CacheTable* cache_table,
                   CorePkgSettings* corepkg_settings,
                   bool override_umask,
                   const RepoNames& repo_names,
  ...
  DBHeader dbheader;
  WordVec categories;
  corepkg_settings->pushback_categories(&categories);
  PackageTree package_tree(categories);

  dbheader.world_sets = *(corepkg_settings->get_world_sets());

  /* We must first initialize all caches and erase unneeded ones,
     because some cache methods like coreqcache know about each other
     and call each other before we can call them in a loop afterwards. */
  for (CacheTable::iterator it(cache_table->begin()); likely(it != cache_table->end());) {
    BasicCache* cache(*it);
    cache->corepkgsettings = corepkg_settings;
    ...
    if (unlikely(find(exclude_labels.begin(), exclude_labels.end(), overlay.label) != exclude_labels.end())) {
      INFO(_("Excluding \"%s\" %s (cache: %s)")) % overlay.label % cache->getPathHumanReadable() % cache->getType();
    ...
    // cache->setArch((*corepkg_settings)["ARCH"]);
  ...
  /* Build database from scratch. */
  for (CacheTable::iterator it(cache_table->begin()); likely(it != cache_table->end()); ++it) {
    BasicCache* cache(*it);
    INFO(_("[%s] \"%s\" %s (cache: %s)")) % cache->getKey() % cache->getOverlayName() % cache->getPathHumanReadable() % cache->getType();
    statusline->print(coreq::format(P_("Statusline coreq-update", "[%s] %s")) % cache->getKey() % cache->getOverlayName());
    reading_percent_status = new PercentStatus;
    if (cache->can_read_multiple_categories()) {
      reading_percent_status->init(P_("Percent", "     Reading Packages..."));
      cache->setErrorCallback(error_callback);
      reading_percent_status->finish(likely(cache->readCategories(&package_tree)) ? P_("Percent", "Finished") : P_("Percent", "ABORTED!"));
    }
    else {
      if (use_percentage) {
        reading_percent_status->init(P_("Percent", "     Reading category %s|%s (%s%%)"), package_tree.size());
      }
      else {
        reading_percent_status->init(coreq::format(NP_("Percent", "     Reading %s category of packages...", "     Reading up to %s categories of packages...", package_tree.size())) %
                                     package_tree.size());
      }
      ...
      bool aborted(false);
      bool is_empty(true);
      for (PackageTree::const_iterator ci(package_tree.begin()); unlikely(ci != package_tree.end()); ++ci) {
        if (!cache->readCategoryPrepare(ci->first.c_str())) {
          ...
          if (use_percentage) {
            reading_percent_status->next(coreq::format(P_("Percent", ": %s...")) % ci->first);
          }
          is_empty = false;
      ...
      string msg(unlikely(is_empty) ? P_("Percent", "EMPTY!") : (unlikely(aborted) ? P_("Percent", "ABORTED!") : P_("Percent", "Finished")));
      if (use_percentage) {
  ...
  statusline->print(P_("Statusline coreq-update", "Analyzing"));

  /* Now apply all masks... */
  INFO(_("Applying masks..."));
  for (PackageTree::iterator c(package_tree.begin()); likely(c != package_tree.end()); ++c) {
  ...
  statusline->print(coreq::format(P_("Statusline coreq-update", "Creating %s")) % outputfile);
  ...
  if (unlikely(!ok)) {
    *errtext = coreq::format(_("cannot open database file %s for writing (mode = 'wb')")) % outputfile;
    return false;
  }

  dbheader.size = package_tree.countCategories();

  if (!(likely(db.write_header(dbheader, errtext)) && likely(db.write_packagetree(package_tree, dbheader, errtext)))) {
    return false;
  }

  INFO(N_("Database contains %s packages in %s category", "Database contains %s packages in %s categories", dbheader.size)) % package_tree.countPackages() % dbheader.size;
  return true;
}
