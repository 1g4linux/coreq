...
#ifndef SRC_COREQTK_OUTPUTSTRING_H_
#define SRC_COREQTK_OUTPUTSTRING_H_ 1
...
class OutputString {
  ...
  **/
  ...
  void append_internal(const std::string& t) { append_internal(t, 0, 0, false); }

 public:
  OutputString() : m_size(0), absolute(false) {}

  explicit OutputString(const std::string& t) : m_string(t) { append_internal(t); }

  explicit OutputString(const char* s) : m_string(s) { append_internal(s); }

  OutputString(const OutputString& s) : m_string(s.m_string), m_size(s.m_size), m_insert(s.m_insert), absolute(s.absolute) {}

  /**
  @arg s is the size in visible characters
  **/
  OutputString(const std::string& t, WordSize s) : m_string(t), m_size(s), absolute(false) {}
  ...
  bool empty() const { return (m_string.empty() && m_insert.empty()); }
...
inline static coreq::format& operator%(coreq::format& format, const OutputString& str) {  // NOLINT(runtime/references)
  format % str.as_string();
  return format;
}

#endif  // SRC_COREQTK_OUTPUTSTRING_H_
