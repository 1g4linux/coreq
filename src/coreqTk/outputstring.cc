...
bool OutputString::is_equal(const OutputString& t) const {
  return ((m_string == t.m_string) &&
          ...
          likely(m_insert.empty() ? t.m_insert.empty() : (t.m_insert.empty() ? false : likely(m_insert == t.m_insert))));
}
...
void OutputString::append_internal(const string& t, string::size_type ts, string::size_type s, bool a) {
  ...
  for (string::size_type currpos; unlikely((currpos = t.find_first_of("\a\b\n\r\t", lastpos)) != string::npos); lastpos = currpos + 1) {
...
void OutputString::append(const OutputString& a) {
  ...
  if (a.absolute) {
    ...
    for (InsertType::const_iterator it(a.m_insert.begin()); unlikely(it != a.m_insert.end()); ++it) {
...
void OutputString::print(std::string* dest, WordSize* s) const {
  ...
  if (likely(m_insert.empty())) {
    ...
    WordSize r(0);
    WordSize curr(*s);
    for (InsertType::const_iterator it(m_insert.begin()); unlikely(it != m_insert.end()); ++it) {
      if (*it > r) {
