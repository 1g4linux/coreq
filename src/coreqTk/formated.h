...
#ifndef SRC_COREQTK_FORMATED_H_
#define SRC_COREQTK_FORMATED_H_ 1
...
// When COREQ_DEBUG_FORMAT is defined, check about too many/few arguments.
...
// Without COREQ_DEBUG_FORMAT, the output string in case of too few arguments
// is unspecified, but it does not lead to a segfault or memory leak.

#ifndef NDEBUG
#ifdef COREQ_PARANOIC_ASSERT
#ifndef COREQ_DEBUG_FORMAT
#define COREQ_DEBUG_FORMAT 1
#endif
#endif
#endif
...
\code
  ...
  std::cout << coreq::format(_("problems while parsing %s -- %s")) % file % message << std::endl;
  // problems while parsing /etc/make.conf -- something bad happend

  int line = 10;
  std::string::size_type column = 20;
  coreq::say_error(_("problems while parsing %s in line %3$s, column %2$d -- %s"))
...
class FormatManip {
 protected:
  ...
  static CONSTEXPR const ArgType NONE = 0x00, STRING = 0x01, DIGIT = 0x02, BOTH = (STRING | DIGIT);
 ...
 public:
  FormatManip(std::string::size_type ind, ArgCount anum, ArgType typ) NOEXCEPT : m_index(ind), argnum(anum), m_type((typ & DIGIT) == NONE) {  // NOLINT(runtime/references)
  }
};
...
class format {
  ...
  **/
  template <typename T>
  static std::ostream& write_representation(std::ostream& s, const T& t) {  // NOLINT(runtime/references)
    return s << t;
  }
 ...
 public:
  format(FILE* stream, const std::string& format_string, bool newline, bool flush) : add_newline(newline), do_flush(flush), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, const char* format_string, bool newline, bool flush) : add_newline(newline), do_flush(flush), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, char format_char, bool newline, bool flush) : simple(false), add_newline(newline), do_flush(flush), output(stream), m_text(1, format_char) { newline_output(); }

  format(FILE* stream, const std::string& format_string, bool newline) : add_newline(newline), do_flush(false), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, const char* format_string, bool newline) : add_newline(newline), do_flush(false), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, char format_char, bool newline) : simple(false), add_newline(newline), do_flush(false), output(stream), m_text(1, format_char) { newline_output(); }

  format(FILE* stream, const std::string& format_string) : add_newline(false), do_flush(false), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, const char* format_string) : add_newline(false), do_flush(false), output(stream), m_text(format_string) { init(); }

  format(FILE* stream, char format_char) : simple(false), add_newline(false), do_flush(false), output(stream), m_text(1, format_char) { newline_output(); }

  format(const std::string& format_string, bool newline) : add_newline(newline), output(NULLPTR), m_text(format_string) { init(); }

  format(const char* format_string, bool newline) : add_newline(newline), output(NULLPTR), m_text(format_string) { init(); }

  format(char format_char, bool newline) : simple(false), add_newline(newline), output(NULLPTR), m_text(1, format_char) { newline_output(); }

  explicit format(const std::string& format_string) : add_newline(false), output(NULLPTR), m_text(format_string) { init(); }

  explicit format(const char* format_string) : add_newline(false), output(NULLPTR), m_text(format_string) { init(); }
  ...
  format(bool newline, bool flush, FILE* stream) : simple(false), add_newline(newline), do_flush(flush), output(stream) { newline_output(); }

  // Exceptional order in which case we use the empty string as default
  format(bool newline, FILE* stream) : simple(false), add_newline(newline), do_flush(false), output(stream) { newline_output(); }
  ...
  **/
  friend std::ostream& operator<<(std::ostream& os, const format& formater) { return os << formater.str(); }
};
