...
#define md5call(func, a, b, c, d, s, ac, x)   \
  do {                                        \
    a += func(b, c, d) + x + ac;              \
    a = (md5rotate(a, s) + b) & 0xFFFFFFFFUL; \
  } while (0)

static CONSTEXPR const uint32_t sinlistF[16] = {0xD76AA478UL, 0xE8C7B756UL, 0x242070DBUL, 0xC1BDCEEEUL, 0xF57C0FAFUL, 0x4787C62AUL, 0xA8304613UL, 0xFD469501UL,
                                                0x698098D8UL, 0x8B44F7AFUL, 0xFFFF5BB1UL, 0x895CD7BEUL, 0x6B901122UL, 0xFD987193UL, 0xA679438EUL, 0x49B40821UL};

static CONSTEXPR const uint32_t sinlistG[16] = {0xF61E2562UL, 0xC040B340UL, 0x265E5A51UL, 0xE9B6C7AAUL, 0xD62F105DUL, 0x02441453UL, 0xD8A1E681UL, 0xE7D3FBC8UL,
                                                0x21E1CDE6UL, 0xC33707D6UL, 0xF4D50D87UL, 0x455A14EDUL, 0xA9E3E905UL, 0xFCEFA3F8UL, 0x676F02D9UL, 0x8D2A4C8AUL};

static CONSTEXPR const uint32_t sinlistH[16] = {0xFFFA3942UL, 0x8771F681UL, 0x6D9D6122UL, 0xFDE5380CUL, 0xA4BEEA44UL, 0x4BDECFA9UL, 0xF6BB4B60UL, 0xBEBFBC70UL,
                                                0x289B7EC6UL, 0xEAA127FAUL, 0xD4EF3085UL, 0x04881D05UL, 0xD9D4D039UL, 0xE6DB99E5UL, 0x1FA27CF8UL, 0xC4AC5665UL};

static CONSTEXPR const uint32_t sinlistI[16] = {0xF4292244UL, 0x432AFF97UL, 0xAB9423A7UL, 0xFC93A039UL, 0x655B59C3UL, 0x8F0CCC92UL, 0xFFEFF47DUL, 0x85845DD1UL,
                                                0x6FA87E4FUL, 0xFE2CE6E0UL, 0xA3014314UL, 0x4E0811A1UL, 0xF7537E82UL, 0xBD3AF235UL, 0x2AD7D2BBUL, 0xEB86D391UL};

static CONSTEXPR const unsigned int permutG[16] = {1, 6, 11, 0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12};

static CONSTEXPR const unsigned int permutH[16] = {5, 8, 11, 14, 1, 4, 7, 10, 13, 0, 3, 6, 9, 12, 15, 2};

static CONSTEXPR const unsigned int permutI[16] = {0, 7, 14, 5, 12, 3, 10, 1, 8, 15, 6, 13, 4, 11, 2, 9};

static CONSTEXPR const uint32_t md5init[4] = {0x67452301UL, 0xEFCDAB89UL, 0x98BADCFEUL, 0x10325476UL};

static void md5fill(const char* buffer, uint32_t* mybuf, unsigned int len) {
  while (len != 0) {
    *(mybuf++) = (static_cast<uint32_t>(buffer[3]) << 24) + (static_cast<uint32_t>(buffer[2]) << 16) + (static_cast<uint32_t>(buffer[1]) << 8) + static_cast<uint32_t>(buffer[0]);
    buffer += 4;
    --len;
  }
}
...
static void calc_md5sum(const char* buffer, Md5DataLen totalsize, uint32_t* resarr) {
  ...
  switch (restsize % 4) {
    case 3:
      mybuf[len] = (0x80UL << 24) + (static_cast<uint32_t>(buffer[2]) << 16) + (static_cast<uint32_t>(buffer[1]) << 8) + static_cast<uint32_t>(buffer[0]);
      break;
    case 2:
      mybuf[len] = (0x80UL << 16) + (static_cast<uint32_t>(buffer[1]) << 8) + static_cast<uint32_t>(buffer[0]);
      break;
    case 1:
      mybuf[len] = (0x80UL << 8) + static_cast<uint32_t>(buffer[0]);
      break;
    /* case 0: */
    default:
      mybuf[len] = 0x80UL;
...
static void debug_md5(const uint32_t* resarr) {
  for (int i(0); i < 4; ++i) {
    ...
    for (int j(0); j < 8; ++j) {
      ...
      if (c < 10) {
        coreq::print(static_cast<char>('0' + c));
      }
      else {
        c -= 10;
        coreq::print(static_cast<char>('a' + c));
      }
    }
  }
coreq:
...
#ifdef DEBUG_MD5
  coreq::print("file: %s size: %s should be: %s is: ") % file % filesize % md5sum;
  debug_md5(resarr);
#endif
  ...
  for (int i(0); i < 4; ++i) {
    ...
    for (int j(0); j < 8; ++j) {
      ...
      if (c < 10) {
        ...
        c -= 10;
        if ((md5sum[curr] != ('a' + c)) && (md5sum[curr] != ('A' + c))) {
          return false;
        }
      }
      ++curr;
