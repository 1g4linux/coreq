...
#ifndef SRC_COREQTK_FORWARD_LIST_H_
#define SRC_COREQTK_FORWARD_LIST_H_ 1

#include <config.h>  // IWYU pragma: keep

// check_includes: include "coreqTk/forward_list.h"
...
class forward_list_node {
 public:
  ...
  explicit forward_list_node(const T& val) : value(val) {}
};
...
class forward_list_iterator {
 public:
  ...
  explicit forward_list_iterator(forward_list_node<T>* node) : m_node(node) {}
...
class forward_list_const_iterator {
 public:
  ...
  explicit forward_list_const_iterator(forward_list_node<T>* node) : m_node(node) {}

  const_reference operator*() const { return m_node->value; }

  const_pointer operator->() const { return &(m_node->value); }
...
class forward_list {
 private:
  ...
  void assign_no_clear(const forward_list& source) {
    ...
    for (node_ptr curr(source.head); likely(curr != NULLPTR); curr = curr->node) {
 ...
 public:
  ...
  forward_list(const forward_list& source) { assign_no_clear(source); }
...
#ifdef HAVE_MOVE
  forward_list(forward_list&& s) NOEXCEPT : head(s.head) { s.head = NULLPTR; }
...
#endif
  ~forward_list() { clear_no_headnull(); }
...
#endif  // SRC_COREQTK_FORWARD_LIST_H_
