...
#ifndef SRC_COREQTK_VARSREADER_H_
#define SRC_COREQTK_VARSREADER_H_ 1
...
class VarsReader {
 public:
  ...
  static CONSTEXPR const Flags NONE = 0x0000U,          ///< Flag: No flags set; normal behavior.
      ...
      COREPKG_ESCAPES = 0x0100U,                        ///< Flag: Treat escapes like corepkg does.
      COREPKG_SECTIONS = 0x0200U,                       ///< Flag: Section format of e.g. repos.conf
  ...
  **/
  ...
  ATTRIBUTE_NONNULL((2)) bool read(const char* filename, std::string* errtext, bool noexist_ok, WordUnorderedSet* sourced) { return read(filename, errtext, noexist_ok, sourced, false); }
  ATTRIBUTE_NONNULL((2)) bool read(const char* filename, std::string* errtext, bool noexist_ok) { return read(filename, errtext, noexist_ok, NULLPTR); }

  /**
  Use a supplied map for variables.
  **/
  ATTRIBUTE_NONNULL_ void useMap(WordIterateMap* vars_map) { vars = vars_map; }

  /**
  Prefix (path resp. varname) used for sourcing
  **/
  void setPrefix(const std::string& prefix) { source_prefix = prefix; }

  /**
  Set array of keys which values should be prepended to the new value.
  **/
  void accumulatingKeys(const char* const* keys) { incremental_keys = keys; }

  /**
  Operator that helps us to be used like a map.
  **/
  std::string& operator[](const std::string& key) { return (*vars)[key]; }
  ...
  VarsReader::const_iterator begin() const { return vars->begin(); }

  VarsReader::const_iterator end() const { return vars->end(); }
  ...
  */
  ...
  Copy INPUT into value-buffer while INPUT is not in ['\\].
  If the value ends, ASSIGN_KEY_VALUE is called.
  '\\' -> [RV] SINGLE_QUOTE_ESCAPE_COREPKG | '\'' -> [RV] JUMP_NOISE
  **/
  void VALUE_SINGLE_QUOTE_COREPKG();
  ...
  **/
  ...
  Copy INPUT into value-buffer while INPUT is not in ["\\].
  If the value ends, ASSIGN_KEY_VALUE is called.
  '\\' -> [RV] DOUBLE_QUOTE_ESCAPE_COREPKG] | '"' -> [RV] JUMP_NOISE
  **/
  void VALUE_DOUBLE_QUOTE_COREPKG();
  ...
  **/
  void SINGLE_QUOTE_ESCAPE_COREPKG();
  ...
  **/
  ...
  /**
  Care about \\ in double-quote values for COREPKG_ESCAPES.
  ...
  **/
  void DOUBLE_QUOTE_ESCAPE_COREPKG();
  ...
  **/
  ...
  /**
  Care about \\ in values without quotes for COREPKG_ESCAPES.
  \n is ignored, known CorePkg escapes are handled
  and everything else is put into buffer without the \\.
  -> [RV] VALUE_WHITESPACE_COREPKG
  **/
  void WHITESPACE_ESCAPE_COREPKG();
  ...
  **/
  ATTRIBUTE_PURE static bool isValidKeyCharacter(char c) { return (my_isalnum(c) || (c == '_') || (c == '-')); }

  /**
  @return true if c matches [A-Za-z_]
  **/
  ATTRIBUTE_PURE static bool isValidKeyCharacterStart(char c) { return (my_isalpha(c) || (c == '_')); }
  ...
  **/
  ...
  bool isIncremental(const char* key) { return match_list(incremental_keys, key); }
};

#endif  // SRC_COREQTK_VARSREADER_H_
