...
ATTRIBUTE_PURE ATTRIBUTE_NONNULL_ inline static S calc_table_pos(const vector<S>& table, S pos, const T* pattern, T c);
template <typename T>
ATTRIBUTE_NONNULL_ inline static void split_string_template(T* vec, const string& str, bool handle_escape, const char* at, bool ignore_empty);
template <typename T>
ATTRIBUTE_NONNULL_ inline static void join_to_string_template(string* s, const T& vec, const string& glue);
...
const char* ExplodeAtom::get_start_of_version(const char* str, bool allow_star) {
  ...
  while (likely((*str != '\0') && (*str != ':') && (*str != '['))) {
    if (unlikely(*str++ == '-')) {
      if (my_isdigit(*str) || unlikely(allow_star && ((*str) == '*'))) {
        x = str;
      }
    }
  }
...
coreq::SignedBool natcmp(const string& a, const string& b) {
  string::size_type i(0);
  string::size_type j(0);
  coreq::SignedBool numerical(0);
  coreq::SignedBool zeroes(0);
  bool number(false);
  for (;; ++i, ++j) {
    if (i == a.size()) {
      if (j == b.size()) {
    ...
    if (c == d) {
      ...
      coreq::SignedBool result((numerical == 0) ? zeroes : numerical);
      if (result != 0) {
        return result;
      }
      number = false;
...
template <typename T>
inline static void split_string_template(T* vec, const string& str, bool handle_escape, const char* at, bool ignore_empty) {
...
template <typename T>
inline static void join_to_string_template(string* s, const T& vec, const string& glue) {
...
bool resolve_plus_minus(WordSet* s, const WordVec& l, const WordSet* warnignore) {
  ...
  for (WordVec::const_iterator it(l.begin()); likely(it != l.end()); ++it) {
    ...
    if (unlikely((*it)[0] == '+')) {
      coreq::say_error(_("flags should not start with a '+': %s")) % (*it);
    ...
    if (unlikely((*it)[0] == '-')) {
      ...
      if (*it == "-~*") {
        for (WordSet::iterator i(s->begin()); unlikely(i != s->end());) {
          if ((i->size() >= 2) && ((*i)[0] == '~')) {
            s->erase(i++);
          }
          else {
...
void StringHash::store_string(const string& s) {
  if (finalized) {
    coreq::say_error(_("internal error: storing required after finalizing"));
...
void StringHash::hash_string(const string& s) {
  if (finalized) {
    coreq::say_error(_("internal error: hashing required after finalizing"));
    std::exit(EXIT_FAILURE);
  }
  if (!hashing) {
    coreq::say_error(_("internal error: hashing required in non-hash mode"));
    std::exit(EXIT_FAILURE);
  }
...
StringHash::size_type StringHash::get_index(const string& s) const {
  if (!finalized) {
    coreq::say_error(_("internal error: index required before sorting"));
    std::exit(EXIT_FAILURE);
  }
  StrSizeMap::const_iterator i(str_map.find(s));
  if (i == str_map.end()) {
    coreq::say_error(_("internal error: trying to shortcut non-hashed string"));
    std::exit(EXIT_FAILURE);
  }
  return i->second;
}
...
const string& StringHash::operator[](StringHash::size_type i) const {
  if (i >= size()) {
    coreq::say_error(_("database corrupt: nonexistent hash required"));
    std::exit(EXIT_FAILURE);
  }
...
**/
template <typename S, typename T>
inline static S calc_table_pos(const vector<S>& table, S pos, const T* pattern, T c) {
  while (pattern[pos] != c) {
    if (pos == 0) {
      return 0;
    }
