...
static CONSTEXPR const ColorType acNone = 0, amNone = 50, amBold = 1, amItalic = 3, amUnderlined = 4, amBlink = 5, amInverse = 7, fgFirst = 30, fgBlack = fgFirst + 0, fgRed = fgFirst + 1,
                                 fgGreen = fgFirst + 2, fgYellow = fgFirst + 3, fgBlue = fgFirst + 4, fgPurple = fgFirst + 5, fgCyan = fgFirst + 6, fgGray = fgFirst + 7, fg256 = fgFirst + 8,
                                 bgFirst = 40;

void AnsiColor::init_static() {
  coreq_assert_static(static_color_map == NULLPTR);
...
bool AnsiColor::initcolor(const string& str, string* errtext) {
  ...
  for (;;) {
    ...
    if (f != static_color_map->end()) {
      ...
      if (errtext != NULLPTR) {
        *errtext = coreq::format(_("illegal color name \"%s\" in \"%s\"")) % curr % str;
      }
      ok = false;
      break;
    }
    if (col != amNone) {
      ...
      coreq::array<char, 10> buffer;
      if (iscol < 0) {
        std::snprintf(buffer.data(), buffer.size(), ";%d;5;%s", static_cast<int>(col), curr.c_str());
      }
      else {
        std::snprintf(buffer.data(), buffer.size(), ";%d", static_cast<int>(col));
      }
      if (iscol == 0) {
        ...
        if (errtext != NULLPTR) {
          *errtext = coreq::format(_("more than two colors specified in \"%s\"")) % str;
        }
        ok = false;
        break;
      }
    ...
    if (str[nextpos] == ',') {
      ...
      if (likely(curr == "1")) {
        ...
        if (errtext != NULLPTR) {
          *errtext = coreq::format(_("invalid brightness value \"%s\"")) % curr;
        }
        ok = false;
        break;
      }
...
void AnsiColor::AnsiPalette() {
  coreq::array<char, 40> buffer;
  for (CalcType red(0); red < 6; ++red) {
    for (CalcType green(0); green < 6; ++green) {
      for (CalcType blue(0); blue < 6; ++blue) {
        std::snprintf(buffer.data(), buffer.size(), "\x1B]4;%d;rgb:%2.2x/%2.2x/%2.2x\x1B\\", static_cast<int>(16 + (red * 36) + (green * 6) + blue), static_cast<int>(transcalc(red)),
                      static_cast<int>(transcalc(green)), static_cast<int>(transcalc(blue)));
  ...
  for (CalcType gray(0); gray < 24; ++gray) {
    ...
    std::snprintf(buffer.data(), buffer.size(), "\x1B]4;%d;rgb:%2.2x/%2.2x/%2.2x\x1B\\", static_cast<int>(232 + gray), trans, trans, trans);
    coreq::print() % buffer.data();
  }
}
