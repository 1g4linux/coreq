...
RegexList::~RegexList() {
  for (vector<Regex*>::iterator it(reglist.begin()); likely(it != reglist.end()); ++it) {
    delete *it;
  }
}

bool RegexList::match(const char* str) {
  for (vector<Regex*>::const_iterator it(reglist.begin()); likely(it != reglist.end()); ++it) {
    if ((*it)->match(str)) {
      return true;
    }
  }
