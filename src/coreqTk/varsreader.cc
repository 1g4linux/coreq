...
const VarsReader::Flags VarsReader::NONE, VarsReader::ONLY_KEYWORDS_SLOT, VarsReader::KEYWORDS_READ, VarsReader::SLOT_READ, VarsReader::SUBST_VARS, VarsReader::INTO_MAP, VarsReader::APPEND_VALUES,
    VarsReader::ALLOW_SOURCE, VarsReader::ALLOW_SOURCE_VARNAME, VarsReader::COREPKG_ESCAPES, VarsReader::COREPKG_SECTIONS, VarsReader::RECURSE, VarsReader::HAVE_READ, VarsReader::ONLY_HAVE_READ;
...
void VarsReader::ASSIGN_KEY_VALUE() {
  if (unlikely(sourcecmd)) {
    ...
    if (unlikely(!source(value, &errtext))) {
      m_errtext = coreq::format(_("%s: failed to source %s (%s)")) % file_name % value % errtext;
      ERROR;
    }
...
void VarsReader::JUMP_WHITESPACE() {
  ...
  switch (INPUT) {
    case ';':
      if ((parse_flags & COREPKG_SECTIONS) == NONE) {
        break;
      }
    ...
    case '.':
      NEXT_INPUT;
      if ((parse_flags & ALLOW_SOURCE) && (INPUT == '\t' || INPUT == ' ')) {
        sourcecmd = true;
        CHSTATE(EVAL_VALUE);
      }
      else {
...
void VarsReader::FIND_ASSIGNMENT() {
  key_len = 0;
  if (likely((parse_flags & COREPKG_SECTIONS) == NONE)) {
...
'\'' -> [RV] VALUE_SINGLE_QUOTE{_COREPKG}
'"' -> [RV] VALUE_DOUBLE_QUOTE{_COREPKG}
'\\' -> [RV] WHITESPACE_ESCAPE{_COREPKG} | -> VALUE_WHITESPACE{_COREPKG}
We are resetting the value-buffer.
**/
void VarsReader::EVAL_VALUE() {
  NEXT_INPUT;
  VALUE_CLEAR;
  if (parse_flags & COREPKG_ESCAPES) {
    SKIP_SPACE;
    switch (INPUT) {
      case '"':
        NEXT_INPUT;
        CHSTATE(VALUE_DOUBLE_QUOTE_COREPKG);
      case '\'':
        NEXT_INPUT;
        CHSTATE(VALUE_SINGLE_QUOTE_COREPKG);
      case '\r':
      case '\n':
      ...
      case '\\':
        NEXT_INPUT;
        CHSTATE(WHITESPACE_ESCAPE_COREPKG);
      default:
        CHSTATE(VALUE_WHITESPACE_COREPKG);
    }
  }
  switch (INPUT) {
    case '"':
...
/**
Backslash escapes supported by corepkg.
**/
...
/**
Read a value enclosed in single quotes (') for COREPKG_ESCAPES.
Copy INPUT into value-buffer while INPUT is not in ['\\].
If the value ends, EVAL_READ is called.
'\\' -> [RV] SINGLE_QUOTE_ESCAPE_COREPKG | '\'' -> [RV] EVAL_READ
**/
void VarsReader::VALUE_SINGLE_QUOTE_COREPKG() {
  while (likely((INPUT != '\'') && (INPUT != '\\'))) {
    ...
    if (unlikely((INPUT == '%') && ((parse_flags & (SUBST_VARS | COREPKG_SECTIONS)) == (SUBST_VARS | COREPKG_SECTIONS)))) {
  ...
  switch (INPUT) {
    ...
    default:
      NEXT_INPUT_EVAL;
      CHSTATE(SINGLE_QUOTE_ESCAPE_COREPKG);
  }
}
...
/**
Read value enclosed in double-quotes (") for COREPKG_ESCAPES.
Copy INPUT into value-buffer while INPUT is not in ["\\].
If the value ends, EVAL_READ is called.
'\\' -> [RV] DOUBLE_QUOTE_ESCAPE_COREPKG | '"' -> [RV] EVAL_READ
**/
void VarsReader::VALUE_DOUBLE_QUOTE_COREPKG() {
  while (likely(INPUT != '"' && INPUT != '\\')) {
    if (unlikely((INPUT == '$') && ((parse_flags & SUBST_VARS) != NONE))) {
      resolveReference();
      if (INPUT_EOF) {
  ...
  switch (INPUT) {
    ...
    default:
      NEXT_INPUT_EVAL;
      CHSTATE(DOUBLE_QUOTE_ESCAPE_COREPKG);
  }
}
...
/**
Read value not enclosed in any quotes for COREPKG_ESCAPES.
Thus there are no spaces and tabs allowed. Everything must be escaped.
Move INPUT into buffer while it's not in [ \t\n\r\\'"#].
If the value ends we call EVAL_READ.
[# \t\r\n] -> EVAL_READ |
'\\' -> WHITESCAPE_ESCAPE_COREPKG | \' -> VALUE_SINGLE_QUOTE_COREPKG |
'"' -> VALUE_DOUBLE_QUOTE_COREPKG
**/
void VarsReader::VALUE_WHITESPACE_COREPKG() {
  for (;;) {
    switch (INPUT) {
      case '#':
      case ' ':
      ...
      case '$':
        ...
        break;
      case '%':
        if ((parse_flags & (SUBST_VARS | COREPKG_SECTIONS)) == (SUBST_VARS | COREPKG_SECTIONS)) {
          resolveSectionReference();
          if (INPUT_EOF) {
            CHSTATE(EVAL_READ);
          }
...
/**
Care about \\ in single-quote values for COREPKG_ESCAPES.
\n is ignored, known CorePkg escapes are handled
and everything else is put into buffer without the \\.
-> [RV] VALUE_SINGLE_QUOTE_COREPKG
**/
void VarsReader::SINGLE_QUOTE_ESCAPE_COREPKG() {
  bool wasnl(false);
  switch (INPUT) {
    case 'a':
      VALUE_APPEND(ESC_A);
  ...
  NEXT_INPUT_EVAL;
  /* any amount of backslashes forbids corepkg to expand var */
...
/**
Cares about \\ in double-quote values for COREPKG_ESCAPES.
\n is ignored, known CorePkg escapes are handled
and everything else is put into buffer without the \\.
-> [RV] VALUE_DOUBLE_QUOTE_COREPKG
**/
void VarsReader::DOUBLE_QUOTE_ESCAPE_COREPKG() {
  bool wasnl(false);
  switch (INPUT) {
    case 'a':
      VALUE_APPEND(ESC_A);
...
/**
Cares about \\ in values without quotes for COREPKG_ESCAPES.
\n is ignored, known CorePkg escapes are handled
and everything else is put into buffer without the \\.
-> [RV] VALUE_WHITESPACE_COREPKG
**/
void VarsReader::WHITESPACE_ESCAPE_COREPKG() {
  switch (INPUT) {
    case '\n':
      break;
    case '\\':
...
void VarsReader::var_append(const char* beginning, size_t ref_key_length) {
  ...
  const_iterator it;
  if (likely(section.empty()) || ((it = vars->find(varname + section)) == vars->end())) {
...
bool VarsReader::read(const char* filename, string* errtext, bool noexist_ok, WordUnorderedSet* sourced, bool nodir) {
  if ((!nodir) && ((parse_flags & RECURSE) != NONE)) {
    ...
    if (pushback_files(dir, &files, pushback_files_recurse_exclude, 3, true, true)) {
      for (WordVec::const_iterator file(files.begin()); likely(file != files.end()); ++file) {
  ...
  if (fd == -1) {
    ...
    if (errtext != NULLPTR) {
      *errtext = coreq::format(_("cannot read file %s")) % filename;
    }
    return false;
  }
  struct stat st;
  if (fstat(fd, &st)) {
    close(fd);
    if (errtext != NULLPTR) {
      *errtext = coreq::format(_("cannot stat file %s")) % filename;
    }
    return false;
  }
  if (st.st_size == 0) {
  ...
  if (likely(topcall)) {
    ...
    if (sourced->count(truename) != 0) {
      if (errtext != NULLPTR) {
        *errtext = coreq::format(_("recursively sourced file %s")) % truename;
      }
      return false;
    }
  }
...
bool VarsReader::parse() {
  ...
  for (iterator i(vars->begin()); i != vars->end(); ++i) {
    if ((!i->second.empty()) && isIncremental(i->first.c_str())) {
  ...
  for (IncType::const_iterator it(incremental.begin()); it != incremental.end(); ++it) {
