...
void format::bad_format() const {
	coreq::say_error(_("internal error: bad format specification \"%s\""))
		% m_text;
...
void format::too_few_arguments() const {
	coreq::say_error(_("internal error: too few arguments passed for \"%s\""))
		% m_text;
	std::exit(EXIT_FAILURE);
}

void format::too_many_arguments() const {
	coreq::say_error(_("internal error: too many arguments passed for \"%s\""))
		% m_text;
	std::exit(EXIT_FAILURE);
}
#endif
