...
// check_includes.sh: coreq::format()

namespace coreq {

const FormatManip::ArgType FormatManip::NONE, FormatManip::STRING, FormatManip::DIGIT, FormatManip::BOTH;

void format::bad_format() const {
  coreq::say_error(_("internal error: bad format specification \"%s\"")) % m_text;
  std::exit(EXIT_FAILURE);
}

#ifdef COREQ_DEBUG_FORMAT
void format::too_few_arguments() const {
  coreq::say_error(_("internal error: too few arguments passed for \"%s\"")) % m_text;
  std::exit(EXIT_FAILURE);
}

void format::too_many_arguments() const {
  coreq::say_error(_("internal error: too many arguments passed for \"%s\"")) % m_text;
  std::exit(EXIT_FAILURE);
}
#endif
...
void format::finalize() {
  for (std::vector<FormatManip>::const_reverse_iterator it(manip.rbegin()); it != manip.rend(); ++it) {
    m_text.insert(it->m_index, (it->m_type ? args[it->argnum].s : args[it->argnum].d));
  }
  manip.clear();
  wanted.clear();
  args.clear();
