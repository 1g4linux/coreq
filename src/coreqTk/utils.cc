...
class Directory {
 ...
 public:
  ...
  bool opendirectory(const char* name) { return ((dh = opendir(name)) != NULLPTR); }
...
ATTRIBUTE_NONNULL((1, 2)) static bool pushback_lines_file(const char* file, WordVec* v, bool keep_empty, coreq::SignedBool keep_comments, string* errtext);
...
**/
static bool pushback_lines_file(const char* file, LineVec* v, bool keep_empty, coreq::SignedBool keep_comments, string* errtext) {
  string line;
  std::ifstream ifstr(file);
  if (unlikely(!ifstr.is_open())) {
    if (errtext != NULLPTR) {
      *errtext = coreq::format(_("cannot open %s: %s")) % file % std::strerror(errno);
    }
    return false;
  }
  ...
  if (errtext != NULLPTR) {
    *errtext = coreq::format(_("error reading %s: %s")) % file % std::strerror(errno);
  }
  return false;
}
...
**/
bool pushback_lines(const char* file, LineVec* v, bool recursive, bool keep_empty, coreq::SignedBool keep_comments, string* errtext) {
  if (!recursive) {
...
bool pushback_files(const string& dir_path, WordVec* into, const char* const exclude[], unsigned char only_type, bool no_hidden, bool full_path) {
  ...
  for (WordVec::iterator it(namelist.begin()); likely(it != namelist.end()); ++it) {
    if (full_path) {
...
bool pushback_files_recurse(const string& dir_path, const string& subpath, int depth, WordVec* into, bool full_path, string* errtext) {
  ...
  if (pushback_files(dir_path, &files, pushback_files_exclude, 3, true, false)) {
    ...
    for (WordVec::const_iterator it(files.begin()); likely(it != files.end()); ++it) {
      if (depth == 100) {
        if (errtext != NULLPTR) {
          *errtext = coreq::format(_("nesting level too deep in %s")) % dir_path;
        }
        return false;
      }
