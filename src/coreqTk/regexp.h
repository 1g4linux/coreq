...
#ifndef SRC_COREQTK_REGEXP_H_
#define SRC_COREQTK_REGEXP_H_ 1
...
class Regex {
  ...
  **/
  Regex(const char* regex, int eflags) : m_compiled(false) { compile(regex, eflags); }

  /**
  Initalize and compile regular expression
  **/
  explicit Regex(const char* regex) : m_compiled(false) { compile(regex, REG_EXTENDED); }
  ...
  **/
  ...
  void compile(const char* regex) { compile(regex, REG_EXTENDED); }
  ...
  **/
  const regex_t* get() const { return &m_re; }
