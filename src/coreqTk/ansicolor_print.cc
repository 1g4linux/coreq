...
const AnsiColor::WhichPalette AnsiColor::PALETTE_NONE, AnsiColor::PALETTE_D0, AnsiColor::PALETTE_D1, AnsiColor::PALETTE_D, AnsiColor::PALETTE_L0, AnsiColor::PALETTE_L1, AnsiColor::PALETTE_L,
    AnsiColor::PALETTE_B, AnsiColor::PALETTE_ALL;

class Display {
 private:
  bool bold, printed_palette;
  coreq::SignedBool foreground;
 ...
 public:
  Display() { printed_palette = false; }

  void palette(coreq::SignedBool f, bool b, const char* s);
};

void Display::resetnl() {
  coreq::say() % AnsiColor::reset();
}
...
void Display::output(CalcType color, CalcType fg, CalcType dark, CalcType light) {
  coreq::array<char, 30> buffer;
  std::snprintf(buffer.data(), buffer.size(), "\x1B[%s38;5;%d;48;5;%dm%3d ", (bold ? "1;" : ""), static_cast<int>(foreground ? color : fg),
                static_cast<int>(foreground ? ((foreground > 0) ? dark : light) : color), static_cast<int>(color));
  ...
  for (CalcType green(0); green < 6; ++green) {
    for (CalcType red(red_s);;) {
      for (CalcType blue(0); blue < 6; ++blue) {
        output((16 + (red * 36) + (green * 6) + blue), ((((green * 4) + (red * 3) + (blue * 1) < 12)) ? 7 : 0), (((green != 0) || (red != 0) || (blue > 2)) ? 0 : 238),
               ((((green * 4) + (red * 3) + (blue * 1) > 26)) ? 244 : 7));
      }
      coreq::print() % AnsiColor::reset();
      if (++red == red_e) {
        break;
      }
      coreq::print(' ');
...
void Display::palette(coreq::SignedBool f, bool b, const char* s) {
  foreground = f;
  bold = b;
  if (printed_palette) {
    nl();
  ...
  coreq::say(_("System Colors (%s):")) % s;
  syscol();
  coreq::say(_("6x6x6 Color Cube (%s):")) % s;
  cube();
  coreq::say(_("Grayscale Ramp (%s):")) % s;
  ramp();
}
