...
coreq::TinyUnsigned ArgumentReader::numargs(Option::Type opt_type) {
  switch (opt_type) {
    case Option::STRING:
...
const Option* ArgumentReader::lookup_option(const int opt, const OptionList& opt_table) {
  for (OptionList::const_iterator it(opt_table.begin()); likely(it != opt_table.end()); ++it) {
...
**/
int ArgumentReader::lookup_longopt(const char** arg, coreq::TinyUnsigned* paramargs_remain, const char* long_opt, const OptionList& opt_table) {
  for (OptionList::const_iterator it(opt_table.begin()); likely(it != opt_table.end()); ++it) {
    ...
    if (unlikely(std::strncmp(optname, long_opt, len) == 0)) {
      ...
      coreq::TinyUnsigned num_args = numargs(it->type);
      switch (*next) {
        case '\0':
          *arg = NULLPTR;
        ...
        case '=':
          *arg = ++next;
          if (num_args == 0) {
            coreq::say_error(_("Option --%s must not have argument but has %s")) % optname % next;
  ...
  coreq::say_error(_("unknown option --%s")) % long_opt;
  std::exit(EXIT_FAILURE);
...
const Option* ArgumentReader::lookup_shortopt(const char short_opt, const OptionList& opt_table) {
  ...
  if (unlikely(option == NULLPTR)) {
    coreq::say_error(_("unknown option -%s")) % short_opt;
    std::exit(EXIT_FAILURE);
  }
  return option;
}
  ...
  while (it != end()) {
    ...
    switch (c->type) {
      ...
      case Option::PAIRLIST_OPTIONAL:
        ...
        if (it == end()) {
          if (!optional) {
            coreq::say_error(_("missing parameter to --%s")) % (c->longopt);
            std::exit(EXIT_FAILURE);
          }
        }
        else {
        ...
        if (it == end()) {
          if (!optional) {
            coreq::say_error(_("missing second parameter to --%s")) % (c->longopt);
            std::exit(EXIT_FAILURE);
          }
        }
        else {
