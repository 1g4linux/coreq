...
#ifndef SRC_COREQTK_STRINGUTILS_H_
#define SRC_COREQTK_STRINGUTILS_H_
...
// check_includes: include "coreqTk/stringutils.h"
...
**/
coreq::SignedBool natcmp(const std::string& a, const std::string& b);
...
**/
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(std::vector<T>* v, const T& e);
#ifdef HAVE_MOVE
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(std::vector<T>* v, T&& e);
#endif
...
**/
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(std::set<T>* s, const T& e);
#ifdef HAVE_MOVE
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(std::set<T>* s, T&& e);
#endif
...
**/
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(UNORDERED_SET<T>* s, const T& e);
#ifdef HAVE_MOVE
template <typename T>
ATTRIBUTE_NONNULL_ inline static void push_back(UNORDERED_SET<T>* s, T&& e);
#endif
...
**/
template <typename T>
inline static std::string join_to_string(T vec, const std::string& glue);
template <typename T>
inline static std::string join_to_string(T vec) {
...
inline static void join_and_split(Td vec, const Ts& s, const std::string& glue, bool handle_escape, const char* at, bool ignore_empty);
template <typename Td, typename Ts>
inline static void join_and_split(Td vec, const Ts& s) {
...
class StringHash : public WordVec {
 public:
  ...
  void store_words(const std::string& s) { store_words(split_string(s)); }

  void hash_string(const std::string& s);
  void hash_words(const WordVec& v);
  void hash_words(const std::string& s) { hash_words(split_string(s)); }
...
**/
template <typename T>
inline static void push_back(std::vector<T>* v, const T& e) {
  v->PUSH_BACK(e);
}
#ifdef HAVE_MOVE
template <typename T>
inline static void push_back(std::vector<T>* v, T&& e) {
  v->PUSH_BACK(MOVE(e));
}
#endif
...
**/
template <typename T>
inline static void push_back(std::set<T>* s, const T& e) {
  s->INSERT(e);
}
#ifdef HAVE_MOVE
template <typename T>
inline static void push_back(std::set<T>* s, T&& e) {
  s->INSERT(MOVE(e));
}
#endif
...
#ifdef HAVE_UNORDERED_SET
template <typename T>
inline static void push_back(UNORDERED_SET<T>* s, const T& e) {
  s->INSERT(e);
}
#ifdef HAVE_MOVE
template <typename T>
inline static void push_back(UNORDERED_SET<T>* s, T&& e) {
  s->INSERT(MOVE(e));
}
#endif
#endif  // HAVE_UNORDERED_SET
...
inline static void make_vector(std::vector<T>* the_list, const std::map<T, Y>& the_map) {
  the_list->clear();
  the_list->reserve(the_map.size());
  for (typename std::map<T, Y>::const_iterator it(the_map.begin()), it_end(the_map.end()); likely(it != it_end); ++it) {
...
**/
template <typename T, typename Y>
inline static void make_vector(std::vector<T>* the_list, const UNORDERED_MAP<T, Y>& the_map) {
  the_list->clear();
  the_list->reserve(the_map.size());
  for (typename UNORDERED_MAP<T, Y>::const_iterator it(the_map.begin()), it_end(the_map.end()); likely(it != it_end); ++it) {
    the_list->push_back(it->first);
  }
}
#endif  // HAVE_UNORDERED_MAP

#endif  // SRC_COREQTK_STRINGUTILS_H_
