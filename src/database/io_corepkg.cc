...
#define WRITE_COUNTER(f)                               \
  do {                                                 \
    coreq::OffsetType counter_save(counter);           \
    counter = 0;                                       \
    bool counting_save(counting);                      \
    counting = true;                                   \
    f;                                                 \
    counting = counting_save;                          \
    coreq::OffsetType counter_diff(counter);           \
...
bool Database::read_Part(BasicPart* b, string* errtext) {
  ...
  if (len != 0) {
    coreq::auto_array<char> buf(new char[len + 1]);
    buf.get()[len] = 0;
...
bool Database::write_version(const Version* v, const DBHeader& hdr, string* errtext) {
  ...
  for (BasicVersion::PartsType::const_iterator it(v->m_parts.begin()); likely(it != v->m_parts.end()); ++it) {
...
bool Database::write_depend(const Depend& dep, const DBHeader& hdr, string* errtext) {
  return (likely(write_hash_words(hdr.depend_hash, dep.m_depend, errtext)) && likely(write_hash_words(hdr.depend_hash, dep.m_rdepend, errtext)) &&
          likely(write_hash_words(hdr.depend_hash, dep.m_pdepend, errtext)) && likely(write_hash_words(hdr.depend_hash, dep.m_bdepend, errtext)) &&
          likely(write_hash_words(hdr.depend_hash, dep.m_idepend, errtext)));
}

bool Database::read_category_header(string* name, coreq::Treesize* h, string* errtext) {
  return (likely(read_string(name, errtext)) && likely(read_num(h, errtext)));
}

bool Database::write_category_header(const string& name, coreq::Treesize size, string* errtext) {
  return (likely(write_string(name, errtext)) && likely(write_num(size, errtext)));
}
...
bool Database::write_header(const DBHeader& hdr, string* errtext) {
  ...
  for (WordVec::const_iterator it(hdr.world_sets.begin()); likely(it != hdr.world_sets.end()); ++it) {
...
bool Database::write_packagetree(const PackageTree& tree, const DBHeader& hdr, string* errtext) {
  for (PackageTree::const_iterator c(tree.begin()); likely(c != tree.end()); ++c) {
    ...
    if (unlikely(!write_category_header(c->first, coreq::Treesize(ci->size()), errtext))) {
      return false;
    }
...
#if 0
bool Database::read_packagetree(PackageTree *tree, const DBHeader& hdr, CorePkgSettings *ps, string *errtext) {
