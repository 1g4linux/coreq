...
class File {
 private:
  FILE* fp;
  bool seek(coreq::OffsetType offset, int whence, std::string* errtext);
 ...
 public:
...
#ifdef HAVE_MOVE
  File(File&& s) NOEXCEPT : fp(s.fp) { s.fp = NULLPTR; }
  ...
  int getch() { return std::fgetc(fp); }

  bool putch(coreq::UChar c) { return (std::fputc(c, fp) != EOF); }
  ...
  bool seekrel(coreq::OffsetType offset, std::string* errtext) { return seek(offset, SEEK_CUR, errtext); }

  bool seekabs(coreq::OffsetType offset, std::string* errtext) { return seek(offset, SEEK_SET, errtext); }

  coreq::OffsetType tell();
...
class Database : public File {
 ...
 protected:
  bool readUChar(coreq::UChar* c, std::string* errtext);
  bool writeUChar(coreq::UChar c, std::string* errtext);
  ...
  **/
  ...
  ATTRIBUTE_NONNULL((2))
  bool read_num(m_Tp* ret, std::string* errtext);

  /**
  Write nonnegative number t to fp (undefined behaviour if t < 0)
  **/
  template <typename m_Tp>
  bool write_num(m_Tp t, std::string* errtext);
  ...
  bool write_hash_string(const StringHash& hash, const std::string& s, std::string* errtext) { return write_num(hash.get_index(s), errtext); }
  ...
  ATTRIBUTE_NONNULL((2, 3)) bool read_category_header(std::string* name, coreq::Treesize* h, std::string* errtext);
  bool write_category_header(const std::string& name, coreq::Treesize size, std::string* errtext);
...
#if 0
		ATTRIBUTE_NONNULL((2, 4)) bool read_packagetree(PackageTree *tree, const DBHeader& hdr, CorePkgSettings *ps, std::string *errtext);
#endif
};

template <typename m_Tp>
bool Database::read_num(m_Tp* ret, std::string* errtext) {
  int ch(getch());
  if (likely(ch != EOF)) {
    coreq::UChar c = static_cast<coreq::UChar>(ch);
    ...
    while (likely((ch = getch()) != EOF)) {
      if ((c = static_cast<coreq::UChar>(ch)) == MAGICNUMCHAR) {
        ++toget;
        continue;
      }
      if (c != 0) {
      ...
      for (;;) {
        ...
        *ret = ((*ret) << 8) | static_cast<m_Tp>(static_cast<coreq::UChar>(ch));
        --toget;
      }
      break;
    }
