...
class File {
	...
	public:
		...
		bool seekrel(coreq::OffsetType offset, std::string *errtext) {
			return seek(offset, SEEK_CUR, errtext);
		}

		bool seekabs(coreq::OffsetType offset, std::string *errtext) {
	...
	if(likely(ch != EOF)) {
		...
		while(likely((ch = getch()) != EOF)) {
			...
			for(;;) {
				...
				*ret = ((*ret) << 8) |
					static_cast<m_Tp>(static_cast<coreq::UChar>(ch));
				--toget;
			}
			break;
		}
