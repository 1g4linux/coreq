...
bool File::seek(coreq::OffsetType offset, int whence, string* errtext) {
#ifdef HAVE_FSEEKO
...
void File::readError(string* errtext) {
  if (errtext != NULLPTR) {
    *errtext = (feof(fp) ? _("error while reading from database: end of file") : _("error while reading from database"));
  }
}
...
bool Database::readUChar(coreq::UChar* c, string* errtext) {
  int ch(getch());
  if (likely(ch != EOF)) {
    *c = static_cast<coreq::UChar>(ch);
    return true;
  }
  readError(errtext);
  return false;
...
bool Database::writeUChar(coreq::UChar c, string* errtext) {
  if (counting) {
    ++counter;
  }
...
bool Database::write_string(const string& str, string* errtext) {
  return (likely(write_num(str.size(), errtext)) && likely(write_string_plain(str, errtext)));
}
