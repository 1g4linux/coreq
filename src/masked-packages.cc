...
static void print_help() {
  ...
  coreq::say(
      _("Usage: %s [options] category/name-version[:slot][::repo] ...\n"
...
static void read_stdin(LineVec* lines, string* name) {
  ...
  if (stdin_count++ == 0) {
    ...
    *name = coreq::format("(stdin%s)") % stdin_count;
  }
}
...
int run_masked_packages(int argc, char* argv[]) {
  ...
  for (WordVec::const_iterator it(args.begin()); likely(it != args.end()); ++it) {
    Mask m(Mask::maskPseudomask);
    string errtext;
    if (unlikely(m.parseMask(it->c_str(), &errtext) == BasicVersion::parsedError)) {
      coreq::say_error(_("warning: ignoring \"%s\": %s")) % (*it) % errtext;
      continue;
    }
    Package p;
    m.to_package(&p);
