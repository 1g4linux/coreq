...
#else
#define USE_BINARY run_coreq_update
#endif
#endif
...
#define BINARY_COLLECTION 1
#else
#define USE_BINARY run_coreq_diff
#endif
#endif
...
#define BINARY_COLLECTION 1
#else
#define USE_BINARY run_coreq_header
#endif
#endif
...
#define BINARY_COLLECTION 1
#else
#define USE_BINARY run_coreq_drop_permissions
#endif
#endif
...
static void sig_handler(int sig) {
  if (sig == SIGSEGV) {
    coreq::say_error(
        _("Received SIGSEGV - you probably found a bug in coreq.\n"
          "Please proceed with the following few instructions and help us find the bug:\n"
          " * install gdb (sys-devel/gdb)\n"
          " * reemerge coreq with FEATURES=\"nostrip\" CXXFLAGS=\"-g -ggdb3\" LDFLAGS=\"\"\n"
...
inline static int run_program(int argc, char* argv[]) {
  ...
  if (unlikely(program_lower.find("diff") != string::npos))
    return run_coreq_diff(argc, argv);
#endif
#ifdef UPDATE_BINARY
#if defined(COREQ_BINARY) || defined(HEADER_BINARY) || defined(VERSIONSORT_BINARY) || defined(MASKED_BINARY) || defined(DROP_PERMISSIONS_BINARY)
  if (unlikely(program_lower.find("update") != string::npos))
#endif
    return run_coreq_update(argc, argv);
#endif
#ifdef DROP_PERMISSIONS_BINARY
#if defined(COREQ_BINARY) || defined(HEADER_BINARY) || defined(VERSIONSORT_BINARY) || defined(MASKED_BINARY)
  if (likely((program_lower.find("drop") != string::npos) || (program_lower.find("perm") != string::npos)))
#endif
    return run_coreq_drop_permissions(argc, argv);
#endif
#ifdef MASKED_BINARY
#if defined(COREQ_BINARY) || defined(HEADER_BINARY) || defined(VERSIONSORT_BINARY)
  if (likely(program_lower.find("mask") != string::npos))
#endif
    return run_masked_packages(argc, argv);
#endif
#ifdef HEADER_BINARY
#if defined(COREQ_BINARY) || defined(VERSIONSORT_BINARY)
  if (likely(program_lower.find("header") != string::npos))
#endif
    return run_coreq_header(argc, argv);
#endif
#ifdef VERSIONSORT_BINARY
#if defined(COREQ_BINARY)
