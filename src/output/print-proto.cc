...
static void set_mask_flags(coreq_proto::MaskFlags* mask_flags, MaskFlags mask);
static void set_key_flags(coreq_proto::KeyFlags* key_flags, KeywordsFlags key);
static void add_restrictions(coreq_proto::Restrictions* restrictions, ExtendedVersion::Restrict restrict);
static void add_properties(coreq_proto::Properties* properties, ExtendedVersion::Restrict props);

void PrintProto::start() {
  collection = new coreq_proto::Collection();
...
void PrintProto::package(Package* pkg) {
  ...
  coreq_proto::Package* package = category->add_package();
  ...
  if ((likely(var_db_pkg != NULLPTR)) && var_db_pkg->isInstalled(*pkg)) {
    ...
    for (Package::reverse_iterator ver(pkg->rbegin()); likely(ver != pkg->rend()); ++ver) {
  ...
  for (Package::const_iterator ver(pkg->begin()); likely(ver != pkg->end()); ++ver) {
    coreq_proto::Version* version = package->add_version();
    version->set_id(ver->getFull());
    version->set_eapi(ver->eapi.get());

    coreq_proto::Repository* repository = version->mutable_repository();
    ...
    if (unlikely(ver->have_reasons())) {
      ...
      for (Version::Reasons::const_iterator it(reasons_ptr->begin()); unlikely(it != reasons_ptr->end()); ++it) {
        const WordVec* vec(it->asWordVecPtr());
        if ((vec == NULLPTR) || (vec->empty())) {
          continue;
        }
        coreq_proto::Lines* lines = version->add_mask_reason();
        for (WordVec::const_iterator wit(vec->begin()); likely(wit != vec->end()); ++wit) {
          lines->add_line(*wit);
        }
      }
    }
    ...
    coreq_proto::Installed* installed = NULLPTR;
...
static void set_mask_flags(coreq_proto::MaskFlags* mask_flags, MaskFlags mask) {
  if (mask.isPackageMask()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::MASK_PACKAGE);
  }
  if (mask.isSystem()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::MASK_SYSTEM);
  }
  if (mask.isProfileMask()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::MASK_PROFILE);
  }
  if (mask.isProfile()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::IN_PROFILE);
  }
  if (mask.isWorld()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::WORLD);
  }
  if (mask.isWorldSets()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::WORLD_SETS);
  }
  if (mask.isMarked()) {
    mask_flags->add_mask_flag(coreq_proto::MaskFlags::MARKED);
  }
}

static void set_key_flags(coreq_proto::KeyFlags* key_flags, KeywordsFlags key) {
  if (key.isStable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::STABLE);
  }
  if (key.isArchStable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::ARCHSTABLE);
  }
  if (key.isUnstable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::ARCHUNSTABLE);
  }
  if (key.isAlienStable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::ALIENSTABLE);
  }
  if (key.isAlienUnstable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::ALIENUNSTABLE);
  }
  if (key.isMinusKeyword()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::MINUSKEYWORD);
  }
  if (key.isMinusUnstable()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::MINUSUNSTABLE);
  }
  if (key.isMinusAsterisk()) {
    key_flags->add_key_flag(coreq_proto::KeyFlags::MINUSASTERISK);
  }
}

static void add_restrictions(coreq_proto::Restrictions* restrictions, ExtendedVersion::Restrict restrict) {
  if (unlikely((restrict & ExtendedVersion::RESTRICT_BINCHECKS) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::BINCHECKS);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_STRIP) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::STRIP);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_TEST) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::TEST);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_USERPRIV) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::USERPRIV);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_INSTALLSOURCES) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::INSTALLSOURCES);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_FETCH) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::FETCH);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_MIRROR) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::MIRROR);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_PRIMARYURI) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::PRIMARYURI);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_BINDIST) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::BINDIST);
  }
  if (unlikely((restrict & ExtendedVersion::RESTRICT_PARALLEL) != 0)) {
    restrictions->add_restrict(coreq_proto::Restrictions::PARALLEL);
  }
}

static void add_properties(coreq_proto::Properties* properties, ExtendedVersion::Restrict props) {
  if (unlikely((props & ExtendedVersion::PROPERTIES_INTERACTIVE) != 0)) {
    properties->add_property(coreq_proto::Properties::INTERACTIVE);
  }
  if (unlikely((props & ExtendedVersion::PROPERTIES_LIVE) != 0)) {
    properties->add_property(coreq_proto::Properties::LIVE);
  }
  if (unlikely((props & ExtendedVersion::PROPERTIES_VIRTUAL) != 0)) {
    properties->add_property(coreq_proto::Properties::VIRTUAL);
  }
  if (unlikely((props & ExtendedVersion::PROPERTIES_SET) != 0)) {
    properties->add_property(coreq_proto::Properties::SET);
  }
}
...
void PrintProto::package(Package*) {
  coreq::say_error(_("protobuf format is not compiled in"));
