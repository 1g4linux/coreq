...
LocalCopy::LocalCopy(const PrintFormat* fmt, Package* pkg) : PackageSave((fmt->recommend_mode) == LOCALMODE_DEFAULT ? NULLPTR : pkg) {
...
bool PrintFormat::parse_variable(Node** rootnode, const string& varname, string* errtext) const {
  ...
  if (f == varcache.end()) {
    ...
    bool ok(cache.init((*coreq_rc)[varname].c_str(), !no_color, true, errtext));
  ...
  if (unlikely(v.in_use)) {
    if (errtext != NULLPTR) {
      *errtext = coreq::format(_("variable %s calls itself for printing")) % varname;
    }
    return false;
  }
  v.in_use = true;
...
void PrintFormat::overlay_keytext(OutputString* s, ExtendedVersion::Overlay overlay, bool plain) const {
  ...
  if (number != 0) {
    ...
    if (overlay_translations != NULLPTR) {
      if ((number = (*overlay_translations)[index]) == 0) {
        for (OverlayTranslations::const_iterator it(overlay_translations->begin()); likely(it != overlay_translations->end()); ++it) {
          if (number < *it) {
            number = *it;
          }
        }
  ...
  string onum(coreq::format() % number);
  if (plain) {
    s->assign_fast(onum);
    return;
  }
...
static void parse_termdark(DarkModes* modes, WordVec* regexp, const string& termdark) {
  ...
  for (WordVec::iterator it(terms_dark.begin()); likely(it != terms_dark.end()); ++it) {
    string* text(&(*it));
    ...
    if (!darkmode.init(*text)) {
      coreq::say_error(_("DARK_TERM has illegal format: %s")) % termdark;
      std::exit(EXIT_FAILURE);
    }
...
void PrintFormat::setupResources(CoreqRc* rc) {
  coreq_rc = rc;
  ...
  if (schemes.size() > 0) {
    coreq::TinyUnsigned entry(0);
    if (schemes.size() > 1) {
      coreq::SignedBool dark(rc->getBoolText("DARK", "auto"));
      if (dark == 0) {
        entry = 1;
      }
      else if (dark < 0) {
...
FormatParser::ParserState FormatParser::state_IF() {
  ...
  for (char c(*band_position); c && (c != '}') && (c != '=') && !(my_isspace(c)); c = *(++band_position))
    ++i;
  if (!i) {
  ...
  /* This MUST be a '=' */
  if (*band_position != '=') {
    last_error = coreq::format(_("unexpected symbol '%s' after '{'")) % (*band_position);
    return ERROR;
  }
...
bool FormatParser::start(const char* fmt, bool colors, bool parse_only_colors, string* errtext) {
  ...
  while ((state != STOP) && (state != ERROR)) {
    switch (state) {
      ...
      case STOP:
        break;
      default:
        last_error = _("bad state: undefined");
        state = ERROR;
    }
  }
  ...
  if (state == ERROR) {
    ...
    if (errtext != NULLPTR) {
      ...
      *errtext = coreq::format(_("line %s, column %s: %s")) % line % column % last_error;
    }
    return false;
  }
...
bool PrintFormat::print(void* entity, GetProperty get_property, Node* root, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s, bool check_only) {
