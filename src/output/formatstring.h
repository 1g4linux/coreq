...
class FormatParser {
 private:
  enum ParserState { ERROR, STOP, START, TEXT, COLOR, PROPERTY, IF, ELSE, FI };
...
#if defined(COREQ_PARANOIC_ASSERT)
  ~FormatParser() { coreq_assert_paranoic(keller.empty()); }
#endif

  ATTRIBUTE_NONNULL((2)) bool start(const char* fmt, bool colors, bool parse_only_colors, std::string* errtext);

  Node* rootnode() { return root_node; }
...
class VarParserCacheNode {
 ...
 public:
  ...
  ~VarParserCacheNode() { delete root_node; }
...
class PrintFormat {
 ...
 protected:
  ...
  VarDbPkg* vardb;
  const CorePkgSettings* corepkgsettings;
 ...
 public:
  ...
  std::string color_overlaykey,  ///< Color for the overlay key
      color_virtualkey,          ///< Color for the virtual key
      color_keyend, color_overlayname, color_overlaynameend, color_numbertext, color_numbertextend, color_end;
  OutputString before_use_start, before_use_end, after_use, before_iuse_start, before_iuse_end, after_iuse, before_coll_start, before_coll_end, after_coll, before_set_use, after_set_use,
      before_unset_use, after_unset_use, maskreasons_skip, maskreasons_sep, maskreasonss_skip, maskreasonss_sep;

  void init(GetProperty get_callback);

  PrintFormat() { init(NULLPTR); }
  ...
  void set_as_virtual(const ExtendedVersion::Overlay overlay) { set_as_virtual(overlay, true); }
  ...
  void set_overlay_translations(OverlayTranslations* translations) { overlay_translations = translations; }
  ...
  void set_marked_list(MaskList<Mask>* m_list) { marked_list = m_list; }

  ATTRIBUTE_NONNULL_ void overlay_keytext(OutputString* s, ExtendedVersion::Overlay overlay, bool plain) const;
  ATTRIBUTE_NONNULL_ void overlay_keytext(OutputString* s, ExtendedVersion::Overlay overlay) const { overlay_keytext(s, overlay, false); }

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL((2, 5, 6, 7, 8))
  bool print(void* entity, GetProperty get_property, Node* root, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s, bool check_only);

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL((2, 5, 6, 7, 8)) bool print(void* entity, GetProperty get_property, Node* root, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s) {
    return print(entity, get_property, root, dbheader, vardbpkg, ps, s, false);
  }

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL((2, 4, 5, 6, 7)) bool print(void* entity, Node* root, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s, bool check_only) {
    return print(entity, m_get_property, root, dbheader, vardbpkg, ps, s, check_only);
  }

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL((2, 4, 5, 6, 7)) bool print(void* entity, Node* root, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s) {
    return print(entity, m_get_property, root, dbheader, vardbpkg, ps, s);
  }

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL_ bool print(void* entity, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s, bool check_only) {
    return print(entity, root_node, dbheader, vardbpkg, ps, s, check_only);
  }

  /* return true if something was actually printed */
  ATTRIBUTE_NONNULL_ bool print(void* entity, const DBHeader* dbheader, VarDbPkg* vardbpkg, const CorePkgSettings* ps, const SetStability* s) {
