...
class VersionVariables {
 ...
 public:
  ...
  void setinst(InstVersion* inst) { m_instver = inst; }

  void setversion(const Version* ver) { m_version = ver; }

  const Version* version() const { return m_version; }

  InstVersion* instver() const { return m_instver; }
};

void PrintFormat::iuse_expand(OutputString* s, const IUseSet& iuse, bool coll, HandleExpand expand) const {
  if (expand == EXPAND_NO) {
  ...
  for (IUseSet::IUseNaturalOrder::const_iterator it(iuse_ordered.begin()); it != iuse_ordered.end(); ++it) {
    string var, expval;
    const string& name(it->name());
    if (corepkgsettings->use_expand(&var, &expval, name)) {
...
void PrintFormat::get_inst_use(OutputString* s, const Package& package, InstVersion* i, HandleExpand expand) const {
  ...
  typedef vector<string> ExpVarNames;
  ExpVarNames expvarnames;
  for (WordVec::iterator it(i->inst_iuse.begin()); likely(it != i->inst_iuse.end()); ++it) {
    string* value(&(*it));
    ...
    if (expand != EXPAND_NO) {
      string var;
      if (corepkgsettings->use_expand(&var, &expval, *value)) {
        if (expand == EXPAND_OMIT) {
          continue;
        }
        value = &expval;
...
void PrintFormat::get_installed(Package* package, Node* root) const {
  coreq_assert_paranoic(vardb != NULLPTR);
  InstVec* vec(vardb->getInstalledVector(*package));
  if (vec == NULLPTR) {
    return;
  }
  bool have_prevversion(false);
  for (InstVec::iterator it(vec->begin()); likely(it != vec->end()); ++it) {
    if (have_prevversion) {
...
void PrintFormat::get_versions_versorted(Package* package, Node* root, PrintFormat::VerVec* versions) const {
  bool have_prevversion(false);
  for (Package::const_iterator vit(package->begin()); likely(vit != package->end()); ++vit) {
...
void PrintFormat::get_versions_slotsorted(Package* package, Node* root, PrintFormat::VerVec* versions) const {
  ...
  if (unlikely(versions != NULLPTR)) {
    for (SlotList::const_iterator it(sl->begin()); likely(it != sl->end()); ++it) {
      const VersionList* vl(&(it->const_version_list()));
      for (VersionList::const_iterator vit(vl->begin()); likely(vit != vl->end()); ++vit) {
...
bool PrintFormat::have_virtual(const Package* p, bool nonvirtual) const {
  for (Package::const_iterator vit(p->begin()); likely(vit != p->end()); ++vit) {
...
class Scanner {
 public:
  enum Diff { DIFF_NONE, DIFF_BETTER, DIFF_BESTBETTER, DIFF_WORSE, DIFF_BESTWORSE, DIFF_DIFFER, DIFF_BESTDIFFER };
  enum Prop { PKG, VER };
 ...
 protected:
  ...
  ATTRIBUTE_NONNULL_ void prop_diff(const char* s, Diff diffprop) { diff[s] = diffprop; }

  ATTRIBUTE_NONNULL_ void prop_colon_pkg(const char* s, ColonVar colfunc) { colon_var[s] = colfunc; }

  ATTRIBUTE_NONNULL_ void prop_colon_ver(const char* s, ColonOther colfunc) { colon_other[s] = colfunc; }

  ATTRIBUTE_NONNULL_ void prop_pkg(const char* s, Plain plainfunc) { plain[s] = pair<Plain, Prop>(plainfunc, PKG); }

  ATTRIBUTE_NONNULL_ void prop_ver(const char* s, Plain plainfunc) { plain[s] = pair<Plain, Prop>(plainfunc, VER); }
...
void PrintFormat::init_static() {
  coreq_assert_static(scanner == NULLPTR);
  scanner = new Scanner;
...
void PrintFormat::get_pkg_property(OutputString* s, Package* package, const string& name) const {
  ...
  if (plain == NULLPTR) {
    ...
    if (unlikely(col == string::npos)) {
      coreq::say_error(_("unknown property \"%s\"")) % name;
      std::exit(EXIT_FAILURE);
    }
    after_colon.assign(name, col + 1, string::npos);
  }
  if (unlikely((t == Scanner::VER) && (version_variables == NULLPTR))) {
    coreq::say_error(_("property \"%s\" used outside version context")) % name;
    std::exit(EXIT_FAILURE);
  }
  if (plain != NULLPTR) {
...
void PrintFormat::COLON_VER_DATE(OutputString* s, Package* package, const string& after_colon) const {
  if (version_variables->isinst) {
    ...
    s->assign_smart(date_conv((*coreq_rc)[after_colon].c_str(), i->instDate));
    return;
  }
}
...
void PrintFormat::colon_pkg_availableversions(Package* package, const string& after_colon, bool only_marked) const {
  ...
  if (unlikely(only_marked)) {
    if (unlikely(marked_list == NULLPTR) || likely(!marked_list->applyMasks(package))) {
      return;
    }
...
void PrintFormat::colon_pkg_bestslotupgradeversions(Package* package, const string& after_colon, bool allow_unstable) const {
  VerVec versions;
  package->best_slots_upgrade(&versions, vardb, corepkgsettings, allow_unstable);
  if (!versions.empty()) {
    get_versions_versorted(package, parse_variable(after_colon), &versions);
    varcache[after_colon].in_use = false;
  }
...
void PrintFormat::PKG_BINARY(OutputString* s, Package* package) const {
  for (Package::const_iterator it(package->begin()); likely(it != package->end()); ++it) {
    if (it->have_bin_pkg(corepkgsettings, package)) {
      s->set_one();
      return;
    }
  }
...
void PrintFormat::PKG_SETNAMES(OutputString* s, Package* package) const {
  s->assign_smart(corepkgsettings->get_setnames(package));
}

void PrintFormat::PKG_ALLSETNAMES(OutputString* s, Package* package) const {
  s->assign_smart(corepkgsettings->get_setnames(package, true));
}
...
void PrintFormat::pkg_upgrade(OutputString* s, Package* package, bool only_installed, bool test_slots) const {
  ...
  if (package->can_upgrade(vardb, corepkgsettings, only_installed, test_slots)) {
    s->set_one();
  }
...
void PrintFormat::pkg_recommend(OutputString* s, Package* package, bool only_installed, bool test_slots) const {
  LocalCopy locallocalcopy(this, package);
  if (package->recommend(vardb, corepkgsettings, only_installed, test_slots)) {
    s->set_one();
  }
  locallocalcopy.restore(package);
}
...
void PrintFormat::PKG_MARKED(OutputString* s, Package* package) const {
  if (unlikely(marked_list != NULLPTR) && unlikely(marked_list->applyMasks(package))) {
    s->set_one();
  }
}

void PrintFormat::PKG_HAVEMARKEDVERSION(OutputString* s, Package* package) const {
  if (unlikely(marked_list != NULLPTR) && unlikely(marked_list->MaskMatches(package))) {
    s->set_one();
  }
}
...
void PrintFormat::ver_overlay(OutputString* s, Package* package, bool numeric, bool only_noncommon, bool only_nonzero) const {
  coreq_assert_paranoic(!numeric || only_nonzero);
  ExtendedVersion::Overlay ov_key;
  if (version_variables->isinst) {
    InstVersion* i(version_variables->instver());
    coreq_assert_paranoic(header != NULLPTR);
    ...
    if (likely(only_noncommon) && likely(package->have_same_overlay_key()) && likely(ov_key == package->largest_overlay)) {
      return;
    }
  }
  else if (likely(only_noncommon) && likely(package->have_same_overlay_key())) {
    return;
  }
  else {
...
void PrintFormat::VER_VERSIONKEYWORDSS(OutputString* s, Package* package) const {
  ...
  corepkgsettings->get_effective_keywords_userprofile(package);
...
void PrintFormat::ver_isbestupgrade(OutputString* s, Package* package, bool check_slots, bool allow_unstable) const {
  if (likely(!version_variables->isinst) && unlikely(package->is_best_upgrade(check_slots, version_variables->version(), vardb, corepkgsettings, allow_unstable))) {
    s->set_one();
  }
}
...
void PrintFormat::VER_ISBINARY(OutputString* s, Package* package) const {
  if (ver_version()->have_bin_pkg(corepkgsettings, package)) {
    s->set_one();
  }
}

void PrintFormat::VER_ISTBZ(OutputString* s, Package* package) const {
  if (ver_version()->have_tbz_pkg(corepkgsettings, package)) {
    s->set_one();
  }
}

void PrintFormat::VER_ISGPKG(OutputString* s, Package* package) const {
  if (ver_version()->num_gpkg_pkg(corepkgsettings, package) != 0) {
    s->set_one();
  }
}

void PrintFormat::VER_ISPAK(OutputString* s, Package* package) const {
  if (ver_version()->num_pak_pkg(corepkgsettings, package) != 0) {
    s->set_one();
  }
}

void PrintFormat::VER_ISMULTIGPKG(OutputString* s, Package* package) const {
  if (ver_version()->num_gpkg_pkg(corepkgsettings, package) > 1) {
    s->set_one();
  }
}

void PrintFormat::VER_ISMULTIPAK(OutputString* s, Package* package) const {
  if (ver_version()->num_pak_pkg(corepkgsettings, package) > 1) {
    s->set_one();
  }
}

void PrintFormat::VER_GPKGCOUNT(OutputString* s, Package* package) const {
  ExtendedVersion::CountBinPkg count(ver_version()->num_gpkg_pkg(corepkgsettings, package));
  if (count != 0) {
    s->assign_fast(coreq::format() % count);
  }
}

void PrintFormat::VER_PAKCOUNT(OutputString* s, Package* package) const {
  ExtendedVersion::CountBinPkg count(ver_version()->num_pak_pkg(corepkgsettings, package));
  if (count != 0) {
    s->assign_fast(coreq::format() % count);
  }
}
