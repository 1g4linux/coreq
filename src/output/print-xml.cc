...
void PrintXml::clear(CoreqRc* coreqrc) {
  if (unlikely(coreqrc == NULLPTR)) {
    ...
    static CONSTEXPR const char* values[] = {"none", "both", "effective*", "effective", "full*", "full", NULLPTR};
    switch (coreqrc->getTinyTextlist("XML_KEYWORDS", values)) {
      ...
      case -1:
        ...
        break;
      case -2:
        keywords_mode = KW_BOTH;
        break;
      case -3:
        keywords_mode = KW_EFFS;
        break;
      case -4:
        keywords_mode = KW_EFF;
        break;
      case -5:
        keywords_mode = KW_FULLS;
        break;
      default:
        keywords_mode = KW_FULL;
        break;
    }
  }
  runclear();
...
void PrintXml::package(Package* pkg) {
  ...
  coreq::say("\t\t<package name=\"%s\">") % escape_xmlstring(true, pkg->name);
  ...
  for (Package::const_iterator ver(pkg->begin()); likely(ver != pkg->end()); ++ver) {
    ...
    coreq::print("\t\t\t<version id=\"%s\" EAPI=\"%s\"") % escape_xmlstring(true, ver->getFull()) % escape_xmlstring(true, ver->eapi.get());
    ...
    if (unlikely(overlay_key != 0)) {
      ...
      if ((print_overlay || overlay.label.empty()) && !(overlay.path.empty())) {
        coreq::print(" overlay=\"%s\"") % escape_xmlstring(true, overlay.path);
      }
      if (!overlay.label.empty()) {
        coreq::print(" repository=\"%s\"") % escape_xmlstring(true, overlay.label);
      }
    }
    if (!ver->get_shortfullslot().empty()) {
      coreq::print(" slot=\"%s\"") % escape_xmlstring(true, ver->get_longfullslot());
    }
    if (!ver->src_uri.empty()) {
      coreq::print(" srcURI=\"%s\"") % escape_xmlstring(true, ver->src_uri);
    }
    if (versionInstalled) {
      coreq::print(" installed=\"1\" installDate=\"%s\" installEAPI=\"%s\"") % escape_xmlstring(true, date_conv(dateformat.c_str(), installedVersion->instDate)) %
    ...
    for (WordVec::const_iterator it(mask_text.begin()); unlikely(it != mask_text.end()); ++it) {
    ...
    for (WordVec::const_iterator it(unmask_text.begin()); unlikely(it != unmask_text.end()); ++it) {
    ...
    if (Version::use_required_use) {
      ...
      if (!(required_use.empty())) {
        coreq::say("\t\t\t\t<required_use>%s</required_use>") % escape_xmlstring(false, required_use);
      }
    }
    if (versionInstalled) {
      ...
      if (!iuse_disabled.empty()) {
        coreq::say("\t\t\t\t<use enabled=\"0\">%s</use>") % escape_xmlstring(false, iuse_disabled);
      }
      if (!iuse_enabled.empty()) {
        coreq::say("\t\t\t\t<use enabled=\"1\">%s</use>") % escape_xmlstring(false, iuse_enabled);
      }
    }
    ...
    if (Depend::use_depend) {
      ...
      if (!depend.empty()) {
        coreq::say("\t\t\t\t<depend>%s</depend>") % escape_xmlstring(false, depend);
      }
      const string& rdepend = ver->depend.get_rdepend();
      if (!rdepend.empty()) {
        coreq::say("\t\t\t\t<rdepend>%s</rdepend>") % escape_xmlstring(false, rdepend);
      }
      const string& pdepend = ver->depend.get_pdepend();
      if (!pdepend.empty()) {
        coreq::say("\t\t\t\t<pdepend>%s</pdepend>") % escape_xmlstring(false, pdepend);
      }
      const string& bdepend = ver->depend.get_bdepend();
      if (!bdepend.empty()) {
        coreq::say("\t\t\t\t<bdepend>%s</bdepend>") % escape_xmlstring(false, bdepend);
      }
      const string& idepend = ver->depend.get_idepend();
      if (!idepend.empty()) {
        coreq::say("\t\t\t\t<idepend>%s</idepend>") % escape_xmlstring(false, idepend);
      }
    }
    coreq::say("\t\t\t</version>");
  }
...
void PrintXml::say_xml_element(const string& prefix, const string& name, const string& content) {
  ...
  coreq::say("%s<%s>%s</%2$s>") % prefix % name % escape_xmlstring(false, content);
}
