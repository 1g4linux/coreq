...
static bool drop_permissions(CoreqRc *coreq);

bool drop_permissions(CoreqRc *coreq, string *errtext) {
	if(drop_permissions(coreq)) {
		errtext->clear();
		return true;
	}
	if(coreq->getBool("NODROP_FATAL")) {
...
static bool drop_permissions(CoreqRc *coreq) {
	...
	uid_t uid;
	const string& user((*coreq)["COREQ_USER"]);
	if(user.empty() || !get_uid_of(user.c_str(), &uid)) {
		valid_user = false;
		uid_t i(coreq->getInteger("COREQ_UID"));
		if(i > 0) {
			uid = i;
		} else {
			set_uid = false;
	...
	bool set_gid(true);
	gid_t gid;
	const string& group((*coreq)["COREQ_GROUP"]);
	if(group.empty() || !get_gid_of(group.c_str(), &gid)) {
		gid_t i(coreq->getInteger("COREQ_GID"));
		if(i > 0) {
			gid = i;
		} else {
			set_gid = false;
	...
	bool success(true);
	bool force_success; {
		coreq::SignedBool w(coreq->getBoolText("REQUIRE_DROP", "root"));
		force_success = ((w < 0) ?
#ifdef HAVE_GETUID
			(getuid() != 0)
#else
