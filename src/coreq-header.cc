...
typedef coreq::TinyUnsigned PrintOverlayMode;
static CONSTEXPR const PrintOverlayMode PRINT_OVERLAY_NONE = 0x00, PRINT_OVERLAY_LABEL = 0x01, PRINT_OVERLAY_PATH = 0x02, PRINT_OVERLAY_FORMAT = 0x04,
...
static void print_help() {
  coreq::say(
      ...
      _("Usage: %s [-q] [-f FILE] [-s SEP] [-c|-C] [-l OV] [-p OV] [-o OV] ...\n"
        "Check whether coreq database FILE has current format, and print label, path,\n"
        ...
        "FILE defaults to %s\n"
        "Note that this default for FILE is independent of the COREQ_CACHEFILE variable,\n"
        "i.e. for usage in portable scripts you should always specify -f as one of the\n"
        "first options, e.g. as in this script snippet:\n"
        "\teval \"$(coreq-functions.sh)\"; ReadFunctions\n"
        "\tReadVar COREQ_CACHEFILE || die 'cache name not known'\n"
        "\t%s -qf \"$COREQ_CACHEFILE\" -c || die 'no current database'\n"
        ...
        "further information.")) %
      program_name % COREQ_CACHEFILE % program_name;
}
...
class OverlayOption {
 public:
  ...
  OverlayOption(char option, const char* argument) NOEXCEPT : opt(option), arg(argument) {}
};
...
int overlay_loop(string* result, const OverlayOptionList& options) {
  ...
  const char* name(COREQ_CACHEFILE);
  DBHeader* header = NULLPTR;
  int ret(EXIT_SUCCESS);
  bool changedname(true);
  for (OverlayOptionList::const_iterator it(options.begin()); likely(it != options.end()); ++it) {
    ...
    if (unlikely((mode & PRINT_OVERLAY_FORMAT) != PRINT_OVERLAY_NONE)) {
      if (likely(name != NULLPTR)) {
        result->append(coreq::format("%s") % header->version);
      }
...
bool open_database(DBHeader* header, const char* name, bool verbose) {
  ...
  if (likely((name[0] != '\0') && db.openread(name))) {
    ...
    if (likely(verbose)) {
      coreq::say_error(
          _("%s was created with an incompatible coreq-update:\n"
    ...
    if (likely(verbose)) {
      coreq::say_error(_("cannot open database file %s for reading (mode = 'rb')")) % name;
    }
  }
  return false;
}
...
bool print_overlay_data(string* result, const DBHeader* header, const char* overlay, const string& print_append, const char* name, PrintOverlayMode mode, bool verbose) {
  ...
  bool found(header->find_overlay(&num, emptysearch ? "0" : overlay, NULLPTR, 0, emptysearch ? DBHeader::OVTEST_NUMBER : DBHeader::OVTEST_ALL));
  if (unlikely((!found) && !emptysearch)) {
    if (likely(verbose)) {
      coreq::say_error(_("cannot find overlay %s in %s")) % overlay % name;
    }
    return false;
  }
  if (unlikely(!verbose)) {
  ...
  const OverlayIdent* ov(found ? &header->getOverlay(num) : new OverlayIdent(""));
