...
bool ParseCache::initialize(const string& name) {
  ...
  corepkg3_0_24 = coreqrc.getBool("EBUILD_COREPKG3_0_24");
  ebuild_type = NULLPTR;
  ...
  if (try_ebuild) {
    if (use_sh) {
      ebuild_sh = true;
      ebuild_type = (corepkg3_0_24 ? "ebuild*3.0.24" : "ebuild*legacy");
    }
    else {
      ebuild_sh = corepkg3_0_24;
      ebuild_type = (corepkg3_0_24 ? "ebuild-3.0.24" : "ebuild-legacy");
    }
...
const char* ParseCache::getType() const {
  ...
  for (FurtherCaches::const_iterator it(further.begin()); likely(it != further.end()); ++it) {
    s->append(1, '#');
    s->append((*it)->getType());
  }
  return s->c_str();
...
ParseCache::~ParseCache() {
  for (FurtherCaches::iterator it(further.begin()); likely(it != further.end()); ++it) {
    delete *it;
  }
  if (ebuild_exec != NULLPTR) {
...
void ParseCache::set_checking(string* str, const char* item, const VarsReader& ebuild, bool* ok) {
  ...
  if (unlikely((str->find('`') != string::npos) || (str->find("$(") != string::npos))) {
    *ok = false;
  }
}
...
void ParseCache::parse_exec(const char* fullpath, const string& dirpath, bool read_onetime_info, bool* have_onetime_info, Package* pkg, Version* version) {
  ...
  if (ok || ebuild_sh) {
    ...
    if (!ebuild.read(fullpath, &errtext, false)) {
      m_error_callback(coreq::format(_("cannot properly parse %s: %s")) % fullpath % errtext);
    }
  ...
  if (verbose) {
    ...
    m_error_callback(coreq::format("%s/%s-%s: %s") % m_catname % pkg->name % version->getFull() % used_type);
  }
  if (!ok) {
    ...
    if (likely(cachefile != NULLPTR)) {
      ...
      m_error_callback(coreq::format(_("cannot properly execute %s")) % fullpath);
    }
  }
...
void ParseCache::readPackage(Category* cat, const string& pkg_name, const string& directory_path, const WordVec& files) {
  ...
  for (WordVec::const_iterator fileit(files.begin()); likely(fileit != files.end()); ++fileit) {
    ...
    if (unlikely(!ExplodeAtom::split_version(&curr_version, fileit->substr(0, pos).c_str()))) {
      m_error_callback(coreq::format(_("cannot split filename of ebuild %s/%s")) % directory_path % (*fileit));
      continue;
    }
    ...
    if (it == further.end()) {
      ...
      if (verbose) {
        m_error_callback(coreq::format("%s/%s-%s: %s") % m_catname % pkg_name % version->getFull() % (*it)->getType());
      }
...
bool ParseCache::readCategory(Category* cat) {
  for (WordVec::const_iterator pit(m_packages.begin()); likely(pit != m_packages.end()); ++pit) {
