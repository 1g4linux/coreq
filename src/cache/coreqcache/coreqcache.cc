...
CoreqCache::~CoreqCache() {
	if(all_coreqcaches != NULLPTR) {
		CachesList::iterator it(std::find(all_coreqcaches->begin(),
			all_coreqcaches->end(), this));
...
bool CoreqCache::get_overlaydat(const DBHeader& header) {
	...
	if(m_overlay == "~") {
		...
		if(!found) {
			thiserror(coreq::format(_("cache file %s does not contain overlay \"%s\" [%s]")) %
				m_overlay_name % m_scheme);
			return false;
		}
	} else if(!header.find_overlay(&m_get_overlay, m_overlay.c_str(), portdir, 0, DBHeader::OVTEST_ALL)) {
		thiserror(coreq::format(_("cache file %s does not contain overlay %s")) %
			m_full % m_overlay);
		return false;
	}
	return true;
...
bool CoreqCache::readCategories(PackageTree *packagetree, const char *cat_name, Category *category) {
	...
	CachesList slaves;
	if(all_coreqcaches != NULLPTR) {
		for(CachesList::const_iterator sl(all_coreqcaches->begin());
			unlikely(sl != all_coreqcaches->end()); ++sl) {
			if(sl->m_full == m_full) {
				if(*sl != this) {
					sl->slavemode = true;
				}
	...
	if(unlikely(!db.openread(m_full.c_str()))) {
		allerrors(slaves, coreq::format(_("cannot read cache file %s: %s")) %
