...
/* Path to corepkg cache */
#define COREPKG_CACHE_PATH "/var/cache/edb/dep"
...
int sqlite_callback(void* /* NotUsed */, int argc, char** argv, char** azColName) {
  ...
  for (int i(0); likely(i < argc); ++i) {
    coreq::say("%s: %s = %s") % i % azColName[i] % welldefine(argv[i]);
  }
#else
...
corepkg versions use different names, we still have (hopefully correct)
default values.
...
class TrueIndex : public TrueIndexMapper {
 public:
  typedef enum { NAME, SLOT, RESTRICT, HOMEPAGE, LICENSE, DESCRIPTION, EAPI, KEYWORDS, IUSE, REQUIRED_USE, PROPERTIES, DEPEND, RDEPEND, PDEPEND, BDEPEND, IDEPEND, SRC_URI, LAST } Names;
 ...
 public:
  TrueIndex() : default_trueindex(LAST, -1) {
    ...
    mapinit(1, NAME, "corepkg_package_key");
...
void SqliteCache::sqlite_callback_cpp(int argc, const char* const* argv, const char* const* azColName) {
  ...
  if (argc <= maxindex) {
    sqlite_callback_error = true;
    m_error_callback(coreq::format(_("sqlite dataset for %s is too small")) % catarg);
    return;
  }
  string::size_type pos(catarg.find('/'));
  if (pos == string::npos) {
    sqlite_callback_error = true;
    m_error_callback(coreq::format(_("\"%s\" not of the form package/category-version")) % catarg);
    return;
  }
  ...
  if (unlikely(!ExplodeAtom::split(&curr_name, &curr_version, name_ver.c_str()))) {
    m_error_callback(coreq::format(_("cannot split \"%s\" into package and version")) % name_ver);
    return;
  }
  ...
  if (unlikely(r == BasicVersion::parsedError)) {
    ...
    version->depend.set(TrueIndex::c_str(argv, &trueindex, TrueIndex::DEPEND), TrueIndex::c_str(argv, &trueindex, TrueIndex::RDEPEND), TrueIndex::c_str(argv, &trueindex, TrueIndex::PDEPEND),
                        TrueIndex::c_str(argv, &trueindex, TrueIndex::BDEPEND), TrueIndex::c_str(argv, &trueindex, TrueIndex::IDEPEND), false);
...
bool SqliteCache::readCategories(PackageTree* pkgtree, const char* catname, Category* cat) {
  ...
  string sqlitefile(m_prefix + COREPKG_CACHE_PATH + m_scheme);
  ...
  if (sqlite3_open(sqlitefile.c_str(), &db) != SQLITE_OK) {
    sqlite3_close(db);
    m_error_callback(coreq::format(_("cannot open cache file %s")) % sqlitefile);
    return false;
  }
  callback_arg = this;
  ...
  category = cat;
  cat_name = catname;
  int rc(sqlite3_exec(db, "select * from corepkg_packages", sqlite_callback, NULLPTR, &errormessage));
  sqlite3_close(db);
  trueindex.clear();
  if (rc != SQLITE_OK) {
    sqlite_callback_error = true;
    m_error_callback(coreq::format(_("sqlite error: %s")) % errormessage);
  }
...
bool SqliteCache::readCategories(PackageTree* /* pkgtree */, const char* /* catname */, Category* /* cat */) {
  m_error_callback(
      _("cache method sqlite is not compiled in.\n"
        "Recompile coreq, using configure option --with-sqlite to add sqlite support"));
  return false;
}
