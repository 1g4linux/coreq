...
bool FlatReader::skip_lines(const coreq::TinyUnsigned nr, ifstream* is, const string& filename) const {
  for (coreq::TinyUnsigned i(nr); likely(i != 0); --i) {
    ...
    if (is->fail()) {
      m_cache->m_error_callback(coreq::format(_("cannot read cache file %s: %s")) % filename % std::strerror(errno));
      return false;
    }
  }
  return true;
...
**/
void FlatReader::get_keywords_slot_iuse_restrict(const string& filename,
                                                 string* eapi,
                                                 string* keywords,
                                                 string* slotname,
                                                 string* iuse,
  ...
  if (!is.is_open()) {
    m_cache->m_error_callback(coreq::format(_("cannot open %s: %s")) % filename % std::strerror(errno));
  }
...
void FlatReader::read_file(const string& filename, Package* pkg) {
  ...
  for (coreq::TinyUnsigned linenr(5); is.good(); ++linenr) {
    ...
    switch (linenr) {
      case 5:
        pkg->homepage = linebuf;
        break;
      case 6:
        pkg->licenses = linebuf;
        break;
      case 7:
        pkg->desc = linebuf;
        is.close();
