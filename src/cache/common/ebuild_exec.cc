...
class EbuildExecSettings {
 ...
 private:
  ...
  string corepkg_rootpath;
  string corepkg_bin_path, corepkg_pym_path, exec_ebuild_sh;
  bool read_corepkg_paths, know_corepkg_paths;
...
void EbuildExec::delete_cachefile() {
  ...
  if (is_pure_file(c)) {
    if (unlink(c) < 0) {
      base->m_error_callback(coreq::format(_("cannot unlink tempfile %s")) % c);
    }
    else if (is_file(c)) {
      base->m_error_callback(coreq::format(_("tempfile %s still there after unlink")) % c);
    }
  }
  else {
    base->m_error_callback(coreq::format(_("tempfile %s is not a file")) % c);
  }
  remove_handler();
  cache_defined = false;
  cachefile.clear();
...
void EbuildExec::calc_environment(const char* name, const string& dir, const Package& package, const Version& version, const string& eapi, int fd) {
...
#ifndef HAVE_SETENV
    if (!(base->corepkgsettings->export_portdir_overlay))
#endif
      return;
  }
  WordIterateMap env;
  ...
  {  // NOLINT(whitespace/braces)
    ...
    env["dbkey"] = cachefile;
    const string& corepkg_rootpath(settings->corepkg_rootpath);
    if (likely(!corepkg_rootpath.empty())) {
      env["COREPKG_ROOTPATH"] = corepkg_rootpath;
    }
    env["EAPI"] = eapi;
    env["COREPKG_BIN_PATH"] = settings->corepkg_bin_path;
    env["COREPKG_PYM_PATH"] = settings->corepkg_pym_path;
    env["COREPKG_REPO_NAME"] = base->getOverlayName();
    if (fd != -1) {
      env["COREPKG_PIPE_FD"] = coreq::format("%d") % fd;
    }
    WordVec eclasses;
    eclasses.PUSH_BACK(base->getPrefixedPath());
    RepoList& repos(base->corepkgsettings->repos);
    // eclasses.PUSH_BACK((*(base->corepkgsettings))["PORTDIR"]);
    // for(RepoList::const_iterator it(repos.second());
    for (RepoList::const_iterator it(repos.begin()); likely(it != repos.end()); ++it) {
      if (likely(it->path != eclasses[0])) {
        eclasses.PUSH_BACK(it->path);
      }
    }
    for (WordVec::iterator it(eclasses.begin()); likely(it != eclasses.end()); ++it) {
      escape_string(&*it, shellspecial);
    }
    join_to_string(&env["COREPKG_ECLASS_LOCATIONS"], eclasses);
  }
  env["PORTDIR_OVERLAY"] = (*(base->corepkgsettings))["PORTDIR_OVERLAY"].c_str();
...
string* EbuildExec::make_cachefile(const char* name, const string& dir, const Package& package, const Version& version, const string& eapi) {
  ...
  if (unlikely(got_exit_signal)) {
    ...
    base->m_error_callback(coreq::format(_("ebuild got signal %s")) % type_of_exit_signal);
  }
  if (unlikely(got_exit_signal)) {
    delete_cachefile();
  ...
  if (likely(WIFEXITED(exec_status))) {
    ...
    if ((WEXITSTATUS(exec_status)) == EXECLE_FAILED) {
      base->m_error_callback(coreq::format(_("could not start %s")) % exec_name);
    }
    else {
      base->m_error_callback(coreq::format(_("ebuild failed with status %s")) % WEXITSTATUS(exec_status));
    }
  }
  else {
...
bool EbuildExec::corepkgq(std::string* result, const char* var) const {
  int fds[2];
  ...
  if (child == 0) {
    ...
    dup2(fds[1], 1);
    close(fds[1]);
    execlp("corepkgq", "corepkgq", "envvar", var, static_cast<const char*>(NULLPTR));
    _exit(EXECLE_FAILED);
  }
  close(fds[1]);
  ...
  if ((ret_status != 0) || res.empty()) {
    base->m_error_callback(_("corepkgq failed"));
    return false;
  }
...
void EbuildExecSettings::init() {
  CoreqRc& coreq(get_coreqrc());
  ebuild_depend_temp = coreq["EBUILD_DEPEND_TEMP"];
  tmpdir = coreq["COREQ_TMPDIR"];
#ifndef HAVE_SETENV
  exec_ebuild = coreq["EPREFIX_COREPKG_EXEC"];
  exec_ebuild.append("/usr/bin/ebuild");
#endif
  exec_ebuild_sh = "ebuild.sh";
  corepkg_rootpath = coreq["COREPKG_ROOTPATH"];
  read_corepkg_paths = false;
}

bool EbuildExecSettings::init_ebuild_sh(const EbuildExec* e) {
  if (likely(read_corepkg_paths)) {
    return know_corepkg_paths;
  }
  read_corepkg_paths = true;
  if (unlikely(!e->corepkgq(&corepkg_bin_path, "COREPKG_BIN_PATH")) || unlikely(!e->corepkgq(&corepkg_pym_path, "COREPKG_PYM_PATH"))) {
    know_corepkg_paths = false;
    return false;
  }
  exec_ebuild_sh = corepkg_bin_path;
  exec_ebuild_sh.append("/ebuild.sh");
  know_corepkg_paths = true;
  return true;
}
