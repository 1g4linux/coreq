...
bool MetadataCache::initialize(const string& name) {
  ...
  if (casecontains(pure_name, "metadata")) {
    ...
    if (casecontains(pure_name, "md5")) {
      ...
      setType(PATH_METADATA, !(s_assign || (pure_name.find('*') != string::npos)));
    }
    return true;
  }
  ...
  if (caseequal(pure_name, "flat") || casecontains(pure_name, "corepkg-2.0")) {
    setType(PATH_FULL, true);
    return true;
  }
  if (caseequal(pure_name, "assign") || casecontains(pure_name, "backport") || casecontains(pure_name, "corepkg-2.1")) {
...
static int cachefiles_selector(SCANDIR_ARG3 dent) {
  return ((dent->d_name[0] != '.') && (std::strchr(dent->d_name, '-') != NULLPTR));
}
  ...
  if (have_override_path) {
    ...
    switch (path_type) {
      ...
      default:
        m_catpath = m_prefix;
        optional_append(&m_catpath, '/');
        m_catpath.append(COREPKG_CACHE_PATH);
        break;
    }
  }
  switch (path_type) {
...
bool MetadataCache::readCategory(Category* cat) {
  for (WordVec::const_iterator it(names.begin()); likely(it != names.end());) {
    ...
    if (unlikely(!ExplodeAtom::split(&curr_name, &curr_version, it->c_str()))) {
      m_error_callback(coreq::format(_("cannot split \"%s\" into package and version")) % (*it));
      ++it;
      continue;
    }
