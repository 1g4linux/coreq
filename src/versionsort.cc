...
static void failparse(const string& v) {
  coreq::say_error(_("cannot determine version of \"%s\"")) % v;
...
int run_versionsort(int argc, char* argv[]) {
    ...
    if (mode) {
      ...
      for (int curr(2);;) {
        ...
        if (mode > 0) {
          ...
          BasicVersion b;
          parse_version(&b, curr_version);
          coreq::print() % (full ? b.getFull() : (revision ? b.getRevision() : b.getPlain()));
        }
        if (argc != 3) {
          coreq::say_empty();
        }
  ...
  for (Versions::const_iterator it(versions.begin()); likely(it != versions.end()); ++it) {
    coreq::say() % it->getFull();
  }
  return EXIT_SUCCESS;
}
