...
**/
static void dump_help() {
	coreq::say(_("Usage: %s [options] EXPRESSION\n"
"\n"
...
"further information.")) %
	program_name %
	COREQ_CACHEFILE %
	COREQ_REMOTECACHEFILE1 %
	COREQ_REMOTECACHEFILE2;
}
...
int run_coreq(int argc, char** argv) {
	...
	VarDbPkg varpkg_db(var_db_pkg, !rc_options.quick, rc_options.care,
		...
		coreqrc.getBool("RESTRICT_INSTALLED"),
		coreqrc.getBool("CARE_RESTRICT_INSTALLED"),
	...
	if(unlikely(rc_options.test_unused)) {
		...
		if(likely(coreqrc.getBool("TEST_KEYWORDS"))) {
			print_unused(coreqrc.m_eprefixconf + USER_KEYWORDS_FILE1,
				coreqrc["KEYWORDS_NONEXISTENT"],
				all_packages);
			print_unused(coreqrc.m_eprefixconf + USER_KEYWORDS_FILE2,
				coreqrc["KEYWORDS_NONEXISTENT"],
				all_packages);
		}
		if(likely(coreqrc.getBool("TEST_MASK"))) {
			print_unused(coreqrc.m_eprefixconf + USER_MASK_FILE,
				coreqrc["MASK_NONEXISTENT"],
				all_packages);
		}
		if(likely(coreqrc.getBool("TEST_UNMASK"))) {
			print_unused(coreqrc.m_eprefixconf + USER_UNMASK_FILE,
				coreqrc["UNMASK_NONEXISTENT"],
				all_packages);
		}
		if(likely(coreqrc.getBool("TEST_USE"))) {
			print_unused(coreqrc.m_eprefixconf + USER_USE_FILE,
				coreqrc["USE_NONEXISTENT"],
				all_packages, empty);
		}
		if(likely(coreqrc.getBool("TEST_ENV"))) {
			print_unused(coreqrc.m_eprefixconf + USER_ENV_FILE,
				coreqrc["ENV_NONEXISTENT"],
				all_packages, empty);
		}
		if(likely(coreqrc.getBool("TEST_LICENSE"))) {
			print_unused(coreqrc.m_eprefixconf + USER_LICENSE_FILE,
				coreqrc["LICENSE_NONEXISTENT"],
				all_packages, empty);
		}
		if(likely(coreqrc.getBool("TEST_RESTRICT"))) {
			print_unused(coreqrc.m_eprefixconf + USER_RESTRICT_FILE,
				coreqrc["LICENSE_RESTRICT"],
				all_packages, empty);
		}
		if(likely(coreqrc.getBool("TEST_CFLAGS"))) {
			print_unused(coreqrc.m_eprefixconf + USER_CFLAGS_FILE,
				coreqrc["CFLAGS_NONEXISTENT"],
				all_packages, empty);
		}
	...
	if (!matches.empty()) {
		if(rc_options.xml) {
			...
			print_formats = new PrintXml(&header, &varpkg_db, format, &stability, &coreqrc,
				corepkgsettings["PORTDIR"]);
	...
	coreq::SignedBool print_count_always(rc_options.pure_packages ? -1 :
		coreqrc.getBoolText("PRINT_COUNT_ALWAYS", "never"));
