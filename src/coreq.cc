...
typedef coreq::ptr_container<vector<Package*> > PackageList;
...
ATTRIBUTE_NONNULL_ static void set_format(CoreqRc* rc);
ATTRIBUTE_NONNULL_ static void setup_defaults(CoreqRc* rc, bool is_tty);
...
static void dump_help() {
  coreq::say(
      _("Usage: %s [options] EXPRESSION\n"
        "\n"
        "Search for packages in the index generated by coreq-update.\n"
        ...
        "                           of /etc/corepkg/package.* and non-matching names\n"
        "                           of installed packages; this option is best\n"
        "                           combined with -T to clean up /etc/corepkg/package.*\n"
        ...
        "         --nowarn (toggle)  suppress warnings about wrong corepkg config\n"
        ...
        "                          +: settings from LOCAL_COREPKG_CONFIG=true\n"
        "                          -: settings from LOCAL_COREPKG_CONFIG=false\n"
        ...
        "                          /etc/corepkg/package.* (see man coreq)\n"
        ...
        "further information.")) %
      program_name % COREQ_CACHEFILE % COREQ_REMOTECACHEFILE1 % COREQ_REMOTECACHEFILE2;
}

static const char* formatstring;
static const char* coreq_cachefile(NULLPTR);
static const char* var_to_print(NULLPTR);
static const char* color(NULLPTR);

enum OverlayMode { mode_list_used_renumbered = 0, mode_list_used = 1, mode_list_all_if_any = 2, mode_list_all = 3, mode_list_none = 4 };

static OverlayMode overlay_mode;

static PrintFormat* format;

static coreq::TinyUnsigned remote_default;
...
static struct LocalOptions {
  bool ansi, palette256, palette256b, palette256d, palette256d0, palette256d1, palette256l, palette256l0, palette256l1, be_quiet, no_warn, quick, care, deps_installed, verbose_output, compact_output,
      normal_output, show_help, show_version, pure_packages, only_names, brief, brief2, dump_coreqrc, dump_defaults, known_vars, xml, proto, test_unused, do_debug, ignore_etc_corepkg, is_current,
      remote, remote2, hash_eapi, hash_iuse, hash_keywords, hash_slot, hash_license, hash_depend, print_profile_paths, world_sets;
} rc_options;

/**
Arguments and options
**/
class CoreqOptionList : public OptionList {
 public:
  CoreqOptionList();
};

CoreqOptionList::CoreqOptionList() {
  // Global options
  ...
  push_back(Option("dump", O_DUMP, Option::BOOLEAN_T, &rc_options.dump_coreqrc));
  ...
  push_back(Option("ignore-etc-corepkg", O_IGNORE_ETC_COREPKG, Option::BOOLEAN_T, &rc_options.ignore_etc_corepkg));

  push_back(Option("print", O_PRINT_VAR, Option::STRING, &var_to_print));

  push_back(Option("format", O_FMT, Option::STRING, &formatstring));

  push_back(Option("cache-file", O_COREQ_CACHEFILE, Option::STRING, &coreq_cachefile));
...
**/
static void setup_defaults(CoreqRc* rc, bool is_tty) {
  // Setup defaults
  ...
  format->no_color = (rc->getBool("NOCOLORS") ? true : (rc->getBool("FORCE_COLORS") ? false : (!is_tty)));
  ...
  else if (unlikely(overlay.find("no") != string::npos) || unlikely(overlay.find("false") != string::npos)) {
...
static bool print_overlay_table(PrintFormat* fmt, DBHeader* header, PrintFormat::OverlayUsed* overlay_used) {
  ...
  for (ExtendedVersion::Overlay i((overlay_mode == mode_list_all) ? 0 : 1); likely(i != header->countOverlays()); ++i) {
    ...
    coreq::say("%s %s%s%s") % s % fmt->color_overlayname % header->getOverlay(i).human_readable() % fmt->color_overlaynameend;
...
static void parseFormat(const char* sourcename, const char* content) {
  ...
  coreq::say_error(_("problems while parsing %s: %s")) % sourcename % error_text;
  std::exit(EXIT_FAILURE);
}

static void set_format(CoreqRc* rc) {
  if (unlikely(formatstring != NULLPTR)) {
    parseFormat("--format", formatstring);
    return;
  }
  coreq::SignedBool use_verbose(0);
...
int run_coreq(int argc, char** argv) {
  ...
  CoreqRc& coreqrc(get_coreqrc(COREQ_VARS_PREFIX));
  {
    string errtext;
  ...
  setup_defaults(&coreqrc, is_tty);

  // Read our options from the commandline.
  ArgumentReader argreader(argc, argv, CoreqOptionList());
  ...
  if (unlikely(color != NULLPTR)) {
    if (unlikely(strncasecmp("au", color, 2) == 0)) {
      ...
      format->no_color = !CoreqRc::istrue(color);
    }
  }
  ...
  // Dump coreqrc-stuff
  if (unlikely(rc_options.dump_coreqrc || rc_options.dump_defaults)) {
    coreqrc.dumpDefaults(stdout, rc_options.dump_defaults);
    return EXIT_SUCCESS;
  }
  ...
  string cachefile;
  const char* tooltext("coreq-update");
  if (unlikely(coreq_cachefile != NULLPTR)) {
    cachefile = coreq_cachefile;
  }
  else {
    ...
    if (rc_options.remote) {
      cachefile = coreqrc["COREQ_REMOTE1"];
      tooltext = "coreq-remote add1/update1";
    }
    else if (rc_options.remote2) {
      cachefile = coreqrc["COREQ_REMOTE2"];
      tooltext = "coreq-remote add2/update2";
    }
    if (cachefile.empty()) {
      cachefile = coreqrc["COREQ_CACHEFILE"];
    }
  }
  ...
  {  // Print color palette if requested
    ...
    if (unlikely(rc_options.palette256)) {
      ...
      palette = (unlikely(rc_options.palette256b) ? AnsiColor::PALETTE_B : AnsiColor::PALETTE_NONE);
  ...
  if (unlikely(rc_options.xml || rc_options.proto)) {
    ...
    only_printed = coreqrc.getBool("COUNT_ONLY_PRINTED");
  }
  ...
  CorePkgSettings corepkgsettings(&coreqrc, parse_error, true, false, rc_options.print_profile_paths);
  if (unlikely(rc_options.print_profile_paths)) {
    return EXIT_SUCCESS;
  }

  string var_db_pkg(coreqrc["EPREFIX_INSTALLED"] + VAR_DB_PKG);
  VarDbPkg varpkg_db(var_db_pkg, !rc_options.quick, rc_options.care, rc_options.deps_installed, coreqrc.getBool("RESTRICT_INSTALLED"), coreqrc.getBool("CARE_RESTRICT_INSTALLED"),
                     coreqrc.getBool("USE_BUILD_TIME"));
  varpkg_db.check_installed_overlays = coreqrc.getBoolText("CHECK_INSTALLED_OVERLAYS", "repository");
  ...
  if (unlikely(!db.read_header(&header, NULLPTR, 0))) {
    coreq::say_error(
        ...
        cachefile % header.version % DBHeader::current % tooltext;
    return EXIT_FAILURE;
  }
  ...
  if (unlikely(rc_options.world_sets)) {
    const WordVec* p(corepkgsettings.get_world_sets());
    for (WordVec::const_iterator it(p->begin()); likely(it != p->end()); ++it) {
      coreq::say() % (*it);
    }
    return EXIT_SUCCESS;
  }

  corepkgsettings.store_world_sets(&(header.world_sets));

  if (header.countOverlays() != 0) {
    header.set_priorities(&corepkgsettings);
    format->clear_virtual(header.countOverlays());
    for (ExtendedVersion::Overlay i(1); likely(i != header.countOverlays()); ++i)
      format->set_as_virtual(i, is_virtual((coreqrc["EPREFIX_VIRTUAL"] + header.getOverlay(i).path).c_str()));
  }

  LocalMode local_mode(LOCALMODE_DEFAULT);
  if (unlikely(!coreqrc.getBool("LOCAL_COREPKG_CONFIG"))) {
    rc_options.ignore_etc_corepkg = true;
    local_mode = LOCALMODE_NONLOCAL;
  }
  else if (!rc_options.ignore_etc_corepkg) {
  ...
  SetStability stability(&corepkgsettings, !rc_options.ignore_etc_corepkg, false, coreqrc.getBool("ALWAYS_ACCEPT_KEYWORDS"));

  MatchTree* matchtree = new MatchTree(coreqrc.getBool("DEFAULT_IS_OR"));
  parse_cli(matchtree, &coreqrc, &varpkg_db, &corepkgsettings, format, &stability, &header, parse_error, &marked_list, argreader);

  PackageList matches;
  PackageList all_packages;
  {
    PackageReader reader(&db, header, &corepkgsettings);
    bool add_rest(false);
    while (likely(reader.next())) {
      if (unlikely(add_rest)) {
        ...
        if (unlikely(only_printed && (rc_options.brief || (rc_options.brief2 && (matches.size() > 1))))) {
  ...
  if (unlikely(rc_options.test_unused)) {
    bool empty(coreqrc.getBool("TEST_FOR_EMPTY"));
    if (likely(coreqrc.getBool("TEST_KEYWORDS"))) {
      print_unused(coreqrc.m_eprefixconf + USER_KEYWORDS_FILE1, coreqrc["KEYWORDS_NONEXISTENT"], all_packages);
      print_unused(coreqrc.m_eprefixconf + USER_KEYWORDS_FILE2, coreqrc["KEYWORDS_NONEXISTENT"], all_packages);
    }
    if (likely(coreqrc.getBool("TEST_MASK"))) {
      print_unused(coreqrc.m_eprefixconf + USER_MASK_FILE, coreqrc["MASK_NONEXISTENT"], all_packages);
    }
    if (likely(coreqrc.getBool("TEST_UNMASK"))) {
      print_unused(coreqrc.m_eprefixconf + USER_UNMASK_FILE, coreqrc["UNMASK_NONEXISTENT"], all_packages);
    }
    if (likely(coreqrc.getBool("TEST_USE"))) {
      print_unused(coreqrc.m_eprefixconf + USER_USE_FILE, coreqrc["USE_NONEXISTENT"], all_packages, empty);
    }
    if (likely(coreqrc.getBool("TEST_ENV"))) {
      print_unused(coreqrc.m_eprefixconf + USER_ENV_FILE, coreqrc["ENV_NONEXISTENT"], all_packages, empty);
    }
    if (likely(coreqrc.getBool("TEST_LICENSE"))) {
      print_unused(coreqrc.m_eprefixconf + USER_LICENSE_FILE, coreqrc["LICENSE_NONEXISTENT"], all_packages, empty);
    }
    if (likely(coreqrc.getBool("TEST_RESTRICT"))) {
      print_unused(coreqrc.m_eprefixconf + USER_RESTRICT_FILE, coreqrc["LICENSE_RESTRICT"], all_packages, empty);
    }
    if (likely(coreqrc.getBool("TEST_CFLAGS"))) {
      print_unused(coreqrc.m_eprefixconf + USER_CFLAGS_FILE, coreqrc["CFLAGS_NONEXISTENT"], all_packages, empty);
    }
    if (likely(coreqrc.getBool("TEST_REMOVED"))) {
      print_removed(var_db_pkg, coreqrc["INSTALLED_NONEXISTENT"], all_packages);
    }
  }
  ...
  if (!matches.empty()) {
    if (rc_options.xml) {
      if (unlikely(rc_options.proto)) {
        coreq::say_error(_("--xml and --proto must not be specified simultaneously"));
        std::exit(EXIT_FAILURE);
      }
      print_formats = new PrintXml(&header, &varpkg_db, format, &stability, &coreqrc, corepkgsettings["PORTDIR"]);
    }
  ...
  string limit_var(rc_options.compact_output ? "COREQ_LIMIT_COMPACT" : "COREQ_LIMIT");
  coreq::Treesize limit(is_tty ? coreqrc.getInteger(limit_var) : 0);
  for (PackageList::iterator it(matches.begin()); likely(it != matches.end()); ++it) {
    ...
    if (it->largest_overlay != 0) {
      ...
      if (overlay_mode <= mode_list_used) {
        for (Package::iterator ver(it->begin()); likely(ver != it->end()); ++ver) {
    ...
    if (overlay_mode != mode_list_used_renumbered) {
      if (format->print(*it, &header, &varpkg_db, &corepkgsettings, &stability, reached_limit)) {
        have_printed = true;
        ++count;
  ...
  if (need_overlay_table) {
    if (print_overlay_table(format, &header, (overlay_mode <= mode_list_used) ? &overlay_used : NULLPTR)) {
  ...
  coreq::SignedBool print_count_always(rc_options.pure_packages ? -1 : coreqrc.getBoolText("PRINT_COUNT_ALWAYS", "never"));
  if (likely(print_count_always >= 0)) {
    if ((print_count_always != 0) || (count > 1)) {
      ...
      coreq::say("%s%s%s") % format->color_numbertext % (coreq::format(N_("Found %s match", "Found %s matches", count)) % count) % format->color_numbertextend;
    }
    else if (unlikely(count == 0)) {
      have_printed = true;
      coreq::say("%s%s%s") % format->color_numbertext % _("No matches found") % format->color_numbertextend;
    }
  }
  if (likely(have_printed)) {
    coreq::print() % format->color_end;
    if (unlikely(over_limit)) {
      coreq::say(
          N_("Only %s match displayed on terminal\n"
...
static bool opencache(Database* db, const char* filename, const char* tooltext) {
  ...
  coreq::say_error(
      _("cannot open database file %s for reading\n"
...
static void print_unused(const string& filename, const string& excludefiles, const PackageList& packagelist, bool test_empty) {
  ...
  for (WordVec::iterator i(lines.begin()); likely(i != lines.end()); ++i) {
    if (i->empty()) {
      continue;
    }
    if (unlikely(!know_excludes)) {
      ...
      for (WordVec::const_iterator it(excludelist.begin()); likely(it != excludelist.end()); ++it) {
        LineVec excl;
  ...
  if (unused.empty()) {
    coreq::say(test_empty ? _("No non-matching or empty entries in %s") : _("No non-matching entries in %s")) % filename;
    return;
  }
  coreq::say(test_empty ? _("Non-matching or empty entries in %s:") : _("Non-matching entries in %s:")) % filename;
...
static void print_removed(const string& dirname, const string& excludefiles, const PackageList& packagelist) {
  ...
  CatName cat_name;
  for (PackageList::const_iterator pit(packagelist.begin()); likely(pit != packagelist.end()); ++pit) {
  ...
  for (WordVec::const_iterator cit(categories.begin()); likely(cit != categories.end()); ++cit) {
    WordVec names;
    ...
    for (WordVec::const_iterator nit(names.begin()); likely(nit != names.end()); ++nit) {
      string curr_name;
  ...
  if (likely(failure.empty())) {
    coreq::say(_("The names of all installed packages are in the database."));
    return;
  }
  coreq::say(_("The following installed packages are not in the database:"));
